...
* **Linux:** Uses `direnv` + Nix flake. If `command not found: cargo` happens, use `zsh -il -c "cd <path_to_project> && cargo <command>"`. **Make sure to confirm that `cargo` cannot be executed directly before proceeding to execution with `direnv`.**
...
2. **Implement:** `#[derive(ene_tool_derive::ToolSpec)]` on the args struct.
3. **Wire up:** Call `run_tool_server::<MyAction>()` from `ene-tool-proto` inside `main`.
