...
| `fs` | `ene-tool-fs` | Read / write / edit / list / glob / delete / search / patch / undo in the workspace. No shell. `fs.glob` is workspace-relative and capped without following directory symlinks. `fs.delete` is approval-gated and removes only files or empty directories. Search is literal unless `regex` is set and delegates to host `rg`, with optional `include`, case-insensitive, context, count, capture-group, and line-number modes. `fs.read` returns `text` and a blake3 `hash` of the raw file bytes. `fs.write`, `fs.edit`, and `fs.patch` accept optional `expected_hash`; a mismatch fails with a stale-precondition error and leaves the file unchanged. Writes use a temp file plus rename, and per-path operations serialize. Edits try exact matching first, then CRLF-normalized indent-tolerant, line-trimmed, and block-anchor fallbacks; multiple matches without `replace_all` are ambiguity errors. Line endings (CRLF/LF), UTF-8 BOM, and trailing newlines are preserved. `fs.undo` only reverts writes from the same job (`job_id` or `ENE_JOB_ID`); secret-looking paths and bodies over 1 MiB are not stored in the undo journal. Unified diffs match hunk context, not only line numbers. The `tool.fs` child has no workspace filesystem rights; fs tools are grant-checked by the fiber FileBroker and executed host-side. |
| `exec` | `ene-tool-exec` | Process execution by program name or shell (`exec.run` / `exec.shell`). Output is capped while streaming (1 MiB stdout, 1 MiB stderr, 2 MiB combined) with truncation metadata. Timeouts signal the process tree via `process-wrap` (Unix process group TERM then KILL; Windows Job Object terminate). Working directory is confined to `ENE_WORKSPACE`; inherited env is allowlisted and secret-shaped vars are dropped. |
