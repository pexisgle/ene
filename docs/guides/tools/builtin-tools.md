...
| `web` | `ene-tool-web` | HTTPS fetch and public search. The host net broker performs every hop (SSRF, DNS pin, 1 MiB stream cap, text content types). The plugin process is network-isolated and cannot dial HTTP itself. Search uses DuckDuckGo answers, then HTML fallback when the instant API is empty. |
