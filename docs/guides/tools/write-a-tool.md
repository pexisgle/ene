...
`ene_tool_registry::run_tool_plugin` with **local** `specs` and `execute`.
...
`ene-plugin-ipc` + `ene-tool-registry`, `src/logic.rs` for the tool body, and a
`main` that calls `ene_tool_registry::run_tool_plugin("my_tool", logic::specs, logic::execute)`.
