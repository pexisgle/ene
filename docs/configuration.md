...
Conversation, classifier, embedding, TTS, STT, and approve bind through
`ai.tasks.<task>`
...
fallback) and pops up when that helper fails. API keys are vault secrets (`ENE_AI__TASKS__<TASK>__API_KEY`
