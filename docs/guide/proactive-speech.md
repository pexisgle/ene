...
- When `sources.screen_summary` is enabled, desktop captures a **fresh** frame at ~50% scale on every observe tick (no cross-tick cache): active window when another app is focused, **full display** when Ene is focused. On Wayland the portal `ActiveWindow` / `Screen` targets are used; on X11 active-window vs primary-monitor capture applies. Summarizes with the **local** proactive GGUF + `mmproj` (Gemma 4 multimodal) using the OS app label as a prior, then discards the desktop-side image. The runtime keeps an ephemeral JPEG of that frame for the next generation turn when `ai.tasks.chat` (or cloud `ai.tasks.proactive`) has `supports_vision: true`. That observe cycle immediately drives the decision LLM. The decision model first writes `screen_digest` (reorganized screen sketch) before `reason`. If vision fails, the screen source is marked unavailable (no fabricated summary).
