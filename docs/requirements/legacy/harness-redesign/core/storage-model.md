...
> **P-518**(スキーマ進化)、P-110(エクスポートの基礎)、
> **P-525**(重複効果なしのクラッシュ回復)の記憶面。
> 参照: pi の「three stores, one invariant」を Ene のイベントソーシング設計に適合させる。

[session-log.md](session-log.md) が「何が記録されるか」(語彙)を定義するのに対し、
この文書は「どう永続化されるか」(機構)を定義する。

## 0. 段階の切り分け

この文書は v1.0 と後継設計をまたぐ(D-4)。

| 節 | 段階 |
|---|---|
| entries(会話ログの追記ストア) | **v1.0** |
| usage ledger(使用量台帳、P-516) | **v1.0** |
| スキーマ進化(P-518) | **v1.0** |
| registers(操作状態のセル) | **後継**(P-525) |
| 3ストア不変条件・精密書き換え | **後継**(P-525) |

さらに、**イベントソーシングの適用範囲は会話ログに限る**(D-9)。
記憶・スケジュール・成果物・設定は通常のテーブル構造(`companions.db`)で持ち、
このストレージモデルの対象外である。追記ログにすると得るものが少ないのに
実装量が跳ね上がるため。会話ログを追記式にするのは、fork(P-109)が接頭辞
コピーだけで実装でき、compaction と秘匿処理が「上書き」ではなく
「新イベント + 投影規則」になり、UI 履歴・モデル履歴・エクスポートが
1つの投影関数から出るからである。

読み取りが必ず投影を通るため、素朴に書くと履歴取得が O(イベント数) になる。
v1.0 は compaction(P-506) で投影結果を有界にし、スナップショットや
投影キャッシュは持たない(D-9)。
...
v1.0 の可変面は会話ログのイベント payload と使用量台帳である。レジスタは
持たない(D-4)。オープン時の連鎖マイグレーションは、既知の kind の読み込み
互換と、未知 `v` の読み飛ばしを保証する。開いているターン/job を表す
ログイベントも変換対象だが、変換後は [agent-loop.md §12](agent-loop.md#12-中断の検出と報告p-515--d-5)
に従い中断として確定する。実行は再開しない(D-5、P-518)。

`storage_version` と「version > 現行ならオープン拒否」は v1.0 から持つ。
マイグレーション連鎖の途中でプロセスが死んだ場合、記録済み version から
**マイグレーション自体**を続ける。これは job の再開ではない。
