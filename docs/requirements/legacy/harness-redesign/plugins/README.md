...
| [composition.md](composition.md) | ファイバー・巻き戻し可能な effect・反応的な依存・システム境界 | P-1010, P-1005 のライフサイクル面 |
| [manifest-and-profile.md](manifest-and-profile.md) | manifest・能力宣言・プロファイル/バンドル/パッチ・差分 reconcile・コミュニティ形式 | P-1002, P-909, P-1008, P-616, P-1010 の宣言面 |
...
- **ホスト文脈は時空間合成する**(D-32)。子プロセスの kill はプラグイン側の
  メモリしか捨てない。ツール登録・seam・Broker 委譲はファイバーの
  巻き戻しで回収し、行の依存は `provides`/`requires` で再評価する。
  プロファイル差分はファイバー単位で reconcile し、デーモンごと落とさない。
...
1. `ipc.md`(線)
2. `composition.md`(ホスト文脈の寿命)
3. `manifest-and-profile.md`(誰が乗るか、差分をどう適用するか)
4. `broker.md`(資源の仲介)
