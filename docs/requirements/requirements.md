...
- **会話と管理の役割分担**: 一対一の会話とグループ会話は見やすく区別します。通常のタスク操作と報告はパートナーとの会話で行い、番号付き作業一覧や専用のタスク操作画面を必要としません。設定、記録の診断、認証、安全操作は用途に応じた画面から利用できます。
- **作業中も会話可能**: パートナーが裏でタスクを実行している間も、チャット画面がタスクログで埋め尽くされることはなく、普段通りのおしゃべりを楽しめます。

### UIの優先順位

- **会話を中心にしたすっきりした画面**: 普段の画面は、パートナーの姿（アバター）、会話タイムライン、現在のパートナーの様子を中心に配置します。タスクの状況や報告は必要なときに会話で伝え、専用の進捗表示を常設しません。
...
- **アバターが停止しても安心**: 万が一アバターの描画がクラッシュしても、チャットでのタスク操作、設定、安全操作の画面はそのまま問題なく利用できます。
...
- **音声とテキストの併用**: タスクの依頼・状況確認・変更・中止・再開と報告は音声で行えます。報告は会話タイムラインにもテキストで提示し、音声が使えないときも同じ会話をテキストで続けられます。音声入力や読み上げの停止だけで、受理済みのタスクを中止したとは扱いません。
- **静かな報告と未伝達**: 出力ミュートや静粛時間中の自発的な読み上げは控えます。報告を生成しただけでは伝達済みにせず、音声もテキストも提示できなければ復帰後に現在の状況を伝えます。変化のない進捗を繰り返し話し続けません。
...
一方で、パスワードなどの機密情報の保護や、プライバシー目的の完全削除（targeted deletion）といった安全ルールは、AIの解釈だけに頼らずシステム側で機械的に確実に適用します。
...
- **文脈に沿った対象の確認**: パートナーは依頼内容と会話の文脈から対象の作業を確かめます。複数の候補があるときは尋ね、ユーザーにタスク番号や一覧からの選択を要求しません。不足する Workspace や作業条件も会話で尋ねますが、新たなアクセス許可や高権限操作に必要な正式な確認を音声だけで済ませません。
...
- **結果のわかりやすい報告**: タスクの状況、完了、判断待ちを、確定した変更、作成・編集したファイルと保存場所、残作業、結果不明な作用を区別してパートナーが音声と会話タイムラインのテキストで伝えます。成果物の確認・プレビュー・保存操作は引き続き利用できます。

#### 切断中の報告と中断した作業の再開

- Client の切断や接続の置き換えは、Host 上で継続中の作業をキャンセルする指示ではありません。現在の許可で Host だけで進められる部分は継続し、端末操作や新たなユーザー確認が必要な部分だけを実行せずに保留します。
- 不在中の進捗・完了・結果は Host に残し、次の Client で現在の状況をまとめて伝えます。報告の対象事項は、その回の要約本文全体が会話タイムラインに実際に表示された場合、または音声の再生が完了した場合に限り提示済みとします。音声だけが失敗してもテキスト提示が成立していれば提示済みです。どちらも成立しなければ未伝達のままにします。生成・送信・受信だけでは報告済みにせず、提示確認は既読、理解、承認、作業の成功を意味しません。提示確認が失われた場合には、同じ事項が再提示されることがあります。
- Host 再起動で実行が途切れても、タスクの進捗を失敗やキャンセルへ書き換えません。保存済みの結果を現在の条件と照合して完了を確定することはできますが、そのために AI や外部操作を再実行してはいけません。作業を続ける場合は、保存済みの進捗・確定した変更・不明な作用・残作業を示してユーザーの明示指示を受けます。
- 再開は、未完了の同じタスクについて残作業を進める新しい指示です。古いエージェントや未送信操作を再生しません。結果不明の外部作用が残る間は通常の再開を保留し、その作用の確認を求めます。単に「再開」と指示したことを二重実行リスクへの同意とは扱いません。
- 完了・達成不能と確定した失敗・キャンセル済みのタスクは再開しません。もう一度行う場合は別のタスクとして依頼します。以前のタスクに残る不明な作用はそのまま報告し、新しい依頼の許可と混同しません。
- 再開要求と新しい指示や結果確定が競合した場合は、古い前提を自動で最新のものに置き換えて実行せず、現在の状態を示します。画面の復帰や報告の提示だけで中断した作業が動き出すことはありません。
