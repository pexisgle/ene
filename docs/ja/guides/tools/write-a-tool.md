...
`ene_tool_registry::run_tool_plugin` に渡して登録します。
...
`ene-plugin-ipc` + `ene-tool-registry`、本体は `src/logic.rs`、`main` から
`ene_tool_registry::run_tool_plugin("my_tool", logic::specs, logic::execute)` を呼びます。
