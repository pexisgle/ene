...
コア寿命、`character_positions` による体ごとのオーバーレイ配置、アバター直接反応の設定）はクライアント側です。
テーマ（`light` / `dark` /
