...
| キャプション | `surface` | ストリームされた発話。プロバイダ / HTTP エラー（`The chat provider failed…`、`401 Unauthorized`）はチャットのステータス行に留め、字幕としては出さない。ターン終了（`turn/end`）でオーバーレイは閉じる。長い行はオーバーレイ内で折り返す。 |
