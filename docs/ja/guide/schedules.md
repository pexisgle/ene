...
スケジュールは単一のアプリインスタンスで管理されます：スケジュールを有効にした
まま 2 つの Ene プロセスを同じ `memory.db` に対して実行しないでください。2 番目の
インスタンスの起動時リコンサイルが、最初のインスタンスの実行中ジョブを
`interrupted` と記録し、2 つのタイマーが同じ実行時刻を奪い合うことになります。
