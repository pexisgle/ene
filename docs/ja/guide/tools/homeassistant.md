...
- `base_url` — インスタンスの URL。例: `http://homeassistant.local:8123`。
  ...
  必要があります (例: `https://home.example.com/ha/`)。受け付けるのは
  `http` と `https` のみです。
- `token` — Home Assistant の **プロフィール → セキュリティ → 長期アクセス
  トークン** で作成する長期アクセストークン。

> **セキュリティ:** 平文の `http://` を使うと、長期アクセストークンが
> すべてのリクエストでネットワーク上を暗号化されずに送信され、同じ
> ネットワーク内の誰でもトークンを傍受してスマートホームを操作できて
> しまいます。`https://` (例: TLS リバースプロキシの背後に置いた Home
> Assistant) を推奨します。少なくとも `http://` の利用は信頼できる
> ネットワークに限定してください。Ene は設定されたベース URL が `http`
> の場合、起動時に警告をログ出力します。
...
Ene はホストのログから編集 (redact) します。設定 UI によるシークレット
欄のマスクは今後実装予定です。環境変数
...
ただし自宅の Home Assistant インスタンスへのネットワーク通信は発生する
ため、上記の通信路のセキュリティ注意がこのアクションにも適用されます。
...
「このセッションでは許可」を選ぶと会話の間中許可が続きます。直接ツール
呼び出し (`ene-cli tool call`) は新しい合成ターンとして実行されるため、
チャット中に付与された承認を引き継ぐことはありません。
