...
- **初期化失敗:** スタイル付きのローカライズされたエラーを stderr に表示し、終了コード `1` で終了します。
- **ターン失敗（REPL）:** ストリーム終了後に `TerminalReason::Failed` を stderr に表示します。
- **ターン失敗（`ene run`、テキストモード）:** 終了前にローカライズされた失敗メッセージを stderr に表示します。
- ランタイムエラーは `apps/ene-cli/src/runtime_error.rs` で Fluent 文字列にマッピングされます（EN/JA カタログ）。
