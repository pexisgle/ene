...
| **PR5 — v2: クリックスルー + オフスクリーン マスク** | §4 PR5 | **PR5.2 出荷済み (Windows, per-bone sphere collider に再々設計)**、PR5.3+ 未着手 | PR5.1 の BVH-trimesh 方式は rest pose 固定で per-frame アニメーションに追従できず、デフォルト VRMA 再生中に顔・足先がクリックできない問題があった。PR5.2 で per-bone sphere collider 方式に変更: `CharacterRenderer::build_character_bone_data` が `model.humanoid` を walk して各 bone の rest-pose world position と auto-sized radius (子 bone への最長距離 × 0.6 + リーフ bone は 0.05 m デフォルト、0.025 m 未満は除外で指をフィルタ) を取り、`PhysicsWorld::add_character_bone_colliders` で world 原点に置いた単一 kinematic body にぶら下げる。`about_to_wait` で `current_bone_local_positions` (これが `model.nodes.world_positions` を読み、`update_skin_palette` が毎フレーム更新する) → `update_character_bone_positions` → 各 collider の `set_translation_wrt_parent` でローカル位置更新。アニメーションに追従しつつ per-frame GPU readback ゼロ。`apps/ene-desktop-v2/src/platform/` モジュールは PR5.1 で削除済み (Wayland 実装時に復活)。 |
...
> **ステータス (2026-06-19):** **PR5.2 (Windows) 出荷済み — per-bone Rapier sphere collider 方式に再々設計。** PR5.1 の BVH-trimesh 方式は rest pose 固定で per-frame アニメーションに追従できず、デフォルト VRMA 再生中に顔・足先がクリックできない問題があった。PR5.2 で `CharacterRenderer::build_character_bone_data` が `model.humanoid` を walk して各 bone の rest-pose world position と auto-sized radius (子 bone への最長距離 × 0.6 + リーフ bone は 0.05 m デフォルト、0.025 m 未満は除外で指をフィルタ) を取り、`PhysicsWorld::add_character_bone_colliders` で world 原点に置いた単一 kinematic body にぶら下げる。`about_to_wait` で `current_bone_local_positions` (これが `model.nodes.world_positions` を読み、`update_skin_palette` が毎フレーム更新する) → `update_character_bone_positions` → 各 collider の `set_translation_wrt_parent` でローカル位置更新。アニメーションに追従しつつ per-frame GPU readback ゼロ。PR5.3 以降 (Wayland input region、X11 shape、Linux オフスクリーン マスク + gizmo、フレーム ペーサー) は未着手のまま。
...
1. **Windows クリックスルー (PR5.2 — 2026-06-19 出荷済み, 2 度目の再設計)** — 3 つのピース、Win32 を直接叩かない:
   ...
   - ヒットテストは per-bone Rapier sphere collider に対する raycast。`CharacterRenderer::build_character_bone_data` が `model.humanoid` を walk して各 bone の `(local_position, radius)` を発行。radius は `BONE_RADIUS_RATIO × (子 bone への最長距離)`、子なし bone は `DEFAULT_LEAF_BONE_RADIUS = 0.05` m デフォルト、`MIN_BONE_RADIUS = 0.025` m 未満で除外 (指)。`PhysicsWorld::add_character_bone_colliders` で world 原点の単一 kinematic body にぶら下げ。`update_character_bone_positions` が `current_bone_local_positions` (これが `model.nodes.world_positions` を読み、`update_skin_palette` が毎フレーム更新する) → 各 collider の `set_translation_wrt_parent` でローカル位置更新 → アニメーション追従、per-frame GPU readback ゼロ。当たり判定が実モデルより少し大きくなる (ユーザ許容済み)。
   - `apps/ene-desktop-v2/src/platform/` モジュールは PR5.1 で削除済み (Wayland 実装時に復活)。
