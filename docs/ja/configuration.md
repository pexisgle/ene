...
デスクトップの設定 UI は下書き経由で編集します。変更は登録済みスキーマで
検証され、原子的に保存された後、ランタイムへ送られます。ランタイムは
自身の設定との差分を適用し、実際の影響（即時 / ホットリロード /
プラグイン再起動 / アプリ再起動）を返します。ランタイム適用が失敗した場合は
保存済み設定をロールバックします。プラグイン設定
（`plugins.list.<name>` の `config` / `profiles`）はプラグイン自身の
JSON Schema で編集され、`x-ene-ui` フィールドメタデータと
`x-ene-profiles-schema` 拡張に対応します。未知のキーは保持されます。
また、古いランタイムリビジョンに基づく下書きは競合として拒否され、
複数の書き込み元が互いの変更を黙って上書きすることはありません。
`ai.local_models` は適用時に `plugins.list.local-llm` / `llama-server` の
プロファイルから自動導出されるため、プロファイル側だけを編集します。
