...
会話・分類・埋め込み・TTS・STT・ジョブは `ai.tasks.<task>`（`plugin`、`model`、
`model_path`、`base_url`、`voice`、`max_tokens`）でバインドします。チャットは
未設定のまま起動するので、最初のメッセージの前に `ai.tasks.chat.plugin` を
`provider.*` に設定してください。裏層ジョブは `ai.tasks.job`（無ければ chat。
どちらも未設定なら echo）を、対話レーンとは別のレーンで使います。API キーは vault 秘密です（起動時は
