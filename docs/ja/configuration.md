...
ツールが権限または対話的なユーザー入力を要求すると、ランタイムは消費者（デスクトップ UI・CLI・自動化など）の応答を待ちますが、**無期限には待ちません**。`permission_prompt_timeout_ms`（既定 `300000`、5 分）と `user_input_prompt_timeout_ms`（既定 `600000`、10 分）がそれぞれの待ち時間に上限を設け、さらに各待機はターンのキャンセル トークンとも select されます。消費者が一切応答しない場合（イベントの消失、ヘッドレス消費者、閉じられたウィンドウなど）は、プロンプトは**フェイルセーフ**として扱われます。権限は*拒否*、ユーザー入力は*キャンセル*として処理され、ターンは必ず `Terminal` に到達してターンゲートが解放されます。以降のすべての `run()` が `RunError::Busy` で詰まることはありません。入力タイムアウトの既定値が大きいのは、承認/拒否のクリックより入力の打ち込みの方が時間がかかるためです。

どちらのタイムアウトも `0` に設定しても待機は**無効になりません**。プロンプトは即座にタイムアウトします（消費者がすでに回答している場合を除き、権限は*拒否*・ユーザー入力は*キャンセル*としてフェイルセーフ処理されます）。消費者が回答に長い時間を正当に必要とする場合は、大きい値を使用してください。
