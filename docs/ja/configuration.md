...
1. **キャプチャ + ROI 合成**: フル解像度でキャプチャし、ビジョンモデルには 50% 縮小の概要と、
   ...
   キャプチャ面のグローバル座標の両方が分かる場合のみです: X11（xcap のウィンドウ/モニター）と、
   Wayland の KWin/Hyprland（`active_win_pos_rs` 経由）。GNOME Wayland ではウィンドウの
...
2. **差分ゲート**: 毎 tick、概要と ROI の 64×64 グレースケール指紋を計算します。
   ...
   かつ ROI の指紋が不変の場合、キャッシュされた要約を再利用してローカルビジョンモデルを
   **実行しません**。ウィンドウ切替・スクロール・ROI 内の編集・サーフェスのリサイズでは
   再推論が強制されます。ヒットは構造化ログの `event="screen_diff_gate"` と
   `cached=true` で確認できます。
