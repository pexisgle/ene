...
- 制御面は diagnostics ファサードではなく `EneHandle` 自体にあります (#406): `set_character` (キャラクターカード交換) と `compress_context` (手動の圧縮専用パス、`ene_mind::CompressionResult` を返す — セッション ID は変わりません)。`EneHandle::diagnostics()` は厳密に観測用です: パイプライン詳細、プロバイダヘルス、メモリ/ジャーナル検査、アクタースナップショット。
- メモリ面 (`EneDiagnostics::memory` → `MemoryHandle`) は diagnostics ファサードがメモリを変更する唯一の場所です: pin/status/restore/forget、約束のライフサイクル (`complete_commitment` / `cancel_commitment`)、保留書き込みの検査と drain、ストアのバックアップ/整合性診断。生の `MemoryStore` は公開しません — かつて存在した `store()` の裏口は削除され (#409)、消費者が DB ハンドルに直接到達することはできません。約束の書き込みは、`ene_mind::commitments::CommitmentLedger` がステートレス (毎回 `commitments` テーブルを再読込) であるためアクター安全です。UI 側の書き込みがアクター側のキャッシュと不整合になることはありません (#409)。
