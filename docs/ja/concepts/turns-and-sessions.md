...
`text.delta` は発話の逐次チャンクです。`provider.openai_compat` と
`provider.gguf` はトークン到着ごとに `LlmChunk` を送り、他の LLM プロバイダは
一括生成のあとレーンが 1 チャンクを転送します。セッションログは確定した
アシスタント発話を一度だけ書きます（モデル可視 = ログ）。チャンク送信中の
`abort` はターンを `interrupted` で終え、そのアシスタント発話は書きません。
...
委譲 id）を出すので、クライアントは ask-user フォームを閉じられます。
