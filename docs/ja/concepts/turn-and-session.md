...
アイデンティティカーネルの `Speech style` 行はキャラクター自身の宣言です。
カードが `extensions.ene.speech`（長さ / 一人称・二人称 / 丁寧さ / 口癖）を
定義していればそれをレンダリングし、そうでなければ簡潔な言語別既定文を返します。
プロンプトのテキストから推論することは一切ないため、日本語カードの話し方が英語の
...
### 感情の動態
- **自然減衰**: 時間経過に伴い、感情値は基準値に向かってゆるやかに減衰します。基準値はキャラクターごとにカードの `extensions.ene.affect_baseline`（全8次元）で定義でき、未定義のカードは全 0 を基準として従来どおり動作します。なお、現時点で減衰に効くのは減衰対象である5次元（valence / arousal / dominance / irritation / fatigue）のみです。
