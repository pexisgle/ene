...
ターン数の項こそが**緩やかな**ずれを検出可能にする累積器です。話題に密着して追従する重心は距離の項を小さく保ちますが、長すぎる話題はターン数によって依然として検知されます。`mind.topic_boundary.min_utterance_chars` 未満の短い発話は相槌として扱われ、境界の評価にも重心の更新にも寄与しないため、「うん」が話題モデルを壊すことはありません。ハードコードされた語リスト（「ところで」「話は変わるけど」等）は使わず、埋め込みがシグナルを担うことで、検出器は言語をまたいで機能します。

検出は記憶書き込みや感情分類と同じ遅延ポストタースロットで実行されるため、応答を遅延させることはなく、完了したターンのユーザー入力埋め込みを判定材料にします。重心のライフサイクルは以下の通りです：
