...
### 能力の宣言 (`x-ene-capabilities`)

共有ランタイム能力（ローカル推論エンジン `gguf-runner` やパイプライン部品
`g2p/ja` など）を提供・依存するプラグインは、`config_schema()` が返す
スキーマのトップレベルで、既存の `x-` マーカーと同じ場所に宣言します:
...
- `provides` — このプラグインが提供する能力。各エントリは `name@version`。
  バージョンは具体的な semver バージョンです（`1` は `1.0.0` を意味します）。
  **major の引き上げは能力契約の非互換変更**です — ワイヤ ABI と同じ規律で、
  利用側がこのバージョンに対する範囲で依存を表現するためです。
- `requires` — このプラグインが必要とする能力。各エントリは `name@range`
  で、`range` は semver `VersionReq`（`1`、`^1`、`>=1.2.0, <2` など）です。
  末尾の `?` は**ソフト**要求を意味します（`g2p/ja@^1?`）: 未充足でも
  プラグインは起動し、内蔵実装へフォールバックすることが期待されます。

**名前** は `[A-Za-z0-9._-]` に加えて `/` による名前空間セグメントを最大
1 つ許します（`gguf-runner`、`tts/synthesize`、`g2p/ja` はいずれも有効）。
名前とバージョンを分ける `@` は名前文字ではありません。**プリリリース・
ビルドメタデータは、バージョン・範囲とも禁止**です。プリリリース表記は
`?` ソフトマーカーと衝突し、ビルドメタデータは順序に影響しないためです。

**解決。** ホストは起動に成功したプラグインの `provides` に対して、各
プラグインの `requires` を起動時に解決します。解決は**不動点に達するまで
繰り返されます**: プラグインは自身もコミットされるプロバイダーにのみ依存
できるため、ハード要求が未充足で無効化されたプラグインは他プラグインの
`requires` を満たすことがなく、その宣言はホストのレジストリから除去されて
解決 API に一切現れません。**ハード**要求が未充足のプラグインは、監視開始
前にシャットダウンされ、ツール・LLM プロバイダーを一切登録せず、不足能力
を列挙した `PluginHealthEvent::RequirementsUnmet`（ステータス
`requirements_unmet`）を発行します。ソフト要求は起動を妨げません。複数の
プラグインが一致する能力を提供する場合、ホストは決定的に 1 つを選びます
（プラグイン名の辞書順最小）。ただし正確な順序は **API 契約ではありませ
ん** — 呼び出し側は「一致するプロバイダーが選ばれる」ことだけを前提にし
てください。利用者が実装を選ぶ選好機構は次スライスで設計します。未充足で
無効化されたプラグインは、同じホストセッション中にプロバイダーが現れて
も**再評価されません**。ホストを再起動してください。

**検証のタイミング。** 宣言はプラグイン起動時に検証されます。エントリは
それぞれ独立にチェックされ、不正なエントリは警告のうえ無視されます
（プラグイン自体は起動します）。同一名の重複は最初の宣言が採用されます。
能力の*呼び出し*（プラグイン A がプラグイン B の能力をホスト経由で呼ぶ）
は次スライスの対象で、この節は宣言と解決までを扱います。

**`PluginCapabilities` との混同に注意** — これはハンドシェイク時に広告
される *IPC* 能力（ツール数、LLM プロバイダー、dynamic-config フラグ）で、
単一プラグインの IPC 面の能力を表します。`x-ene-capabilities` はプラグイン
同士が共有する *機能レベル* の契約で、性質が異なります。
