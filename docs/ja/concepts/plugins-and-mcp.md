...
**解決。** ホストは起動に成功した全プラグインの `provides` を束ね、各
プラグインの `requires` を起動時に一度だけ解決します。**ハード**要求が
未充足のプラグインは、監視開始前にシャットダウンされ、ツール・LLM
プロバイダーを一切登録せず、不足能力を列挙した
`PluginHealthEvent::RequirementsUnmet`（ステータス `requirements_unmet`）を
発行します。ソフト要求は起動を妨げません。複数のプラグインが一致する能力を
提供する場合、ホストは決定的に 1 つを選びます（プラグイン名の辞書順最小）。
ただし正確な順序は **API 契約ではありません** — 呼び出し側は「一致する
プロバイダーが選ばれる」ことだけを前提にしてください。利用者が実装を選ぶ
選好機構は次スライスで設計します。未充足で無効化されたプラグインは、同じ
ホストセッション中にプロバイダーが現れても**再評価されません**。ホストを
再起動してください。
