...
- **プレビュー付き書き込み確認。** すべての変更系アクションはストアに
  ...
  され、`update_event` は変更前 → 変更後の差分を表示します。リクエスト ID
  は `(action, target)` の決定的ハッシュなので、承認後の再呼び出しは
  記録済みの承認と一致して再プロンプトされません。「セッション中許可」は
  `(action, target-prefix)` パターンとして記録され、以降ゲートを通過します。
- **プライバシー。** 予定の本文（タイトル・メモ・参加者）はプラグインの
  ログやホストの監査ログに一切登場しません。パーミッションの `target` は
  `calendar:<id>` / `calendar:<id>#<イベントID>` のような安定した識別子で、
  監査ログには `action`・`target`・決定のみが記録されます。本文が表に
  出るのは必要な場所だけです: 承認プロンプト（ユーザー向け表示）と、
