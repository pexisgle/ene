...
保守的な選択です。実用上の帰結として、組み込みと同じ名前のバイナリを配置した
ユーザーには、自分のものではなく組み込みが実行されて見えます。挙動を上書きする
には、別のプラグイン名を選んでください。
...
- `kind: "oauth2"` — ホストが駆動する OAuth2 フロー。`client_id` は PKCE
  ...
  エンドポイントです。両エンドポイントは HTTPS 必須で、平文の `http://` は
  ループバックアドレス（ローカル開発サーバーなど）のみ許可されます。平文
  エンドポイントはフロー開始時またはリフレッシュ時に拒否されます。フロー・
  自動更新・保管の詳細は [資格情報](credentials.md) を参照してください。
