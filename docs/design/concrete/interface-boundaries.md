...
`create_task`（会話・第一者管理・自発・run now の新 Task）と `create_delegation` は、担当 Companion の canonical `Running` と削除開始の新規活動禁止 hold 不在を、Stop の `Stopped` commit / hold 確定と同じ SQLite master の短い比較・commit 点で確認します。事前に読んだ Running、会話の currentness、既存 Task の非 terminal はこの照合の代わりになりません。Stop / hold が先なら新しい Task 一式 / delegation を書かず、AU2 の `TaskCreationOutcome::CompanionUnavailable` / AU3 の domain refusal を返します。作業担当は AU2 の型付き結果を `TaskProposalOutcome` または H-H の受付結果へ写し、技術的エラーや架空の TaskRef にしません。AU2 / AU3 が先に commit した場合はその Task / delegation を維持し、後続 Stop は Task の Cancel と実行中処理の停止を best-effort で試みます。既存 attempt、結果、`Unknown` は消しません。予定回の Task 作成は AU18 の同じ比較・commit 点に含め、Stop の停止区間内に due となった回だけ `Missed`・Task なしとし、Stop より前に due となった未受付回は停止済みと推測せず `Unclassified`・Task なしで保護します。
...
enum TaskProposalOutcome {
    ...
    Superseded,                      // 会話起点の旧 Owner 入力が新しい accepted 入力で置換。Task 一式なし
...
- **Task 作成の結果マッピング**: AU2 の `Created` / `AlreadyCreated` は受理済み TaskRef、`CompanionUnavailable` / `Superseded` / `HeldByGlobalHold` は同名の候補結果へ写し、Schedule の拒否は H-H の受付結果へ写します。事前 read の後に新しい Owner 入力が commit した場合も AU2 の同一 master 比較で `Superseded` を返し、技術的障害や `StaleSchedule` に丸めません。
...
Backup 以外の Schedule の `RunNowScheduleCommand` も認証済み Owner source と指定 schedule の現行版（`expected_version` と一致）・有効な内容 / 初期 Workspace 入力・担当 Running Companion・hold を AU2 の同じ比較点で照合します。現在の設定から目的 / 担当 / Workspace を確定して通常 Task を新規作成し、`Normal.schedule_run_now` に schedule・照合版・Owner source を保存します。事前に組み立てた古い目的や Workspace を現行版のものとして採用しません。Schedule 変更・停止・削除が先なら Task を作らず stale / unavailable を返し、Task 作成が先ならその Task を保持します。元の `Missed` / `Unclassified` / `Started` 回と cursor は変更しません。同じ Owner source の受付再送は新規作成の currentness gate より先に保存済みの帰属と内容を照合して既存 Task を返し、別の schedule・Task 種別への再利用は拒否します。作成後の委任と実行は通常 Task の gate を通り、Schedule の後続変更だけで既開始 Task を失効させません。
...
|---|---|---|
| タスク提案・方針指示・委任（コマンドレベル） | `TaskProposalOutcome`、`DelegationOutcome` | 受理（Accepted）／会話作成の後続 Owner 入力先勝ち（Superseded：書き込みなし）／前提不一致（StalePremise：現在値付き）／タスク終端（TaskTerminal：Completed / Failed / Cancelled のため書き込みなし。方針指示・委任時）／タスク未存在（MissingTask：方針指示・委任時）／リビジョン上限超過（RevisionExhausted：方針指示時のみ）／全体保留中（HeldByGlobalHold：hold スライスで追加）／再照合が必要（NeedsRevalidation）／情報不足（InsufficientContext）。TaskProposalOutcome は候補側の意味論をすべて持ち、DelegationOutcome は HeldByGlobalHold・NeedsRevalidation・InsufficientContext を、それぞれの producer を持つスライスで追加します。Cancel は専用の `TaskCancelOutcome` を持ち、この enum に混ぜません |
| タスク作成（AU2、リポジトリレベル） | `TaskCreationOutcome` | Created／同一 Run now source の再送（AlreadyCreated）／Stop・削除先勝ち（CompanionUnavailable）／会話 Owner 入力の先勝ち（Superseded）／Schedule の期限切れ（StaleSchedule）／停止・削除済み Schedule（ScheduleUnavailable）／別帰属に使用済みの指示（SourceConflict）／Host・復元等の hold（HeldByGlobalHold）。いずれの拒否も Task 一式を書かず Ok 側で返す。候補側・H-H 側の受付判定へ作業担当が写す |
...
enum TaskCreationKind {
    Normal { workspace: Option<WorkspaceAssociationPremise>, schedule_run_now: Option<NormalScheduleRunNow> },
    BackupExport { source: BackupTaskSource }, // StartedOccurrence または Owner RunNow。Workspace / delegation は持たない
}
struct NormalScheduleRunNow {
    schedule: ScheduleId,
    expected_version: ScheduleVersion,
    instruction: OwnerInstructionSourceRef,
}
...
// Normal.schedule_run_now が Some なら現行 Schedule 版と内容・担当・Workspace、Owner
// source の対象と現在性を同じ commit で照合し、通常 Task と専用の由来行を一括保存する。
// None は Schedule Run now を意味せず、由来を OwnerManagement に偽装しない。
...
enum TaskCreationOutcome {
    Created(TaskRef),
    AlreadyCreated(TaskRef),         // 同じ Schedule Run now source の一致する受付再送
    CompanionUnavailable,            // Stop / Deleted / deletion hold が先。Task 一式なし
    Superseded,                      // 会話起点の依拠 Owner 入力が新入力に先行される。Task 一式なし
    StaleSchedule,                   // Run now の版・内容・担当が失効
    ScheduleUnavailable,             // 指定 Schedule が停止・削除済み、または起動条件が不成立
    SourceConflict,                  // Owner source が別の Schedule / Task 種別に帰属済み
    HeldByGlobalHold(HoldConditionRef), // Host / restore 等の新規活動禁止。Task 一式なし
}

trait TaskRepository {
    // タスクの新規作成：task + task_revision + 初期 task_context_entry + kind 別の
    // workspace_assoc / normal_schedule_run_now または backup_task_source を不可分に永続化。AU18 の予定回は同じ
