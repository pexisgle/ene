...
上位の設計原則との優先関係や矛盾が生じた場合の判断基準は、[設計文書 README](../README.md#設計文書の優先順位と信頼できる情報源) に従います。また、文中の「SO」「DR」「CC」は、それぞれ [状態の所有権（state-ownership.md）](../architecture/state-ownership.md)、[依存関係ルール（dependency-rules.md）](../architecture/dependency-rules.md)、[横断的関心事の設計（cross-cutting.md）](../architecture/cross-cutting.md) を指します。
...
- データの由来（どこから来たか）は参照や説明のための記録であり、最新知識のマスターデータではありません。最新の認識は長期記憶（Memory）、タスク、ルールなどの担当機能が保持します。
...
- リストア復元の対応関係は、`(復元操作, バックアップ取得時点, RestoreGeneration, 復元後の保留状態)` です。マスターデータ切り替えの前は「復元前の正常データ」がマスターであり、切り替え後は「復元されたデータ」がマスターとなります。両者が中途半端に入り混じった第3の混合状態を作ってはいけません。
...
// 列挙した Action 試行集合（seal 時点で membership 固定）との完全一致の下で再評価される。
// 同じ identity の retry で本文は増えず、二重採用・二重完了もしない。
struct TaskResultId(/* 不透明なID */);
// 1 delegated Task Agent execution の durable な対応関係。execution lifetime であり、
// 0..N inference turn + その間の 0..N Action attempt + 0..1 final result を束ねる。
// final result の到着で seal され、以後この delegation からの新規 work は開始しない。
// 1 turn の source inference attempt identity を lifetime key にしない（再委任・リトライは新しい identity）。
...
struct TaskPurposeRef {
    task: TaskId,
    adopted_revision: TaskRevision, // 目的を採用した revision。本文の原本は task_revision snapshot。引き継いだ目的は現在 revision より前を指し得る
}
...
struct DelegationRef {
    delegation: DelegationId,    // durable な相関 identity。行の存在は実行中・生存・完了の証明ではない
...
- 推論試行（ticket）は、`(チケットID, 消費主体, タスク／委任との対応, 利用目的, 依拠リビジョン／世代前提)` の durable 対応を保持し、利用実績（`usage_fact`）からの帰属は ticket 対応を辿って解決します。割り当ての解決結果そのものは派生データであり、マスターデータにしてはなりません。
...
/// 接続・存在機能が管理する個体ごとの滞在帰属記録。唯一のマスターデータ。
...
- リストア復元時に、受理、説明、隔離、照合、マスターデータ切り替え、復元後保留、一括有効化という段階を厳格に踏むこと（単一のフラグに潰さないこと）。
