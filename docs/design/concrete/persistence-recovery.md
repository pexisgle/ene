...
| `task_revision` | `(task_id, task_revision)` | 目的本文、`TaskPurposeRef`、担当者、委任が依拠するリビジョン前提（委任の目的解決はこの revision snapshot から行い、`delegation` 行は目的本文を複製しない）、方針指示（steering）の前後の区別。採用した追加指示は採用リビジョンの `task_context_entry`（H-A steering 配線スライス）が表現します。<br>REV = `task_revision` | D2 | 同上 | 遅れたタイミングで届いた実行結果の正しい帰属判定（試行 attempt → task revision → 現在タスク）に利用します。 |
