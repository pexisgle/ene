...
V24 は、AU15a で seal 済み・AU15b 未採用の `task_result`（`adopted_revision IS NULL`）を bounded に列挙する recovery reconciliation のための query-support index `idx_task_result_unadopted ON task_result (recorded_at, result_id) WHERE adopted_revision IS NULL` を追加します。reconciliation はこの未採用候補を `(recorded_at, result_id)` の keyset で 1 ページずつ一巡し（1 read は page limit で bounded）、各候補を既存の `adopt_result` で再評価します。index は、その bounded candidate enumeration を支えるだけで、pending flag / retry queue / adoption state ではありません。候補の canonical truth は `task_result` 行の存在と `adopted_revision IS NULL` のままであり、index の有無で候補集合・採用可否・withheld の意味は変わりません。index は既存行を書き換えず、値を backfill しません。fresh V24 database と V23 → V24 upgraded database は同じ意味の状態へ収束し、version 前進は index の存在だけを記録します。
...
| **V23 → V24 sealed-but-unadopted reconciliation の query-support index** | 列・テーブルの追加なし。`task_result(recorded_at, result_id) WHERE adopted_revision IS NULL` の query-support index を追加し、既存行は一切書き換えず、値を backfill しません。**(A)** V23 の `task_result` 行は `adopted_revision` を含めてそのまま保持され、`completed` / `failed` / `cancelled` の捏造も result の再発行もない。**(B)** `adopted_revision IS NULL` の sealed result は `(recorded_at, result_id)` の bounded な keyset page で全件を一巡でき、先頭に残る `RecordedToOriginalOnly` / 継続 `Withheld` の候補が後続の recoverable result を starvation させない。**(C)** index は pending / retry / adoption state ではなく、候補集合と採用可否は index の有無に依存しない。**(D)** schema version のみ 24 へ前進する（index の有無は意味状態ではない）。 | §4.5, §6, §7 AU15a/AU15b |
