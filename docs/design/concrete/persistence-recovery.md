...
| `inference_attempt`（担当: 推論） | `ticket` | 消費主体（Companion / Observer専用 / Task Agent 等）、機能（Capability）、利用目的、依拠した同意 `(id, rev)`、認証情報セット改訂、プロバイダ／モデル、開始日時。Task Agent の利用では CORR → `delegation` と依拠 `TaskRef`（`(task_id, task_revision)` 前提）。結果到着・採用・確定度の状態は持ちません（結果と採用はそれぞれの producer が同じ設計変更で追加）。 | D2（追記専用の試行記録） | 個人データ完全削除に参加しますが、試行が存在した事実と利用実績をなかったことにはしません。 | 再起動後にチケットから委任・依拠リビジョン・消費主体を辿るために読み込みます。行の存在は実行中・成功の証明ではありません。provider の自動 replay の根拠にしてはなりません。 |
...
※ `usage_fact_provider` の実装段階では `ticket` をキーとし、消費主体・利用目的・CORR は `inference_attempt` 側で保持します。`usage_id` キーと消費主体/CORR 列の追加、および利用実績合計との不可分な照合（AU6）は、K-G の予約・確定 producer を持つスライスが同じ設計変更で行います。
...
- **AU14（担当責任者を跨ぐ不可分な読み取り）**: Task Agent 推論試行の確定時に、ticket・同意 `(id, rev)`・認証情報セット改訂・委任の対応関係・依拠タスクリビジョン（`(task_id, task_revision)`）の現在性を単一の短いトランザクションで照合し、一致した場合のみタスク対応を含む試行行を挿入します。不一致は `TaskPremiseStale`（タスク前提）または `Stale`（同意・認証情報）として書き込みなしの正常系ドメイン結果とし、provider へは送信しません。このトランザクションの中で await や外部I/Oを行いません。
