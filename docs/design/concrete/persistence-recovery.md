...
- **復旧するだけで、自動実行してはならないもの**:
  ...
  - `cancelled` のタスクの中断解除・自動再開・新しい委任 / steering / 推論 claim / Action 開始（受理は吸収的であり、再開は新しい Task / 新しい delegation としてユーザーの指示の下で行う）。
...
| **タスク中断の受付（Cancel、AU16）** | 対象 `TaskId`、現在の `task.progress`（非 terminal か）。revision・purpose の前提は持たない | 1 つの短い `Immediate` トランザクションで現在の `task.progress` を読み、`started` / `in_progress` のときだけ `cancelled` へ CAS します。この commit が中断要求の受理の durable marker であり、外部作用の停止完了・`Unknown` の解消・Action の確定度を意味しません。`completed` / `failed` は `TaskTerminal`、既に `cancelled` は `AlreadyCancelled`（冪等）、識別子不在は `MissingTask` として書き込みません。開始済みの `inference_attempt`・`data_use` 相関・`action_attempt` を削除・書き換えず、確定度は ene-action owner の CAS だけが更新します。cancel 専用の停止フラグ・停止行・gate 条件は追加しません。 | 受理（作業担当）と実際の停止試行・成否不明の報告（実行・拡張担当）を混同しません。停止要求の名目で新しい未承認アクションを開始しません。 |
...
- **AU16（同一担当の不可分コミット。タスク中断の受付）**: 個体調整（会話・公式の管理経路のいずれか）から直接届いた中断要求を、1 つの短い `Immediate` トランザクションで `cancel_task` として処理します。現在の `task.progress` を読み、`started` / `in_progress` のときだけ `cancelled` へ CAS します（受理 = commit）。この commit が中断要求の受理の唯一の durable な marker であり、メモリ上の cancellation token・`Notify`・Future drop・停止シグナルの送信は受理の権威ではなく、外部作用の停止完了・`Unknown` の解消・Action の確定度を意味しません。`completed` / `failed` は `TaskTerminal`、既に `cancelled` は `AlreadyCancelled`（書き込みなしの冪等結果）、識別子不在は `MissingTask` とし、いずれも `Ok` 側のドメイン判定です。revision・purpose の前提を持たないため、同時 steering と競合しても stale にせず、勝った順に durable に残します。開始済みの `inference_attempt` と `data_use` 相関・`action_attempt` を削除・無効化・書き換えず、確定度は ene-action owner の新しい客観的証拠による CAS だけが更新します。cancel 専用の停止フラグ・停止行・gate 条件は追加せず、以後の AU3/AU4/AU14/AU5/AU15b は既存の非 terminal gate が `TaskTerminal` / `RecordedToOriginalOnly` として拒否します。AU16 と AU3/AU4/AU14/AU5/AU15b は同一 SQLite master 上の短い `Immediate` トランザクションで直列化されるため、先に terminal を確定した方が勝ち、敗者が書き込む隙間はありません。このトランザクションの中で await や外部I/Oを行いません。
...
| **タスク中断の受付 → 遅延結果 → 再起動** | 中断要求は AU16 の 1 つの短いトランザクションで `task.progress` が非 terminal のときだけ `cancelled` へ CAS し、その commit を受理の durable marker とします（受理は停止完了を待たず、停止完了を意味しません）。以後の AU3/AU4/AU14/AU5/AU15b は既存の非 terminal gate が拒否し、開始済みの `inference_attempt`・`data_use` 相関・`action_attempt` を削除・書き換えず、`Unknown` を成功・失敗・未実行へ変えません。cancel 後に到着した final result は AU15a が本文・identity の 1 回だけの到着 record と seal を行い、AU15b は attempt 相関の記録に留めて `RecordedToOriginalOnly` を返します。再要求は `AlreadyCancelled`（冪等）、`Completed` / `Failed` との競合は同一 SQLite master 上の直列化で先勝ちし、敗者は書き込みません。再起動後も `cancelled` は durable に読み戻せ、自動再開・自動 replay・自動再採用を行いません。 | §3.2, §4 Group D/E, §6.2/§6.3, §7 AU16・AU15a・AU15b, §10 |
