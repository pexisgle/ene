...
| **タスク結果の採用・完了確定** | 到着 record 済みの結果行、委任 ⇔ 依拠 TaskRef の対応、現在 `task.revision` と purpose identity、`task.progress` の非 terminal、delegation（execution lifetime）から列挙した Action 試行の authoritative set と claim の完全一致、各試行の確定度（`ConfirmedSuccess` のみ完了可）、採用リビジョン、cancel marker（後続 producer） | 1 つの短い `Immediate` トランザクションで、result 行の存在確認、依拠 Action 試行の delegation からの完全列挙（durable 存在・`(delegation, task, task_revision)` 対応。membership は seal 時点で固定）、claim との完全一致（欠如・追加・重複は技術的エラーで fail closed）、確定度の読み取り、`adopted_revision` の刻印、`task_result_attempt` の相関刻印、`task.progress` の terminal への CAS を行います。現在 revision が前進済み・terminal・cancel marker ありの場合は attempt 相関のみを確定して現在 Task を変更しません（`RecordedToOriginalOnly`）。authoritative set に `ConfirmedSuccess` 以外がある場合は attempt 相関を確定した上で現在 Task を完了しません（`WithheldByEffectFacts`）。seal 後に `Unknown` が客観的証拠で `ConfirmedSuccess` へ進んだ場合は、同じ result の再評価で完了が成立し得ます（membership は変えません）。同じ result identity の再 arrival は行を増やさず、二度目の terminal transition を行いません。 | 外部作用の確定度は実行・拡張担当の事実のまま読み取り、作業担当が書き換えません。エージェントの自己申告を証拠にしません。完了とアクション成功は別事実です。caller の attempt 列挙は authoritative set を置き換えず、seal 後の membership も変えません。 |
...
| **タスク委任 → 実行結果の受入** | タスク作成時に AU2 により不可分コミット（タスク本体 ＋ 改訂 ＋ 初期コンテキスト ＋ 関連付けを確定した場合のワークスペース関連付けを保存し、完了前は外部エージェントへの委任や実行から不可視とする）。エージェントへの委任時は期待する改訂番号（`expected_task_revision`）と非 terminal な `task.progress` を不可分に照合し、`Started → InProgress` を同じトランザクションで進めます（terminal は `TaskTerminal` で書き込みなし）。Task Agent execution が final result を提出したら、まず結果本文を到着 record（AU15a）として 1 度だけ durable 化し、同じ不分区間でその delegation（execution lifetime）を seal します（1 delegation につき final result は最大 1 つ）。その後の採用判定（AU15b）で delegation 行から依拠リビジョンを解決し（試行 → タスク改訂 → 現在のタスク）、delegation から列挙した authoritative set と claim の完全一致、およびすべての依拠試行が `ConfirmedSuccess` であることを同一トランザクション内で検証します。客観的事実の記録と、結果の採用＋完了確定を明確に分離し、欠如・追加・重複のある claim は技術的エラーとして fail closed、`ConfirmedSuccess` 以外がある場合は `WithheldByEffectFacts` として完了しません。seal 後は同じ execution からの新しい推論 claim・Action 開始を拒否し、membership は seal 時点で固定します（certainty だけが owner の証拠で進展し得ます）。古いタスク目的・リビジョンのために得られた結果を、方向転換後の新しい目的へ勝手に自動採用しません。 | §4 Group D/E, §7 AU2/AU3/AU5/AU14/AU15a・AU15b, §10 |
