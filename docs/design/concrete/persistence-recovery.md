...
| **finalization 後・採用前の Host クラッシュ** | Task Agent execution が final answer を提出し、`TaskResultId` と本文が `task_result` に到着 record され、delegation が seal された後、採用判定の前に Host がクラッシュしても、再起動後に本文・依拠リビジョン・委任（execution lifetime）・execution seal・Action attempt の対応を読み戻せます（`adopted_revision = NULL` = 到着済み・採用未確定）。provider 呼び出しは自動 replay せず、結果を自動再採用・自動完了しません。seal は採用より先に durable であるため、再起動後の新規 work も seal により拒否されます。 | §4 Group D, §6.2, §7 AU14/AU15a・AU15b |
