...
| 論理テーブル名 | PK | 主なフィールド | 保存分類 | 削除時の動作 | 再構築の起点 |
|---|---|---|---|---|---|
| `task` | `task_id` | 現在の改訂番号（REV = `task_revision`）、現在の `TaskPurposeRef` と目的本文の現行値（方針指示 steering 前提は現在リビジョンと `TaskPurposeRef` で表現）、担当者、progress（closed world `started / in_progress / completed / failed`。Task revision とは別軸で、terminal は吸収的。AU3/AU4/AU14/AU5 は同一の不分区間で非 terminal を必須とし、terminal は `TaskTerminal` で書き込み・開始なし。AU14/AU5 は加えて delegated execution が seal 済みでないことを同じ不分区間で必須とし、seal 済みは `ExecutionSealed` で書き込み・開始なし）。`completed` は AU15b が同じ不分区間で Task-wide completion barrier（同じ `task_id` に属する全 revision / 全 delegation の `action_attempt` に `unknown` が無いこと）を確認した commit によってのみ成立し、確定度は `unknown → confirmed_*` の一方向にのみ更新されるため、完了後に新しい `unknown` は生えません）、採用済み result identity（`task_result.adopted_revision` から bounded read で解決）、待機状態、完了・失敗・キャンセル・未完了事項、次の判断。execution seal は delegation ごとの `task_result` 行の存在そのものであり、task 行へ二重に持たせません。progress 列を追加する V20 移行の既存行 backfill は §4.5 に従います（Task ごとの delegation existence のみで `started` / `in_progress` を決め、`completed` / `failed` を捏造しません）。<br>CORR → `restore_generation`（保全・消去スライスで導入） | D1 | タスク削除時に固有のワークスペース関連付けを削除します。すでに形成された学習成果や外部成果物へ連鎖削除はしません。 | 再起動後に、保存済みの進捗・確定した作用・結果不明事項・未完了事項を画面に提示し、ユーザーからの明示的な再開指示を待つ起点となります。 |
...
### 4.5 Schema version 移行と既存行の意味付け（V19 → V20 `task.progress` backfill）
...
| **タスク / 定期スケジュール** | `task` ＋ `task_revision` ＋ `task_context_entry` ＋ `task_result` ＋ `task_result_attempt` ＋ `workspace_assoc` ＋ `schedule` ＋ `schedule_occurrence` ＋ `restore_generation_state` | 保存済みの progress（`started / in_progress / completed / failed`）、採用済み result identity と結果本文、到着済み・採用未確定（`adopted_revision = NULL`）の結果本文と execution seal、依拠リビジョン・委任（execution lifetime）・Action 試行相関、確定した外部作用、結果不明事項、未完了事項、人間の判断待ち、スキップまたは開始されたスケジュールの対応。Task が `completed` の場合、その completion commit 時点で同じ Task に `unknown` な Action 試行が 0 件だったという Task-wide completion barrier の不変条件も、`action_attempt` の durable facts と terminal gate から再構成できます（新しい cache・summary・marker は追加しません。確定度は `unknown → confirmed_*` の一方向にのみ更新され、完了後は terminal gate が新しい行を拒否するため、完了後に `unknown` は生えません）。次回の日時表示は、保存された条件と現在の日時・タイムゾーンから動的に導出します。 | **自動再開しません。** ユーザーからの明示的な再開指示と、現在の前提条件（担当個体、権限、同意、利用上限、ワークスペース状態、Client依存、二重実行リスク）の再確認を必須とします。`WithheldByEffectFacts` で記録だけされた結果や到着済み・採用未確定の結果を勝手に採用・完了せず、`Unknown` の外部作用を勝手に再実行しません。provider 出力の自動 replay もしません。停止中にスキップされた回（missed）を勝手に後から自動実行して埋め合わせることはしません。 |
...
| **タスク完了済み Task への新規開始拒否と execution seal** | `Completed` / `Failed` の terminal 後は、同じ current `TaskRef` のままでも AU3（委任）・AU4（steering）・AU14（Task Agent 推論試行 claim）・AU5（Action 開始）が同一の不分区間で非 terminal を必須とし、terminal は `TaskTerminal`（タスク所有の値型を import しない Action 側は unit outcome）として書き込み・provider I/O・外部作用のいずれも発生させません。加えて Task が InProgress のままでも、final result の到着で seal 済みの delegation からは AU14/AU5 が `ExecutionSealed` として開始を拒否します（terminal と seal は別概念）。terminal を revision stale へ丸めず、キャンセル機構の producer を先取りしません。 | §4 Group D/E, §7 AU3/AU4/AU5/AU14, §6.2 |
