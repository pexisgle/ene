...
| `task_result` | `result_id` | CORR → `task_id` ＋ `task_revision`（委任が依拠したリビジョン。目的はこの revision snapshot から解決）、CORR → `delegation_id`、結果本文（`body`。provider 出力の正本はこの 1 行だけであり、他のテーブル・ログ・監査へ複製しない）、CORR → `task_result_attempt` の依拠 Action 試行相関、採用リビジョン（`adopted_revision`: NULL = 記録のみ、Some = 現在 Task へ採用済み）、記録日時。<br>REV なし（結果は内容更新されない）。同じ `result_id` の再 arrival は行を増やさず、既存行から同じ採用判定を再評価します（本文・委任・依拠リビジョンの不一致を伴う identity 再利用は技術的エラー）。 | D2 + D3（記録履歴 ＋ 採用状態） | タスク削除時に結果と採用状態を削除します。個人データ完全削除では本文を確実に除去し、attempt 相関の識別子は対応関係の保全に必要な最小限に留めます。採用済みであることを理由に削除を拒否しません。 | 再起動後に、採用済み result identity・依拠リビジョン・委任・Action 試行の対応を bounded read で再構成します。自動 replay・自動再採用の根拠にはしません。 |
| `task_result_attempt` | `(result_id, attempt_id)` | 結果が依拠した Action 試行の相関（本文・確定度は複製しない。確定度は `action_attempt` が正本）。arrival に依拠試行がある場合のみ行を持ちます。 | D2（相関のみ） | 親 `task_result` とともに削除します。確定度・作用事実は `action_attempt` 側の削除規則に従います。 | 結果と Action 試行の対応を辿り、採用時に durable に検証するための材料として読み込みます。 |
...
| 論理テーブル名 | PK | 主なフィールド | 保存分類 | 削除時の動作 | 再構築の起点 |
|---|---|---|---|---|---|
| `action_attempt` | `attempt_id` | CORR → `task_id` ＋ `task_revision`（依拠タスクリビジョン）、CORR → `delegation_id`、CORR → `workspace_assoc_id`。解決された実際の操作対象（`real_target`: 実行直前に解決された canonical な対象。単なる入力文字列ではない）、操作種別（`operation`: このスライスは `list / read / create / edit` の closed world。`delete / execute` はユーザー確認・拡張 producer を持つスライスが同じ設計変更で追加）、依拠した Action 評価（`relied_evaluation`: K-B.1 の single-use な `ActionPermissionEvaluationId`。評価の fingerprint（操作・実対象・委任・依拠 TaskRef・関連付け）を試行行の各列が再現する。同じ ID の二度目の試行挿入は技術的エラー）、開始日時（`started_at`）。確定度（`certainty`: `confirmed_success / confirmed_failure / unknown`）と判断根拠（`grounds`: 開始時は NULL、観測を記録した場合のみ closed 語彙。`unknown` は NULL または `outcome_unverified`）。<br>実行段階（受付 / 開始 / 送信 / 把握の区別）、保留（hold）、CORR → `presence_generation`・`restore_generation`、CORR → `prior_attempt_id`（リトライ時の元不明試行との対応）は、対応する producer を持つスライスが同じ設計変更で追加し、それまで代役値や placeholder 列を置きません。 | D2 + D3<br>（追記専用の試行記録 ＋ 結果不明 Unknown の確実な維持） | 内部データの削除は「操作ログの消去」であり、すでに外部システムに及んだ作用を取り消す（rollback）ことではありません。個人データ完全削除では対象の本文を除去しつつ、操作が行われた事実の記録と両立させます。確定度のステータスを削除理由で書き換えてはなりません。 | 再起動後も「Unknown（結果不明）」は不明のまま正しく復元し、二重実行のリスクを明示した上でユーザーの判断へ戻します。リトライを行う場合は、必ず新しい `attempt_id` を発行します。 |
...
struct TaskResultRow {
    ...
    body: TaskAgentOutput,               // provider 出力本文。この 1 行が正本（他のテーブル・ログへ複製しない）
    attempt_refs: Vec<RawId>,            // 依拠 Action 試行の opaque な相関（確定度は action_attempt が正本）
    adopted_revision: Option<TaskRevision>, // Some = 現在 Task へ採用済み（同じ result の再 arrival で再評価）
...
| **タスク結果の記録・採用・完了確定** | 結果 identity と本文（1 行）、結果 ⇔ 委任 / 依拠 TaskRef の対応、結果 ⇔ Action 試行相関、現在 `task.revision` と purpose identity、`task.progress` の非 terminal、依拠 Action 試行の確定度（`ConfirmedSuccess` のみ完了可）、採用リビジョン、cancel marker（後続 producer） | 結果行（本文・相関）の挿入、依拠 Action 試行の同一トランザクション内読み取り（durable 存在・`(delegation, task, task_revision)` 対応・確定度）、`adopted_revision` の刻印、`task.progress` の terminal への CAS を 1 つの短い `Immediate` トランザクションで行います。現在 revision が前進済み・terminal・cancel marker ありの場合は結果行のみを確定して現在 Task を変更しません（`RecordedToOriginalOnly`）。確定度に `ConfirmedSuccess` 以外がある場合は結果行を確定した上で現在 Task を完了しません（`WithheldByEffectFacts`）。同じ result identity の再 arrival は行を増やさず、二度目の terminal transition を行いません。 | 外部作用の確定度は実行・拡張担当の事実のまま読み取り、作業担当が書き換えません。エージェントの自己申告を証拠にしません。完了とアクション成功は別事実です。 |
...
- **AU3（担当責任者を跨ぐ不可分な読み取り）**: `delegation`（委任）レコード作成時に、期待するタスク改訂番号（`expected_task_revision`）が現在のタスク改訂番号と一致しているかを不可分に照合（atomic read & compare）します。不一致であれば受理を拒否し、再評価へ戻します。同一の短い即時トランザクション内で、現在の Task 行（リビジョンと担当者）を読み、欠如していれば `MissingTask`（書き込みなし）、不一致であれば `StaleTaskRevision { current }`（書き込みなし）とし、一致する場合はさらに同じリビジョンの `task_revision` snapshot を読み、担当者が Task 行と一致することを確認します（snapshot の欠如・不一致は技術的エラーであり、合成した対応関係を作りません）。確認が通った場合のみ、委任時点のスコープの写しと、同一トランザクションで読んだ Task 行の担当者を delegator として `delegation` 行を挿入します。委任作成はタスクリビジョンを前進させません。同じタスクの同じリビジョンに対する複数の委任は許可し、単一委任の一意制約を課しません。部分的な委任の対応関係を可視にしてはなりません。委任作成は割り当て（assignment）の identity を保存しません（Task Agent の割り当ては推論受付時に委任元の現在の同意から live 解決します）。
...
- **AU5（同一担当の不可分コミット ＋ 担当間の順序保証）**: `action_attempt` の挿入（前提条件の照合付き）。同一の短い `Immediate` トランザクション内で、(1) 前提の `delegation_id` 行の存在と、その `(task_id, task_revision)` が premise の依拠 `TaskRef` と一致すること、(2) 現在の `task` 行のリビジョンが依拠リビジョンと一致すること、(3) 現在の `workspace_assoc` 行が premise の関連付け識別子と一致し、かつ委任の `scope_assoc`（委任時に依拠した境界の写し）と一致すること、(4) premise の `relied_evaluation`（K-B.1 の今回限りの判断）がまだどの試行にも使われていないこと、を照合してから行を挿入します。委任行・タスク行・現在の関連付けの欠如、リビジョンや関連付けの前進は `StalePremise` とし、書き込みも実行も行いません。委任行と premise の不一致（(1) の不一致）や `workspace_assoc` の複数行は技術的エラーとして fail closed します（推測で stale に丸めません）。このトランザクションの中で await や外部I/Oを行いません。確定度の更新は、同一レコードに対する期待確定度と新しい証拠（`expected_certainty + 新 evidence`）の不可分な更新（CAS）とします。許可される遷移は `unknown → confirmed_success / confirmed_failure` と、証拠が確認できない場合の `unknown → unknown`（根拠 `outcome_unverified` 付き）のみで、確定済みの値からの更新や根拠のない `unknown` への据え置きは拒否します。試行IDは再利用せず、重複した `attempt_id` の挿入は技術的エラーとします。タスクの達成判定は別トランザクションで試行レコードを読み取って更新します（durable-before-adopt）。
...
- **AU14（担当責任者を跨ぐ不可分な読み取り）**: Task Agent 推論試行の確定時に、ticket・同意 `(id, rev)`・認証情報セット改訂・委任の対応関係・依拠タスクリビジョンの現在性を単一の短いトランザクションで照合し、一致した場合のみタスク対応を含む試行行を挿入します。照合は次の 3 条件です: (1) 前提の `delegation` が `delegation` 行に存在する、(2) その行の `(task_id, task_revision)` が依拠 `TaskRef` 前提と一致する、(3) 現在の `task` 行のリビジョンが依拠リビジョンと一致する。委任行・タスク行の欠如や (3) の不一致は `TaskPremiseStale`、同意・認証情報の不一致は `Stale` として、いずれも書き込みなしの正常系ドメイン結果とし provider へ送信しません。(2) の不一致や部分的・不整合な行は技術的エラー（fail closed）とし、重複 ticket の再確定は既存契約どおり `Stale` とします。`TaskPremiseStale` に技術的エラーを吸収させません。このトランザクションの中で await や外部I/Oを行いません。
- **AU15（同一担当の不可分コミット ＋ 担当間の順序保証）**: Task Agent 結果の記録と採用・完了確定。同一の短い `Immediate` トランザクション内で、(1) arrival が指す `delegation` 行から依拠 `TaskRef` を解決し（delegation 行の欠如は `MissingDelegation`、Task 行の欠如は `MissingTask`。いずれも書き込みなしの正常系ドメイン結果）、(2) 現在の `task.revision` と purpose identity を依拠リビジョンの `task_revision` snapshot と照合し（本文文字列は使わない。不一致は `RecordedToOriginalOnly`）、(3) 現在の `task.progress` が非 terminal かつ cancel marker（後続 producer）が無いことを確認し（terminal・marker ありは `RecordedToOriginalOnly`）、(4) arrival の `attempt_refs` ごとに `action_attempt` を読み、行の durable な存在と `(delegation, task, task_revision)` の一致を検証し（欠如・不一致は部分的な単位として技術的エラー。stale へ丸めない）、(5) いずれかが `ConfirmedSuccess` でなければ結果行を記録して `WithheldByEffectFacts` を返し、(6) すべて `ConfirmedSuccess` の場合のみ `task_result` の挿入（`result_id` PK で 1 回だけ）＋ `task_result_attempt` の相関 ＋ `adopted_revision` の刻印 ＋ `task.progress` の非 terminal からの `Completed` CAS を不可分に行います。同じ `result_id` の再 arrival は本文行・相関行を増やさず、記録済みの `adopted_revision` と現在状態から同じ判定を再評価して二度目の terminal transition を行いません（本文・委任・依拠リビジョンが異なる identity 再利用は技術的エラー）。`Unknown` は実行・拡張担当の新しい客観的証拠が確定するまで `action_attempt` 側で保持し、作業担当は確定度を書き換えません。このトランザクションの中で await や外部I/Oを行いません。
...
| **タスク委任 → 実行結果の受入** | タスク作成時に AU2 により不可分コミット（タスク本体 ＋ 改訂 ＋ 初期コンテキスト ＋ 関連付けを確定した場合のワークスペース関連付けを保存し、完了前は外部エージェントへの委任や実行から不可視とする）。エージェントへの委任時は期待する改訂番号（`expected_task_revision`）を不可分に照合します。実行結果が届いた際は、delegation 行から依拠リビジョンを解決し（試行 → タスク改訂 → 現在のタスク）、結果本文を結果 identity の 1 行として 1 度だけ記録し、依拠 Action 試行を同一トランザクション内で検証します。客観的事実の記録と、結果の採用＋完了確定（AU15）を明確に分離し、依拠試行がすべて `ConfirmedSuccess` でない場合は完了しません。古いタスク目的・リビジョンのために得られた結果を、方向転換後の新しい目的へ勝手に自動採用しません。 | §4 Group D/E, §7 AU2/AU3/AU5/AU15, §10 |
