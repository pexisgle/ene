...
| `TaskId`・`TaskRevision`・`TaskRef`・`TaskPurposeRef`・`SteeringPremiseRef`・`TaskContextEntry(Id)`・`AdoptedInstructionRef`・`AssigneeRef`・`WorkspaceNeedRef`・`WorkspaceAssocId`・`DelegationId`・`DelegationRef`・`ScheduleId`・`ScheduleOccurrenceId` | `ene-task` | Task・委任・Schedule・関連付けを一つの state へ潰さない。担当削除で Task 記録を消さない |
