...
| `ActionAttemptId`、`ActionCertainty`、`OperationKind`、`RealTargetRef`、`EffectGrounds`、`ActionAttemptRef`、`AttemptCommitPremise`、`ActionStartOutcome`、`CertaintyUpdateOutcome`、`ActionAttemptRecord` | `ene-action`（試行・確定度・実対象・作用事実） | 外部作用の試行、権限の判定記録、タスクの達成を同一視してはいけません。再試行する際は常に新しい `ActionAttemptId` を発行します。`RealTargetRef` は入力文字列から直接組み立てず、実行直前の解決処理を通します |
...
- **タスク立案 → 作業エージェント委任 → 外部作用の実行**:
  ...
  5. `ene-task` が結果の到着（`TaskAgentResultArrival`）を受け取り、結果本文を `TaskResultId` の 1 行として 1 度だけ記録し、delegation 行から解決した依拠リビジョン・purpose identity・依拠 Action 試行の確定度を 1 つの短いトランザクションで照合して、採用と `TaskProgress::Completed` を同時に確定します（依拠試行が `ConfirmedSuccess` でなければ `WithheldByEffectFacts` として完了せず、revision が前進していれば `RecordedToOriginalOnly` とします）。
