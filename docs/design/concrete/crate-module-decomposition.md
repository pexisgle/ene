...
| `apps/ene-desktop` | 製品 text GUI。Client channel と、製品が意図する first-party control seat。Body の親。確認面 | `ui`、`session`、`control`、`body_supervise`、`i18n`、`erasure` | ドメインのマスターデータや決定権限を持たせない。overlay / wgpu をこの process に入れない。同一 UID であることや socket 開封を Owner 確認にしない。空席先着の真正性は証明しない。Computer Use adapter を同居させる場合は確認面 / 秘密面を注入対象から外す |
...
| `ene-local-control` | ● | ●（製品 `ene-desktop` だけが crate に依存。席の取得は空席への local-transport 先着） | —（remote / `ene-api` に載せない） | seat に束縛した session 完了と秘密 intake。同一 UID・第二 socket・空席先着は Owner 確認でも公式 GUI 証明でもない。`ene-ctl` は依存しない |
| `apps/ene-core` | ●（Host 側の結合ルート） | — | — | ドメインとしての意味判断を持ちません。Client 側アダプターや `ene-body` には依存しません |
| `apps/ene-desktop` | — | ●（text GUI、意図する control seat、Body の親、確認面） | — | `ene-api` + `ene-client` + `ene-local-control`。Host 内部ドメイン、`ene-store` には依存しません。生値は入力 widget と redacted control field の揮発区間に限り、通常 DTO に載せません。空席先着の真正性は証明しません |
...
- first-party desktop の process 名、Body 隔離、control channel、exclusive `FirstPartyControlSeat` と席への完了束縛（取得者の真正性は証明しない）、性能の分母は [First-party desktop](first-party-desktop.md) が固定しており、ここでの Freedom ではない。toolkit / VRM runtime / overlay backend / `keyring` crate は同文書第7節の provisional であり、probe 前に恒久 contract としない。
