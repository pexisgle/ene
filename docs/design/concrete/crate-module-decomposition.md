...
- **タスク立案 → 作業エージェント委任 → 外部作用の実行**:
  ...
  6. 公式（第一者）管理経路からの中断要求は、`ene-companion` を介さず `ene-task` の `cancel_task`（AU16）へ直接届き、`task.progress` を非 terminal から `cancelled` へ CAS して即座に受理します。以後の AU3/AU4/AU14/AU5/AU15b は既存の非 terminal gate が拒否し、開始済みの inference / Action 試行と `Unknown` は変更しません（受理と停止完了は別事実。遅延結果は `RecordedToOriginalOnly`）。`ene-action` は Task lifecycle の値型を import せず、`TaskTerminal` として開始拒否を受け取ります。
