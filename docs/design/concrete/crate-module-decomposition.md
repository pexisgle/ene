...
- Rust workspace tree 例、現行 Repository からの移行案（retain / rename / move / split / merge / deprecate）。
...
8. **巨大 `ene-core` を composition root へ痩せさせる。** 現行 `apps/ene-core` の全依存は維持しない。意味判断は各 owner crate、永続化は `ene-store`、外部 hosting は `ene-plugin-host`、wire DTO は `ene-api` へ移し、`ene-core` は wiring / lifecycle / storage init / route 配線だけに留める。

## 3. 現行 Repository 構造の評価

現行 crate の依存は `Cargo.toml` と `src/` の公開 API から確認した。理想論の全面再編ではなく、維持・変更・新設・廃止を分ける。

| 現行 crate / app | 主な中身（現状） | 評価 | 目標との差 |
|---|---|---|---|
| `ene-config` | config・schema・path 解決 | retain（narrow）。typed config と path 解決だけに限定し、domain state・secret・runtime 判断を持たせない | 依存は葉として維持。汎用設定 layer へ膨張させない |
| `ene-card` | Character card container（V3, PNG/CHARX import）・diff | rename + expand → `ene-character`。現状は card 形式中心であり、静的構成・revision・import provenance・export 範囲の authority として不足 | `ene-companion/package.rs` の供給側を取り込み、適用関係（Companion 側）と分離する |
| `ene-companion` | soul / affect / memory(+scope) / inner / proactive / package / presence / store | split。個体調整・認識・学習・Character 供給・Presence が混在し、12 Subsystem 境界を潰している。`ene-access-control`・`ene-tool-registry` への依存は caller → owner の逆（Companion が Permission / Tool 内部を知る形）で整理が必要 | `memory.rs` → `ene-learning`、`package.rs` 供給側 → `ene-character`、`presence.rs` → `ene-presence`、`store.rs` の History/undelivered 部分は domain trait + `ene-store` 実装へ移す。残る対話・同一性・未伝達・自発性だけが `ene-companion` |
| `ene-work` | delegation / jobs / schedules / skills / MCP / vision / learning / observe / task / store | split。作業・学習・実行・観測が混在する最大の集中点。`ene-companion`・`ene-kernel` への依存が双方向化しやすい | `task.rs`/`schedule.rs`/`workflow.rs`/`host.rs` 等 → `ene-task`、`learning.rs`/`skill.rs` → `ene-learning`、`mcp.rs`/`computer.rs` → `ene-action` adapter、`observe.rs`/`vision.rs` → `ene-observer`、`store.rs`/`migrations.rs` → `ene-store` |
| `ene-kernel` | dialogue lane / context assembly / visibility / observability | split + deprecate。Context Assembly を中央 pipeline 化すると CC-02・IB §3 の「用途別 context・owner 別提供」に反する。lane は対話 orchestration へ、context 選択は各 owner へ、observability は Preservation audit へ戻す | 薄い互換 shim として一時残置後、Walking Skeleton 完了時に廃止。`context.rs`/`compact.rs` 等を各 domain へ移す |
| `ene-access-control` | approval / audit / vault（Permission + Audit + Credential 混在） | split。C-1〜C-8（Permission）と S-1〜S-5（Credential）と PE-6（Audit）を混在させると DR-05（秘密分離）・SO 4.19/4.21 の境界が崩れる | `plane.rs`/`policy.rs`/`request.rs`/`risk.rs`/`ai.rs` → `ene-permission`、`vault.rs` → `ene-credential`、`audit.rs` → `ene-preservation`（Audit 順序・保持）。`popup.rs` は presentation 管理面へ移す |
| `ene-session` | event log / usage ledger / history projection + rusqlite | split + deprecate。History（Companion）・usage（各 owner）・projection（derived）を一つの session store へ集めると SO の owner 分離に反する | D2 history は `ene-store` の group 別 module + domain trait へ、projection は derived module へ、usage は `usage_fact_*` へ移し、`ene-session` は移行期の re-export shim に留める |
| `ene-tool-registry` | tool registry / builtins / pipeline / deny-by-default | split。registry（受入・制限＝実行・拡張）と builtins（filesystem/network/shell 等の各作用）を同一 crate で肥大化させている | domain 側（candidate・authorization・attempt・certainty）→ `ene-action`、registry 実装・builtins は当面 `ene-tool-registry` を `ene-action` trait の実装 crate として残置し、段階的に `ene-action::adapters/*` へ move |
| `ene-body` | performance queue / emotion mapping / duplex voice | split + deprecate。内的状態の意味（Learning）と表示 staging（IO）と Client device（audio）を混在させている | emotion mapping → `ene-presentation` が Learning 状態を参照して行う形へ、queue/voice → Client app + `ene-presentation` staging へ移す |
| `ene-api` | HTTP/WS API types・OpenAPI・typed client | retain（narrow）。現状ほぼ wire DTO として適切。Host domain への依存を持たせないことが条件 | Host 内部 newtype・secret・durable row を露出させない。wire-neutral DTO だけに限定し、Host service 層で mapping する（第9・11節） |
| `ene-plugin-ipc` | length-prefixed MessagePack frames | retain。transport 表現として適切 | domain 意味を持たせない。`net` feature 隔離を維持 |
| `ene-plugin-host` | process supervision / host-context / broker grants | retain（narrow）。外部 code hosting adapter に限定する | 推論 protocol 解決・Action 認可の意味を持たせない。`ene-inference` / `ene-action` の trait 実装として依存方向を反転させる（第8節） |
| `ene-provider-assets` | catalog / manifest / download helpers | retain。外部原本の所有ではなく参照・検証 helper に留める | 割当同意・送信可否の意味を持たせない |
| `ene-sandbox` | Landlock + seccomp + Job Object | retain。OS 隔離だけに限定する | domain 意味・MCP 例外の意味（Permission）を持たせない。`cfg(target_os)` 隔離を維持 |
| `ene-vrm` | VRM 1.0 renderer (wgpu) | retain（Client adapter）。表示資材の利用に限定する | 適用関係・経験状態・権限を持たせない |
| `ene-stage-ui` / `ene-tray-linux` | Slint bindings / ksni tray | retain（Client）。生成物・platform 隔離を維持 | domain 意味を持たせない |
| `ene-stage-poc` | PoC stage | deprecate。製品 client は `ene-stage` へ一本化する | 新規機能を足さない。Walking Skeleton 後に削除 |
| `apps/ene-core` | Host process（session store・lane・lock・HTTP/WS） | retain（slim）。composition root へ痩せさせる | 意味判断・SQL・provider 解決・tool 実行を直接持たない。wiring / lifecycle / storage init / route 配線だけにする |
| `apps/ene-desktop` | frozen desktop client | deprecate（frozen 維持）。新規機能を足さない | 製品 client は `ene-stage`、CLI は `ene-ctl`。`ene-desktop` は修正のみで機能追加しない |
| `apps/ene-stage` / `apps/ene-ctl` | product stage client / CLI client | retain。`ene-api` DTO だけに依存し、Host domain crate へ直接依存しない | capture / audio / device adapter は app 内 module に留め、安定後に crate 分離を検討する |
...
crates/
  ene-primitive/        # NEW: opaque RawId / RevisionInner / GenerationInner / WallClockWithTz（tiny）
  ene-config/           # RETAIN(narrow): typed config・path・schema
  ene-character/        # RENAME(ene-card expand): 静的構成・revision・import/export・provenance
  ene-companion/        # RETAIN(narrow): 同一性・対話調整・未伝達・自発性・適用関係（I-1〜I-7）
  ene-task/             # NEW: Task・委任・Schedule・WorkspaceAssoc・TaskContext（W-1〜W-7）
  ene-learning/         # NEW: Summary・Memory・Skill・Relationship・State・scope 意味（L-1〜L-8）
  ene-presence/         # NEW: 接続事実・帰属・hint/復旧先・切替（CN-1〜CN-7）
  ene-presentation/     # NEW(Host): round 実際・Body/Voice staging・一般設定・管理面・MCP Apps UI（IO-1〜IO-8）
  ene-observer/         # NEW: 対象・時機・Capture・候補・routing 派生（OB-1〜OB-7）
  ene-permission/       # SPLIT: Rule・同意・cap・device・sandbox 例外・control constraint・live check（C-1〜C-8）
  ene-credential/       # SPLIT: 秘密登録・照合・認証用途供給・backup 除外（S-1〜S-5）+ OS store 抽象
  ene-inference/        # NEW: 登録・能力観測・割当解決・per-use 送信・fallback・利用量原記録（I-1〜I-7）
  ene-action/           # NEW: 作用・確定度・拡張受入・Client 限定（E-1〜E-6）+ repository trait
  ene-preservation/     # NEW: 保持・Backup/Restore/Reset・Audit/Debug・全域調整（PE-1〜PE-7）
  ene-store/            # NEW: app.db + derived.db + internal_copies 実装（D1/D2/D3/R/T の機構のみ）
  ene-api/              # RETAIN(narrow): Host↔Client wire-neutral DTO（remote-capable のみ）
  ene-plugin-ipc/       # RETAIN: transport frames
  ene-plugin-host/      # RETAIN(narrow): 外部 code hosting adapter（inference/action trait 実装）
  ene-provider-assets/  # RETAIN: catalog/manifest/download helper
  ene-sandbox/          # RETAIN: OS 隔離
  ene-vrm/              # RETAIN(Client): renderer adapter
  ene-stage-ui/         # RETAIN(Client): Slint bindings
  ene-tray-linux/       # RETAIN(Client): tray adapter
  ene-tool-registry/    # TEMP-RETAIN: ene-action trait 実装（builtins 段階移行用 shim）
  ene-session/          # TEMP-RETAIN: 移行期 re-export shim（新規利用禁止）
  ene-kernel/           # TEMP-RETAIN: 移行期 shim（新規利用禁止、Walking Skeleton 後廃止）
  ene-body/             # TEMP-RETAIN: 移行期 shim（新規利用禁止、分割後廃止）
apps/
  ene-core/             # RETAIN(slim): Host composition root（wiring/lifecycle/storage init/route）
  ene-stage/            # RETAIN: product Client（ene-api のみ依存）
  ene-ctl/              # RETAIN: CLI Client（ene-api のみ依存）
  ene-desktop/          # FROZEN/DEPRECATE: 新規機能なし
plugins/tool/*, plugins/provider/*  # RETAIN: 外部拡張（ene-plugin-host 経由でのみ参加）
```

crate 数は目標時点で domain 12 + storage 1 + primitive 1 + contract 1 + adapter 4 + client helper 3 + 移行期 shim 4 + apps 4 である。中規模として navigation・compile boundary・ownership 追跡・循環防止・test 容易性を優先した粒度であり、最小化・最大分離自体は目標にしていない。`ene-presentation` / `ene-observer` を `ene-presence` へ畳まないのは、帰属（Companion 単位排他）・round 実際（入出力）・観測対象（Client 単位共有）の制御単位・lifecycle が異なり、一つの state・boolean・actor へ潰すと X-1〜X-10 の契約が失われるためである。`ene-permission` / `ene-credential` を分けるのは、説明・context に載せてよい情報と秘密値の保護・失効・backup 除外の契約が異なり（DR-05）、同一 crate では非露出を強制できないためである。`ene-inference` / `ene-action` を分けるのは、推論失敗と外部作用不明の再試行・確定度・replay 契約が異なり（CCT §6・§8）、統合すると不明の自動再実行が生まれるためである。
...
| `ene-store` | 機構のみ（PR Group A〜K の D1/D2/D3 + R/T の保持。semantic owner ではない） | `sqlite`（`app.db` owner 別 group。`Immediate` 短 transaction のみ。await なし）、`derived`（`derived.db` + sqlite-vec。feature 隔離。再構築可能）、`fs`（`internal_copies/` + `*.ene-backup` staging。temp→fsync→rename→pointer commit→orphan cleanup）、`migrate`（schema version・migration。前方のみ。downgrade 保証しない） | pub: 各 group の row 型への mapping 関数・`new`/`open`・cleanup のみ。`Transaction` を pub にしない。各 domain の `*Repository` trait 実装（`ene-store` が各 domain に依存する方向）。禁止: 採否・達成・許可・確定度の判断、revision bump 可否の決定、秘密値の保持、derived からの正本復活 |
...
| `ene-plugin-ipc`/`ene-plugin-host`/`ene-provider-assets`/`ene-sandbox` | transport / hosting / catalog / 隔離（外部境界） | 現状維持 + trait 実装側へ整理（第8節） | pub: transport・hosting・catalog・隔離 API のみ。domain 意味を持たせない |
| `ene-vrm`/`ene-stage-ui`/`ene-tray-linux` | Client adapter（renderer / UI bindings / tray） | 現状維持 | pub: adapter API のみ。domain 正本・権限を持たせない |

`pub(crate)` を既定とし、`pub` は上表の型・command・outcome・trait に限定する。`allow_attributes` の workspace 規約に従い、例外は narrow な `#[expect]` のみとする。`unsafe` は `ene-sandbox` の OS 隔離と `ene-vrm` の描画に限定し、各 `unsafe` ブロックに `// SAFETY:` を付す（AGENTS.md）。
...
| `DeletionOperationId`・`DeletionSweepGeneration`・`DeletionOperationRef`・`ErasureConditionRef`・`BackupPointId`・`RestoreGeneration`・`RestoreRef`・`HoldConditionRef` | `ene-preservation` | `DeletionOperationRef` の完了は全域確定であり、各 domain の意味変更は各 owner が行う。検索 token は操作期間のみ保持し完了時に wipe する |
...
- external adapter（transport・OS・device・MCP）は `ene-plugin-host` / `ene-tool-registry`（移行期）/ Client app module に置き、domain が adapter 具体に依存しない方向（adapter → domain trait 実装）に反転させる（第8節）。

中央 orchestrator にならないことの担保：`ene-companion` は対話用途の Task/Learning/Inference/Action/Presence 呼びに限定し（`dialogue` module に隔離。軽微応答・軽微本体のみ直接。削除・Backup/Restore・予約・adapter 選択を知らない）、`ene-task` は作業用途の Action/Inference/Learning 呼びに限定し、帰属切替・観測 routing・表示を知らない。`ene-preservation` は完了集約・hold・token wipe に限定し、対話・作業の意味を知らない。`apps/ene-core` は wiring・lifecycle・storage init・route 配線に限定し、採否・達成・許可・確定度を決めない。各 crate の禁止事項は第4.2節の表に従う。
...
| filesystem copy implementation（`internal_copies/` blob・`*.ene-backup` staging。temp→fsync→rename→pointer commit→orphan cleanup。CCT §13） | `ene-store::fs` | `ene-store` 内部。DB 指標先行の可視中間を作らない。orphan は cleanup で除去する |
...
| Client 接続材料の秘密部分 | `ene-credential` + `ene-presence` の非秘密参照（SO §8） | DB へ秘密を流さない。形式・保存方式は固定しない |

business/domain layer から `rusqlite::Transaction`・生 SQL・`sqlite-vec`・fsync・rename を漏らさない。repository method が内部で短 transaction を行い、compare と durable 更新を不可分にする。transaction 内で await・外部 I/O を行わない。
...
| MCP（Tool/Resource/Prompt。Local 既定 sandbox・特定例外・Remote） | `ene-action`（`McpAdapter`・`ExtensionUse` trait。受入・制限の意味） | `ene-plugin-host` + `ene-tool-registry`（移行期）+ `plugins/tool/*`。sandbox 適用は `ene-sandbox` | adapter → domain。外部 code・結果を信頼済み control にしない。隔離の黙解除をしない。例外を Plugin へ流用しない |
...
| `apps/ene-stage`・`apps/ene-ctl` | — | ●（presentation・capture・device adapter・管理面入口） | — | `ene-api` のみ依存。Host domain・`ene-store`・secret に依存しない |
| `apps/ene-desktop` | — | ●（frozen） | — | 新規機能なし |
...
| `ene-tool-registry`（移行期） | `ene-action`・`ene-permission`・`ene-credential`・`ene-plugin-ipc` | `ene-action` trait 実装。新規 domain 依存を追加しない |
...
| `apps/ene-stage`・`apps/ene-ctl` | `ene-api`・`ene-config`・Client adapter（`ene-vrm`・`ene-stage-ui`・OS crates） | `ene-api` DTO のみ。Host domain・`ene-store`・secret・`ene-plugin-host` に依存しない |
...
flowchart TB
  ...
  treg["ene-tool-registry<br/>(temp impl)"]
...
| 管理面の表示・操作（第9節 `ManagementOperationCommand` の Client 側入口） | Client（入口） ↔ Host（各 owner の受理・確認・結果） | `ene-api::management`。UI に任意書換権を与えない。本体 LLM・長時間 Task を介在させない |
...
- Task → Task Agent → Action：`ene-companion`（`ProposeTaskCommand`）→ `ene-task`（受理・`CreateDelegationCommand`）→ `ene-permission`（live check。Task が caller）→ `ene-credential`（必要範囲のみ）→ `ene-action`（`ExecuteActionCommand`・`StartedAsAttempt`）→ `ene-action`（`ReportEffectFact`・per-attempt CAS）→ `ene-task`（`TaskAgentResultArrival`・現在受入）。steering 競合は `ene-task` の SD-Task 順序付けで直列化する。`ene-task` は帰属切替・観測 routing を知らない。
...
## 13. 移行方針（big-bang rewrite をしない）

Walking Skeleton（テキスト timeline・Companion scope の Summary/Memory 形成・Task Agent 委任の file 作業・Client 不在時の Task 継続・Host 再起動後の明示再開・presence 復旧・Text 管理面）へ段階的に移行可能な順序を優先する。

| 段階 | 内容 | 既存への影響 |
|---|---|---|
| 0. 凍結・規律 | `apps/ene-desktop`・`ene-stage-poc` の凍結、`ene-kernel`・`ene-session`・`ene-body`・`ene-tool-registry` への新規 domain 追加禁止、`ene-api` の domain 依存追加禁止 | 現行動作を壊さない。clippy・fmt を維持する |
| 1. 葉の分離（Walking Skeleton 前） | `ene-primitive` 新設（RawId・RevisionInner・GenerationInner・WallClock のみ）。`ene-access-control` から `ene-credential`（vault + OS 抽象）を split し、`ene-permission` を残す。`ene-card` を `ene-character` へ rename 開始（型は re-export で互換） | `ene-companion`・`ene-work` は既存 `use` を shim 経由に切替え、意味変更なし。新規 code は新 crate の premise 形を使う |
| 2. authority の分離 | `ene-presence`（`presence.rs` move）、`ene-task`（`task`/`schedule`/`workspace` move）、`ene-learning`（`memory`/`skill` move）を新設し、各 `*Repository` trait を定義。`ene-store` 新設（`sqlite` の group 別 module + `migrate`）。既存 `store.rs` は `ene-store` 実装への委譲 shim にする | `ene-companion`・`ene-work`・`ene-session` の rusqlite 直利用を `ene-store` 経由に寄せる。`Transaction` の public 露出を段階的に塞ぐ。SD-Task / SD-Presence の短 transaction を `ene-store` に集約する |
| 3. 実行境界の反転 | `ene-action`・`ene-inference` 新設（trait + premise + outcome のみ thin start）。`ene-plugin-host`・`ene-tool-registry` を trait 実装側へ反転させる。`ene-preservation` 新設（`audit.rs` move + deletion/backup coordination thin start）。`ene-observer`・`ene-presentation` 新設（`observe`/`round` move） | `ene-kernel` の lane・context を `ene-companion::dialogue`・`ene-task::orchestrate`・`ene-inference::resolve/send` へ move。`ene-kernel` は shim 化する。`ene-body` は staging のみ `ene-presentation` へ move し廃止へ |
| 4. Host 痩身・Client 分離 | `apps/ene-core` を composition のみに痩身化（boot・storage init・route）。`ene-api` を wire DTO のみに narrow（内部 newtype・row・secret の露出を削除）。Client は `ene-api` のみ依存に是正する | `ene-desktop` は frozen のまま。`ene-stage`・`ene-ctl` の Host domain 直依存があれば `ene-api` 経由に置換する |
| 5. shim 廃止 | `ene-kernel`・`ene-session`・`ene-body`・`ene-stage-poc` を削除。`ene-tool-registry` の builtins を `ene-action::adapters` へ move 完了後に `ene-tool-registry` を廃止または adapter 実装の薄い wrapper にする | 削除前に依存がゼロであることを `cargo metadata` + `grep` で確認する。一括 rewrite をしない |

各段階で `cargo check -p <pkg>` / `cargo test -p <pkg>` の focused iteration を回し、最後に `cargo clippy --workspace --all-targets -- -D warnings` と `cargo test --workspace` で確認する。`default-members = ["apps/ene-ctl"]` のため bare `cargo test` では全体を検証できないことに注意する。

現行→目標の対応要約：retain = `ene-primitive`(new)・`ene-config`・`ene-api`・`ene-plugin-ipc`・`ene-plugin-host`・`ene-provider-assets`・`ene-sandbox`・`ene-vrm`・`ene-stage-ui`・`ene-tray-linux`・`apps/ene-core`・`apps/ene-stage`・`apps/ene-ctl`、rename = `ene-card`→`ene-character`、move = presence/memory/skill/task/observe/round 等の module 移動（owner 別）、split = `ene-companion`・`ene-work`・`ene-kernel`・`ene-access-control`・`ene-session`・`ene-body`・`ene-tool-registry`、merge = しない（owner 統合の merge は禁止。`ene-store` の機構集約は merge ではなく technology 共有である）、deprecate = `ene-kernel`・`ene-session`・`ene-body`・`ene-stage-poc`・`apps/ene-desktop`（frozen）。
...
- `RawId` ↔ domain newtype の mapping 方式（Host composition の，最小 helper に留める。具体 code は固定しない）。
...
- Client は `ene-api` のみ依存し、Host domain・`ene-store`・secret に依存しない。Host は DTO ↔ domain premise の mapping を `apps/ene-core` の媒介層で行い、domain crate に wire 依存を持ち込まない。
