...
- **Task Agent の推論開始 vs steering（inference start vs steering）**: Task Agent の推論は、委任の対応関係（`delegation` 行）と依拠タスクリビジョンを推論試行の確定（attempt claim）と同一の短い `Immediate` トランザクションで照合してから開始します。照合に失敗した新規推論は `TaskPremiseStale` として provider へ一切送信しません。確定済みの試行は、その後の追加指示によって開始を取り消されません（結果の採用判定は別の不分区間で行い、遅延結果は元リビジョンへの記録に留めます）。provider I/O は試行確定後にロックなしで実行し、推論試行自体の自動再実行は行いません。
