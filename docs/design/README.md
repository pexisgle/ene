...
2. **設計の具体化の流れ**:
   設計は **全体アーキテクチャ (`architecture/`)** → **重要領域 (`critical-areas/`)** → **サブシステム (`subsystems/`)** → **具象設計 (`concrete/`)** の順に、全体像から具体的なモジュールへと段階的に記述しています。
3. **矛盾が見つかった場合**:
   設計同士の矛盾や、実装上で上位設計の前提を満たせないことが判明した場合は、その場のコードで勝手に解決せず、GitHub Issue として起票して設計方針を合意してください。
4. **後方互換性は一切考慮不要**:
   ene の設計および実装において、後方互換性を考慮する必要はありません（`.old/` に限らず、過去の仕様や旧バージョンとの互換性は全面的に対象外です）。互換性維持のためのシム、移行レイヤー、過渡的な回避コードは導入せず、要件を満たす最もシンプルで堅牢な設計・実装を採用します。
...
- **権限・アクション実行・推論**:
  ...
  - [Credential の登録・更新・失効と有効世代の公開](concrete/credential-publication.md)
- **データの削除・保全・バックアップ**:
