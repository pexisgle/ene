...
本書は、システム内のあらゆるデータ（状態: State）について、**「何についてのデータをマスターデータとして扱い、どのサブシステムがその意味や変更に責任を持ち、いつまで・何のために保持するのか」**というデータ所有権の設計方針を定めます。
...
eneでは、「Hostマシンがすべての内部業務データのマスターデータを保持する」という大原則があります。その上で、パートナーの個性、会話の記憶、日々の学習、自律タスクの進捗、セキュリティ設定などのさまざまなデータについて、**「誰がその意味に責任を持つ担当者（セマンティック・オーナー）なのか」**を明確に切り分けます。
...
- **「マスターデータ（Canonical Data）」の意味**:  
  本書でいう「マスターデータ」とは、「eneシステムがそのデータについて公式に参照すべき唯一の基準データ（原本）」という意味です。客観的な世界の絶対的真理という意味ではありません。
  - 例：記憶（Memory）は「パートナーが現在そうだと理解している知識」のマスターデータであり、会話履歴（History）は「実際に交わされた発言ログ」の原本です。両者は役割が異なるため、一方が他方を勝手に上書きして置き換えることはできません。

本書で定めるのは「概念としてのデータの意味、マスターデータの所在、変更責任、利用範囲、ライフサイクル」です。特定のデータベース製品、Rustのオブジェクト所有権、単一の書き込みプロセス（Single Writer）などの実装手段は後続の詳細設計に委ねます。

---

## 2. データ所有権の12原則（Ownership Principles）

1. **同じ意味のデータには、唯一のマスターデータを置く**  
   検索用インデックス、画面表示用のコピー、監査ログ、外部AIへの送信データなど、用途に合わせてさまざまな形に変換されたデータが存在しますが、それらを元のデータとは別の「第2のマスター」へ勝手に昇格させてはなりません。
2. **Hostがマスターデータを保持し、各サブシステムが意味を管理する**  
...
3. **データの変更、ルールの強制、保存、全体操作への参加を切り離す**  
   「権限・制約」サブシステムは操作を拒否できますが、タスクや記憶のデータを勝手に書き換えられません。「保全・消去」サブシステムはシステム全体のデータ消去やバックアップを取りまとめますが、各サブシステムの日常的なデータ管理者になるわけではありません。
...
8. **AIの生成テキストをセキュリティ設定のマスターデータに直結させない**  
...
※以下で「マスターデータ」と呼ぶものは、特に断りのない限りHostマシン上で管理されるデータを指します。
...
クライアント側の画面に表示されているチャット履歴や音声バッファは一時的なコピーに過ぎず、Host側のデータがマスターデータです。会話の中でタスクの指示を受けた場合、発言ログは会話履歴に残り、実際のタスク進行は「作業」サブシステムが管理します。

音声認識で確定した発言も同じ会話履歴の出所に対応付けます。対象や条件が曖昧な依頼への確認は個体調整の会話上の状態であり、作業が受理した目的・指示ではありません。再起動後に確認を続けるときは保存済みの発言と Task 事実から現在の候補を再確認し、古い音声ストリームや推論中の候補を実行指示として復活させません。Raw Voice、音声合成出力、再生位置はマスターデータにしません。
...
- **内容とルール**:
  ...
  - 端末にデータを送信しただけで勝手に「報告完了」とみなさず、その receipt の対象事項について要約本文全体の画面提示または音声再生完了が確認されたときだけ報告状況を更新します。提示結果が不明なら未伝達のまま保持します。
...
- **内容とルール**:
  ...
  - 会話から対象候補を推測したことと、作業が現在の Task と revision を照合して指示を採用したことは別の事実です。対象が曖昧な間は作用を始めません。
...
- **内容とルール**:
  ...
  - これは外部世界そのものの状態ではなく、**「eneが外部ツールに対して何を指示し、何が確認できたか」**の記録です。
...
- **内容とルール**:
  ...
  「毎週金曜日にバックアップを取る」予定もここで管理し、到来した回は新しいTaskから現在の権限・ルールを通して保全・消去へBackup作成を依頼します。
  - スケジュール設定と、実際に予定時刻が来て実行された「各回のタスク記録」は別物です。
  - パートナー停止中やHost電源OFFの区間に到来したと証明できる予定回は「未実行（missed）」として記録します。稼働中の到来を受付前の異常終了で取りこぼし、停止との順序が証明できない回は未分類として保護し、勝手に連続実行しません。
...
- **内容とルール**:
  ...
  - 費用capはプロバイダー別または全体のUTC日額・月額を選択して設定でき、日額のみ・月額のみ・未設定も認めます。費用不明はゼロの証拠ではありませんが、送信の費用保留は現在適用されるcapのscopeと期間に限り、cap未設定や非適用期間の不明だけでは停止しません。
...
- **マスターデータの担当**: **保全・消去** サブシステム
- **内容とルール**:
  会話履歴の保持期間、過去バージョンの自動クリーンアップ設定、バックアップの保存先・暗号化選択・作成結果を管理します。定期バックアップの時刻条件と到来履歴は4.14の作業が管理し、保全・消去はBackupの対象・整合性・結果を管理します。
...
### 5.1 マスターデータとして扱う対象を限定する
システムを安定して動かすためには、「何がマスターデータで、何が派生データや単なる記録に過ぎないのか」を厳密に区別しておく必要があります。

| 混同しやすい組み合わせ | マスターデータ（原本・基準）として参照するもの | 代用・混同してはならないもの |
|---|---|---|
...
クライアントの画面表示用キャッシュ、入力途中の文字、音声バッファ、AIに渡す一時コンテキスト、MCPから取得したリソースやプロンプトの内部コピーなどは、その目的が終われば破棄されます。

**要点は、「失われてもよいのは一時的な表現や通信データだけであり、オーナーからの確定指示、タスクの記録、未伝達の重要事項、成否不明な外部操作の結果などを失ってはならない」ということです。**
...
ただし、Eneが内部に取り込んだスキルやプラグイン、MCPから取得したデータの内部コピーなどは、Eneの責任として確実に削除・管理します。
...
1. **パートナーの停止（Stop）**:
   ...
   - 画面観測（Observer）の人数カウントから除外され、アバターの表示、音声対話、自発的な行動、裏での自律タスクやPC操作（Computer Use）の新規開始を止めます。Stop成立前に送信開始したClientへの操作指示は、停止通知が届くまでに実入力を開始する可能性があるため速やかに停止を試み、停止完了や未実行を推定しません。成立後にHostから新しい操作指示を送信開始してはなりません。
...
- 復元前のcap状態を確保できない場合も全プロバイダーを一律停止するのではなく、適用され得るscopeとUTC期間だけを安全側に保留します。復元後のcap新設や課金先IDの変更でも欠損期間との照合を省略せず、適用capのない期間の費用不明だけで送信を停止しません。復元後の一括再開は復元前cap由来の安全条件を解除しません。
...
| **Ene ↔ MCPサーバー／プラグイン** | 許可されたツール実行要求、リソース、プロンプト、ツールの実行結果。 | 外部コードや受け取った入力がEneの内部データや制御権のオーナーになることはありません。MCPの入力を第一者の確認に代用しません。<br>Eneが一時的に保持したデータはEneの安全基準に従って管理します。 |
