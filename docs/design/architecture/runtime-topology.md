...
高権限操作には、信頼されたインストールの公式 GUI 上で、ユーザーが対象と影響を見て直接確認した事実が必要です。同一 UID、ペアリング、公開 local endpoint への先着は、その代わりになりません。空席でも任意の local process を確認者へ昇格させません。
...
保証の前提は、OS、公式実行ファイルと読み込むコード、Host / GUI のプロセスメモリ・継承 handle、およびユーザーの対話セッションの完全性です。これらを書き換える debugger、handle 窃取、バイナリ差し替え、ene 外の OS 入力注入が既に可能な OS セッションの侵害まで、このアプリ内 protocol が防ぐとは保証しません。一方、単に同じ UID で接続できるプロセス、通常 Client、モデル・tool・plugin の入力は保証対象内であり、確認権限を与えません。ene が認可する Computer Use による確認面への入力は、OS セッション侵害という例外へ逃がさず、発行側と adapter 側で拒否します。
