...
推論処理は機能（Capability）ごとにHostローカル、LAN内のマシン、クラウドへ柔軟に配置できます。ただし、推論先をどこに置いても、Hostが持つ実行管理の権限や個体の所有権が推論サーバーへ移るわけではありません。外部のMCPサーバーやプラグインは必要な実行環境に接続しますが、ene本体の管理権限とは明確に区別します。特に、ローカルMCPを標準のサンドボックス内で動かす場合と、オーナーの許可を得てサンドボックス外で動かす例外的な場合は、同じ外部拡張であってもセキュリティの強制境界が大きく異なります。
...
HとCはene公式の実行領域、P・M・Xは異なる信頼境界を持つ接続先や拡張コードです。これは役割の分類であり、それぞれを個別のOSプロセスにする指定ではありません。また、任意の機能や拡張機能を常時起動しておく必要もありません。

これらを区別する理由は、**「HとCの配置と寿命の違い」「Pへの推論データ送信」「Mの外部への作用とサンドボックス」「Xに与える限定的な権限」**にあります。一方で、用途別やCompanionごとに無駄な専用実行環境を増やすことはしません。

| 要素 | 役割と配置場所 | ライフサイクルと主な関係 | マスターデータの所在 | 信頼境界と障害時の性質 |
|---|---|---|---|---|
...
| **M: 外部MCPの実行** | ツール（Tool）、リソース（Resource）、プロンプト（Prompt）を提供する外部の実行主体です。Host上の作業を支えるローカルMCPはHost側に配置し、デフォルトでサンドボックス内で動かします。リモートMCPは外部システムとしてネットワーク経由で接続します。 | 必要な接続・利用の期間だけ動作します。外部サーバーの寿命はHostやClientと独立しています。eneが起動するローカルMCPの起動・終了方式やプロセス共有の仕組みは後続の設計で定めます。 | 外部サーバーが独自の状態を持ったり外部へ影響を与えたりすることはありますが、**ene内部データのマスターは保持しません。** | 実行結果、リソース、プロンプトはすべて信頼できない入力として扱います。「ローカルのデフォルトサンドボックス」「明示的なサンドボックス外の例外」「リモート接続」のセキュリティ境界を厳格に区別します。拒否・停止・利用不能時でも、ene本体の管理画面や保存済みデータは問題なく利用できるようにします。 |
...
| **Host停止・異常終了・再起動** | Client端末がHostの代わりのマスターデータになることはなく、Hostでの保存や操作が成功したか未確認のまま「成功」と表示することはありません。起動中だったCompanionの滞在先はRT-02に従って再起動前のClient端末へ自動復元を試みます。作業途中だったTaskは、保存済みの進捗状況と判明している外部への影響を画面に示し、オーナーによる明示的な再開指示を待ちます。Host停止中に到来したと証明できるスケジュール実行だけを「スキップ（missed）」扱いとし、稼働中に到来して受付前の異常終了で取りこぼした可能性がある回は未分類として保持・提示します。後から勝手に二重実行・補完実行することはありません。また、外部プロセスや外部サービスの動作がHostの停止と同時に止まったと勝手に思い込むこともしません。 |
...
| **アバター・音声・デバイス障害** | 3Dアバターの描画に失敗しても、テキスト会話でのTask操作、設定、診断、復旧操作は問題なく利用できるようにします。音声機能でトラブルが起きた場合は、通常のターン制音声や同じ会話のテキスト入力へ段階的に切り替え、失敗した理由を表示します。キーボードによるミュート、緊急停止、キャンセル、承認拒否などの操作は、アバターの描画、音声再生、LLMの応答待ちでブロックされずにいつでも受け付けます。Hostへ届いていない停止要求を「外部処理の停止に成功した」と誤認させるような表示はしません。 |
...
| **ene公式コードと外部拡張コード** | MCPの呼び出し・結果・リソース・プロンプト、制限されたプラグイン機能の入出力。 | 外部のコードやMCPから取得した入力に、ene本体のコントロールプレーン、Permission、同意設定、費用上限、認証情報を直接書き換えさせません。MCPの入力をオーナーの第一者確認や実行権限と同一視せず、アクション権限チェックを迂回させません。登録された認証情報を、必要な通信以外の画面やログへ出力しません。eneが管理する拡張機能の一時データを、「外部コード由来だから」という理由で完全削除の対象外にすることはしません。 |
...
要件が Host PC での直接確認を求める高権限操作（API キー登録、端末ペアリング、バックアップ復元、全データリセット等）には、信頼されたインストールの公式 Host GUI 上で、ユーザーが対象と影響を見て直接確認した事実が必要です。同一 UID、ペアリング、公開 local endpoint への先着は、その代わりになりません。空席でも任意の local process を確認者へ昇格させません。

Host が信頼されたインストールから確認用 GUI を起動し、その子にだけ継承した専用 channel と生存記録を使って確認面の由来を固定します。GUI は Host より短命で、GUI の終了は Host の終了を意味しません。再起動した GUI は新しい channel と確認 session を使います。一般の要求接続と、確認・秘密入力が可能な channel は分離します。具体的な起動・再接続は [First-party desktop 第5節](../concrete/first-party-desktop.md#5-trust--credential--failure) に従います。

この Host-local 高権限確認の保証前提は、OS、公式実行ファイルと読み込むコード、Host / GUI のプロセスメモリ・継承 handle、およびユーザーの対話セッションの完全性です。これらを書き換える debugger、handle 窃取、バイナリ差し替え、ene 外の OS 入力注入が既に可能な OS セッションの侵害まで、このアプリ内 protocol が防ぐとは保証しません。一方、単に同じ UID で接続できるプロセス、通常 Client、モデル・tool・plugin の入力は保証対象内であり、Host-local の確認権限を与えません。ene が認可する Computer Use による確認面への入力は、OS セッション侵害という例外へ逃がさず、発行側と adapter 側で拒否します。

この前提は本人確認の省略や自動承認を許可するものではありません。公式面を起動できない、専用 channel の由来を確認できない、またはユーザーの直接確認がない場合、操作は保留または拒否とします。

指定データの完全削除（Targeted Deletion）は、上記の Host-local 高権限確認とは別の第一者直接確認を使います。ペアリング済みの remote Client からでも、Host が目的・対象・影響を明示して固定した一回限りの challenge を認証済み device・session・connection・Client / Host incarnation に結び付け、公式 Client の専用確認面での現在のオーナー操作を受けて保全・消去が開始直前に現在性を再照合します。公式 Client は提示からオーナーの直接操作までをローカルで担い、ene 管理の LLM・tool・Computer Use に確認を代行させません。Host が検証できるのは認証済み Client の接続と challenge・対象の一致、期限・未消費・現在性であり、人の物理入力や公式 UI 由来そのものを暗号学的に証明することではありません。同じ Client 端末上で確認電文を偽造できる侵害は Host から直接操作と区別できないため、Client process・OS・公式コードと対話 session の完全性を信頼前提とします。通常の管理 intent や chat 電文に付けた確認フラグ、接続済みという事実だけでは開始できません。Host GUI がない場合でもこの専用確認は成立し、逆に高権限操作の Host GUI 確認へ転用できません（[Targeted Deletion lifecycle §15](../concrete/targeted-deletion-lifecycle.md#15-first-party-management-boundary)）。
...
flowchart TB
    ...
    subgraph hostpc["オーナー管理のHost PC"]
        h["H: eneのHost実行 (Core)<br/>内部マスターデータの保持<br/>Companion・Task・Task Agent・スケジュールの継続"]
...
推論プロバイダ（P）の「Hostローカル / LAN / クラウド」は自由に選べる配置先であり、3つの階層すべてを同時に起動しなければならないわけではありません。プラグイン（X）への2本の線も、1つの拡張機能を両方で二重起動する要求ではなく、機能に応じた配置場所を示しています。ローカルMCP（M）でオーナーが明示的に例外を許可した場合は、サンドボックスの外側へ配置されますが、その際に失われる安全性は「Trust Boundaries」の表に記載された通りとなります。

ローカル構成とリモート構成で変わるのは、Clientへの通信経路と、アバターや操作対象が存在する物理的な場所だけであり、「Hostがマスターデータを保持すること」「Host上で許可された作業が止まらずに続くこと」は一切変わりません。

## Design Freedom（後続設計に残された自由度）

以下に挙げる項目は未解決の要求ではなく、確定したセキュリティ境界と責務分担を満たすための「実装上の選択肢」です。

- **内部の責務分割と実装構造:** サブシステム、crateやモジュールの分割、型やトレイトの設計、内部API、エージェントランタイムやハーネスの内部構造、スレッドや非同期タスク、イベントバスなど。HやC、信頼境界をそのまま1対1のモジュールやプロセスに固定する必要はありません。first-party desktop の Host / GUI / Body の process 分割、要求専用 / 確認専用 channel、Host-spawned GUI への seat 発行と直接確認、投影 IPC、性能の分母は [First-party desktop](../concrete/first-party-desktop.md) が具体化しており、GUI と VRM overlay を同一 process に戻すこと、Body を paired Client にすること、Host に wgpu を入れることはここでの自由度ではありません。
...
- **拡張機能の実装技術:** サブプロセスのOSレベルの隔離技術、外部コードのエラーハンドリング、MCPプロセスの起動・終了管理、プラグインの具体的な拡張API設計など。ローカルMCPの標準隔離と明示例外、プラグインの権限制限は前提条件となります。
...
| ローカルMCPのHost配置・標準サンドボックス・明示例外、限定プラグイン | M・X、RT-06、Local MCPの実行境界、SC-05 | AD-01・06・07・11・14 / 「所有と実行」「拡張」「信頼境界」「履歴、保持、Privacy」。Host配置はClient非依存の作業継続を満たす設計判断。 |
| Clientからの観測と個体ごとの自動学習・自発的な行動 | C・P、RT-04、SC-02・03・04・10 | AD-02・06・10・12 / 「Observationと自発性」「Desktop Body」「割当と同意」「Computer Use」 |
| アバター、音声、推論、拡張の障害時でも、管理機能と保存済みデータを確実に保護する | C・P・M・X、Lifecycle and Failure Boundaries、SC-09 | AD-03・11・13 / 「BodyとVoice」「拡張」「品質と利用可能性」 |
