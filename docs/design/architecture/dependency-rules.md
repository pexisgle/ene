...
| **作業 → 保全・消去** | 担当CompanionのScheduleから各回に新しいTaskを作り、現在の権限・ルールを通してBackup作成を依頼する。 | 予定・到来・Taskは作業、Backupの対象・整合性・作成結果は保全・消去が判断する。Schedule設定だけでBackupを許可しない。 |
...
| **バックアップ・リストア・データ消去** | 手動の管理要求は入出力・提示または設定 → 保全・消去 → 各サブシステム。定期バックアップは作業のSchedule到来 → 各回の新しいTask → 現在の権限・ルールの評価 → 保全・消去へのBackup依頼。ファイル操作は実行・拡張、ルールは権限・制約。 | 保守作業を万能な編集特権にしない。Schedule設定を恒久的な実行許可とみなさず、バックアップの内容・整合性・結果の判定は保全・消去が担う。 |
...
外部連携から追加の処理要求が返ってきた場合でも、通常の安全チェックを必ず通します。MCPのツール結果・リソース・プロンプトを第一者の確認に代用しません。外部サーバー側のデータは消去できませんが、ene内部に保存された一時データやキャッシュは完全データ消去の対象に含まれます。
