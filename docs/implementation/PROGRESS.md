...
- ✅ **Stage 4**: 作業用エージェント (Task Agent) とファイル操作タスク
  - 完了済みスライス:
    ...
    - ✅ スライス F（会話・第一者管理経路）:
      - 会話の production 経路: companion が provider 出力の最初の non-empty line に付す closed-world の `[task-control]` directive（各 command の field set も閉じる）を dialogue の `finish_turn` が解釈し、composition root の `DialogueTaskControlPort` が既存 Task owner 境界（Task 作成・委任・steering・progress/report・cancel）へ写す。task-control turn は provider の自然文を reply として残さず、owner の typed outcome を scrub した後に presentation し、同じ文字列を History に保存する（directive 行は presentation / History の双方から隠れる）。通常の Owner message だけで一連の操作が起動し、テスト側の直接 API 呼び出しを必要としない。
