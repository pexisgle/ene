...
- **Stage 7: 管理画面・デスクトップアバター・最初の受け入れ検証 — A1 と B/C/E は統合済み、D/F の実 desktop acceptance に blocker が残る**
  ...
  - 残件: 公式 VRM（[#1651](https://github.com/pexisgle/ene/issues/1651)）、KDE Wayland 実 overlay、Linux Secret Service、Linux 側 Performance Gate。Windows側の残件は公式VRMだけ。placeholder や Seed-san を製品キャラクターとして扱わない。
...
- Stage 7 完了には、公式VRM、KDE Wayland 実 overlay、Linux Secret Service、Linux 側の残る IME / Performance Gate が必要。Windows側の残件は公式VRM #1651だけ。
- 個別の非ブロッキングな設計・実装フォローアップは GitHub Issues で管理します。
- Stage 7 の Linux 自動テストと X11 GUI 操作は [Linux 報告](reports/stage-7-linux-2026-09-19.md) にある。Windows 11 D/F runtime / platform acceptanceは完了し、Windows側の残件は公式VRM #1651だけ。KDE Wayland、Linux Secret Service、Linux側IME / Performance Gateは未完了。
