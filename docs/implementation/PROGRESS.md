...
Task Agent の Host graceful shutdown も、Task cancel とは別の Host-lifecycle `DispatchAbort` で quiesce します。stop 前に claim 済みの provider attempt は Unknown usage を永続化してから runner を join し、完了済みの外部作用 fact と Task の非 terminal 状態を維持します。AU14 / AU5 claim は Host shutdown と同じ短い commit scope で直列化します。workspace 作用は kill/reap 可能な隔離 worker で実行し、10 秒の cooperative phase を超えた場合は全 worker を hard stop してから全 runner の実 join 完了を待ちます。timeout error は全 Task Agent の停止確認後にだけ返します。cancel との競合、shutdown-first / claim-first、停止後に mutation がないこと、restart 後に old execution を replay しないことを自動テストで検証しています。
