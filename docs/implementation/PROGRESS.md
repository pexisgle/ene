...
Task Agent の Host graceful shutdown も、Task cancel とは別の Host-lifecycle `DispatchAbort` で quiesce します。stop 前に claim 済みの provider attempt は Unknown usage を永続化してから runner を join し、完了済みの外部作用 fact と Task の非 terminal 状態を維持します。cooperative に終了しない work には 10 秒の join 上限を設け、Host 終了を無期限に延ばさず明示的な技術的失敗へ戻します。cancel との競合、cancel しない shutdown、restart 後に old execution を replay しないことを自動テストで検証しています。
