...
- ✅ **Stage 4**: 作業用エージェント (Task Agent) とファイル操作タスク
  - 完了済みスライス:
    ...
    - ✅ スライス F（会話・第一者管理経路）:
      - 会話の production 経路: companion が provider 出力の最初の non-empty line に付す closed-world の `[task-control]` directive（各 command の field set も閉じる）を dialogue の `finish_turn` が解釈し、composition root の `DialogueTaskControlPort` が既存 Task owner 境界（Task 作成・委任・steering・progress/report・cancel）へ写す。`[task-control]` は予約 protocol であり、最初の non-empty line 以外に現れた場合は invalid として fail closed する（Task operation なし・返信保存なし・Interrupted。marker / JSON はどの位置でも presentation / History に出ない）。task-control turn は provider の自然文を reply として残さず、owner の typed outcome を scrub した後に presentation し、同じ文字列を History に保存する。通常の Owner message だけで一連の操作が起動し、テスト側の直接 API 呼び出しを必要としない。
      - Workspace authority: Task の Workspace association は第一者管理経路で Owner が選択し Host が canonical directory として検証した trusted premise からのみ成立する。provider / model 出力の任意 path は association / delegation scope / Action authority にならない（protocol に workspace field はなく、extra field は invalid）。
      ...
      - 実行開始: Task 作成 + delegation の受理後、serving composition（shared handle と provider transport）が install した launcher が既存 `run_task_agent` を background 起動する。会話 turn は実行完了を await せず、runner の technical failure を `TaskProgress::Failed` に写さない。会話からの steering が受理された場合は new revision 用の新しい delegation を作成して同 launcher で起動し、old execution は既存 revision gate で stale / refused になって停止する（old delegation の再利用・replay はしない）。
