...
### UI redesign への統合（`codex/stage7-integration`）

[UI redesign 報告](stage7-ui-redesign-windows-2026-09-19.md) の GUI は上記 3 番のレイアウトを置き換えたため、この Linux 修正をそのまま移植せず、redesign 側で同じ不具合が成立しないことを確認した。

| 修正 | 統合 tip での扱い |
|---|---|
| 50ms tick の古い snapshot 適用 | redesign は周期 snapshot 適用を持たず、worker が 1 コマンドずつ apply 完了を待つ。`SnapshotPump` は追加しない |
| ページ切替を Host refresh の前に描く | ナビは GUI スレッドで page を設定してから Refresh を queue する。`apply` は page を書かない |
| ナビ行の 32px 固定 | ナビは管理ウィンドウの縦リスト（stretch する横一列）へ置き換わった。stretched nav は再現しない |
| `CredentialRefused` の区別 | 移植した。`CredentialRefused` / `DeviceUnknown` / `Unavailable` をドメイン結果として扱い、ウィザードへ戻す |

この表は Linux 実画面の再確認ではない。integrated tip の Linux / Windows 再検証は F の残件である。
