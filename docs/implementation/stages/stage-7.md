...
wire を変更する前に、[基盤計画 F1](../follow-ups/foundation.md#f1-通常-wire-の-unknown-値と-round-指定) の未知 payload と round 指定を設計・PR 順へ反映します。これらを WSS 採用だけで解決済みにせず、通常 codec / DTO の境界でそれぞれ検証します。
...
設定完了後の画面と再起動時の初期画面、履歴の最新位置と新着追従、会話表示・入力・失敗状態の残作業は [Desktop 計画 D1](../follow-ups/desktop.md) に従う。窓と Body / Host の寿命変更は同計画 D4 の要件・設計 gate を先行する。
...
Learning の明示的な割り当て、未設定理由の表示、形成方式の後続作業は [Learning 計画](../follow-ups/learning.md) に従う。
...
上限の意味、価格取得、期間集計・cap 編集、provider 障害の表示は [費用計画](../follow-ups/usage.md) に従う。現行の予約契約を実績ベースへ変える場合は要件・設計を先に更新する。
...
待機/発話中の仕草は同梱モーションパック（VRoid `VRMA_MotionPack`、`.vrma`）で再生します。モーションは VRM と同じ install asset であり、本リポジトリには置きません（取り出し可能な形での二次配布は規約で禁止）。配布ページと配置先は [`assets/README.md`](../../../assets/README.md#vroid-motion-pack-install-asset) に記載し、手動で配置します。`ene-desktop` は探索して読み込むだけで、実行時にダウンロードやコピーは行いません。`ene-body` は渡された pose → clip の割り当てだけを再生します。探索順・環境変数・割り当て表は [`apps/ene-body/README.md`](../../../apps/ene-body/README.md#motion-pack-vrma) を正本とします。パックが無い状態は手書きの仕草で継続し、`HealthTick.motion` が `Unsupported` として明示されます。公式 `ene` VRM での製品 acceptance は、サンプル VRM や motion pack の技術成立 probe と分けます。
...
モーションによる framing の変動、公式 VRM、物理 display scale=2 の受け入れは [Desktop 計画 D2–D3](../follow-ups/desktop.md) に従う。
