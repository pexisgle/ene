...
ステータス: **実装計画。D0 の要件・設計契約は未確定。** この文書だけで製品要件や authority を変更しません。正本は [requirements](../../requirements/README.md)、[acceptance](../../requirements/acceptance.md)、[design](../../design/README.md) です。
...
現行要件には Task 管理画面・進捗表示と、画面による報告提示を前提にした記述があります。D0 はその変更を上位文書から反映する **docs-only PR** とし、設計の不足を実装だけで決めません。
...
- 発話を生成・送信しただけでは「伝達済み」にしない。どの再生・提示事実で receipt を確定し、途中停止や失敗では何を未伝達に残すかを定義する。再生完了を、ユーザーが実際に聞いて理解した証拠、操作への承認、Task 成功の証拠と混同しない。
...
[Stage 7 A2 / #1705](https://github.com/pexisgle/ene/issues/1705) と、[#1700](https://github.com/pexisgle/ene/issues/1700) の round 指定を音声入力の接続前に調整します。Host / Client 間の音声統合は WSS の公開境界を使い、旧 transport 用と新 transport 用の二つの production 経路を作りません。D0、対象検索、音声デバイスの技術成立 probe は独立して先行できます。

[#1390](https://github.com/pexisgle/ene/issues/1390) の未知 payload と、[#1684](https://github.com/pexisgle/ene/issues/1684) の表示言語・起動パスは、各境界の変更時に調整します。primitive 整理など無関係な follow-up の全件完了を、この Stage の一律な開始条件にはしません。
...
| 報告の提示 | mute・静粛時間・barge-in・再生失敗・切断中は未伝達を正しく保持する。復帰後は現在の状況へまとめ直し、変化のない進捗を話し続けない |
