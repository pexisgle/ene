...
- `ene-plugin-host` is host-side only: process discovery/spawning, handshake negotiation, capability routing into the tool and LLM/TTS/STT/VAD provider registries, health probes, and shutdown. It bridges plugin-provided providers into the `ene_ai` traits via IPC adapters, and aggregates plugin-provided and MCP tools behind a single tool-registry interface.
