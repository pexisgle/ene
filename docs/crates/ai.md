...
> **Crates**: `ene-ai` (provider traits, message/streaming types, routing policy) | `plugins/provider/local-llm` (`ene-plugin-llama-cpp`, local GGUF inference via `llama-cpp-4`)

`ene-ai` provides LLM chat-completion and text-embedding abstractions for Ene: the generic message/streaming types, provider traits, task routing, failover policy, and retry policy. The plugin host owns the single provider registry and bridges concrete cloud and local providers from `plugins/provider/*` into those traits; local audio (STT/TTS/VAD) is served by the `ene-plugin-whisper` / `ene-plugin-onnx` / `ene-plugin-kokoro` provider plugins, sharing engine code from the separate `ene-voice` crate.
