...
- **Why there is no deterministic keyword appraisal at turn start**: the chat model already reads the current user utterance when generating the reply, so same-turn emotional reaction belongs in that reply. `EmotionEngine` keeps cross-turn continuity (decay, conversation fatigue, post-turn classifier → next turn). The `CharacterState` prompt notes that affect is pre–current-utterance; end-of-turn expression follows the response via `resolve_expression` (LLM proposal canonical).
