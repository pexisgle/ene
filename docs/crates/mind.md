...
- **Why topic compression is retroactive and the window-pressure trigger is token-based** (#368): a topic change is hard to judge instantly and not worth delaying the response for, so boundary detection and summarization run in the deferred post-turn slot (after `Terminal`, alongside memory writing and affect classification) — the response is never blocked, and the assistant's own response is available as extra judgment material. A detected boundary compresses the span *before* it into one scene span (one topic = one summary), leaving "previous-topic summary + recent turns" for the next turn. The secondary window-pressure trigger compares an estimate of the retained history's tokens against a configurable ceiling (`ContextConfig::context_pressure_tokens`) instead of the old message-count ratio and its hard-coded `1.25`, so it acts as a token-aware safety net for over-long topics that never produce a boundary.
