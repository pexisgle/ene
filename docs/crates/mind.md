...
- **Why proactive speech reads memory two ways**: the decision phase injects the user's stored `Preference` / `UserProfile` memories ("don't talk while I work") deterministically — read straight from the store, never through recall scoring, so a suppression condition cannot be dropped by a low score — while the generation phase recalls memories with a lexical-only search keyed on the decision's `topic_hint` (no embedding provider on the proactive path). Both are gated by `mind.proactive.sources.memory`; see `docs/configuration.md`.
...
- **Why relationship stages bind to `affect.affinity`**: the character–user relationship has no dedicated score field; the persistent, clamped affinity dimension is the state that survives across turns, so `extensions.ene.relationship_stages` thresholds are evaluated against it at kernel compile time. Tone lines render only when a threshold matches, keeping cards without the block byte-identical to the previous kernel.
