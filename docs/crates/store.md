...
`ene-store` is the sole owner of the SQLite/SeaORM connection, schema migrations, and vector similarity search (`sqlite-vec`) for the entire workspace. It also runs the shared host-service acceptor (`host_service`) with a `db` passenger (`db_server`) so stateful tool plugins can perform scoped, schema-declared CRUD without opening their own database connection.

---

## Architectural boundaries

- `ene-store` is the **sole owner** of the SQLite / SeaORM connection and schema for the entire workspace. No other crate (`ene-mind`, `ene-runtime`, tool binaries) opens its own database connection or issues raw SQL against `memory.db`; they call into `MemoryStore`, or — for plugin binaries — the IPC-based `ene-plugin-db` client backed by `ene-store`'s host-service `db` passenger.
- `ene-store` does **not** depend on `ene-runtime`, `ene-ai`, or `ene-mind`. It depends on `ene-config`, `ene-core`, and the plugin wire crates (`ene-plugin-proto` / `ene-plugin-db`) for the host-service `db` passenger, so it sits low in the dependency graph and can be called safely from higher crates without introducing a cycle.
...
- **Why a host-service `db` passenger instead of shared file access**: stateful out-of-process tool plugins (`ene-plugin-fs`'s undo ledger, `ene-plugin-utility`'s todo store) need durable state without each becoming a second SQLite writer against `memory.db`. Plugins connect to one shared host-service socket and open the `db` passenger with a per-plugin token; `db_server` then enforces per-tool schema declarations and prefix-based table isolation over the wire. It also enforces a per-plugin storage quota (`plugins.list.<name>.db_quota_mb`): because every plugin writes into the one shared `memory.db`, prefix isolation alone cannot stop a runaway plugin from filling the disk, so `db_server` measures each plugin's footprint and rejects storage-growing writes past the cap (reads and deletes stay permitted).
