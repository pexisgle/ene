...
- **Why a per-tool DB IPC server instead of shared file access**: stateful out-of-process tool plugins (`ene-plugin-fs`'s undo ledger, `ene-plugin-utility`'s todo store) need durable state without each becoming a second SQLite writer against `memory.db`. `db_server` enforces per-tool schema declarations and prefix-based table isolation over the wire instead. It also enforces a per-plugin storage quota (`plugins.list.<name>.db_quota_mb`, #424): because every plugin writes into the one shared `memory.db`, prefix isolation alone cannot stop a runaway plugin from filling the disk, so `db_server` measures each plugin's footprint and rejects storage-growing writes past the cap (reads and deletes stay permitted).
