...
- The `workspace` module (default features) owns document/workspace RAG policy — chunking with heading/line-range metadata, ignore-rule glob matching, hybrid chunk scoring, and the `rag.workspace` configuration. It is the third RAG consumer (after memory recall and tool selection); the runtime indexer orchestrates I/O and `ene-store` persists the index through `ene_core::WorkspaceDocumentPort`.
...
Start at the `scoring` and `decay` modules for the memory policy, the `workspace` module for document/workspace policy, and the `tool` module (feature-gated) for the tool-selection pipeline.
