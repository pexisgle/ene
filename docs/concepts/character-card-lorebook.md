...
section) is not wired yet: entries land in the recall block only, while the
decorators are parsed and stripped regardless.
...
|---|---|
| `@@activate_only_after N` | Match only once the chat log's assistant (character) message count reaches `N`. The count covers the whole log, never the recent-turn window. `N = 0` disables the gate. |
| `@@activate_only_every N` | Match only when the chat log's assistant message count divides `N` evenly. `N <= 1` disables the gate. |
| `@@keep_activate_after_match` | Once matched, always matched (sticky). |
| `@@dont_activate_after_match` | Once matched, never matched again. |
| `@@activate` | Force a match "in any case" (overrides `@@dont_activate` and the key filters). |
...
matches are unknowable, so they are inert there. The injection pipeline, which
will hold session-held state, is not wired yet.

### Placement

| Decorator | Ene behavior |
|---|---|
| `@@depth N` | Parsed; message-depth injection is not wired yet. `N < 1` means prefill, which Ene does not support — it falls back to a high-priority prompt position. |
| `@@reverse_depth N` | Parsed; counted from the oldest message (message-depth injection is not wired yet). |
| `@@instruct_depth N` | **Ignored** — Ene is chat-based and the spec says to ignore this in chat contexts; a following `@@@` fallback line is consulted. |
| `@@position after_desc\|before_desc\|personality\|scenario` | Parsed; semantic-section placement is not wired yet. |
...
| `@@instruct_scan_depth N` | **Ignored** — chat context (see `@@instruct_depth`); `@@@` fallback consulted. |
| `@@is_greeting N` | **Ignored** — Ene does not track the active greeting index; the spec says to ignore the decorator when it cannot be checked. `@@@` fallback consulted. |
...
| `@@exclude_keys X,Y,…` | Entry suppressed when any value appears. Ignored when `use_regex` is true (per spec). NOT logic lives here: the Ene-proprietary `not_keys` card field was consolidated into this decorator and removed. |
| `@@is_user_icon name` | **Ignored** — Ene has no user-icon prompt feature; `@@@` fallback consulted. |

### Meta

| Decorator | Ene behavior |
|---|---|
| `@@role assistant\|system\|user` | Parsed; applies to depth-injected messages once message-depth injection lands. |
| `@@ignore_on_max_context` | Parsed; to be consumed by the token-budget pass once injection lands. |
| `@@disable_ui_prompt post_history_instructions\|system_prompt` | **Ignored** — honoring it would let cards disable Ene's expression output contract, which is a core product guarantee; `@@@` fallback consulted. |

## Fallback chains (`@@@`)

An unknown, invalid, or context-ignored decorator is skipped, but its
**fallback** — the following `@@@name` line — is consulted instead, top to
bottom (the spec's "not recognized ... or decorator is ignored" rule). Chains
can be any length:
