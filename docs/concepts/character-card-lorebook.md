...
| `@@is_greeting N` | Only matches when the session's active greeting index is `N` (`0` = `first_mes`, `i+1` = `alternate_greetings[i]`). Inert when no greeting was chosen — the spec's "ignore when the active greeting cannot be checked" case. |
