...
Before a candidate memory is persisted, the memory arbiter checks whether it contradicts an existing memory of the same kind (`Preference`, `UserProfile`, `Semantic`, `Relationship`, `Commitment`). Which kinds are checked — and how their subjects are keyed — is defined in one place, the per-kind policy table on `MemoryKind`. Two memories are treated as the same subject when the cosine similarity of their **title embeddings** reaches `mind.memory.contradiction_title_similarity_threshold` (default `0.82`), so synonymous titles ("職業" vs "仕事") collapse into one subject instead of accumulating contradictory duplicates. When no embedding provider is configured, the arbiter falls back to exact normalized-title matching.

Once a contradiction is found, the arbiter compares the candidate's confidence against the existing memory's: if it exceeds it by at least `mind.memory.supersede_confidence_delta` (default `0.05`) the new memory *supersedes* the old one; if the shortfall is within `mind.memory.dispute_confidence_gap` (default `0.15`) the existing memory is marked *disputed*; otherwise the decision is deferred to user confirmation. Semantic-duplicate detection during deduplication uses `mind.memory.semantic_similarity_threshold` (default `0.85`). All four thresholds — together with `min_confidence_to_persist` — are configurable under `mind.memory.*` and clamped into `0.0..=1.0` on load (#352).
...
Every arbiter decision that persists a memory stamps an `outcome_rating` (-1.0 negative ..= 1.0 positive) onto both the action and the applied result, derived from the turn's affect valence (`PostTurnInput.affect.valence`). When reflection is enabled, each rated decision is written to the durable `memory_outcomes` table (`rating`, `source`, `source_ref`, `created_at`); the `source` discriminator (`affect` | `user_feedback`) reserves the storage contract for an explicit user-feedback path. Rating writes invalidate the L1 recall cache, so the next recall pass observes the fresh evaluation. The reflection pass aggregates every unconsumed outcome row for the character (newest-first, bounded per pass): rows recorded by calls that did not meet the generation gate — including calls before a restart — are consumed by the next pass, and consumed rows are deleted so the table stays bounded and no window is aggregated twice. Post-turn writes for one character are serialized by the caller, so the unconsumed pool is not split across sessions. In approval mode the rating is carried on the deferred candidate and recorded when the candidate is approved, so approved memories enter the loop even though the approval insert bypasses the arbiter.
