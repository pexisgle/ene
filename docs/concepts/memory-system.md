...
Deferred candidates are parked in the `pending_candidates` queue (`AskConfirmationLater`, #174). They are listed on the desktop settings screen for manual review, and they also compete in **normal hybrid recall** (#356): since a deferred candidate has no embedding, recall loads the live queue and lets topic-related rows (lexical overlap with the query) compete in the ordinary score competition. A surfacing candidate is rendered with an `[unconfirmed]` marker in the prompt, so the character treats it as hearsay to verify in conversation rather than asserting it as fact. `mind.memory.recall_pending_candidate_limit` (default `3`) caps how many compete per turn; `0` disables the recall path entirely without affecting the settings-screen review list.
