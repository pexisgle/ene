...
| **PR5 — v2: click-through (Win32 subclass + Wayland input region + X11 shape) + offscreen mask** | §4 PR5 | **PR5.2 (Windows) + PR-LX.1 (Linux platform scaffold) + PR-LX.2 (Wayland `set_input_region` via stand-alone `wayland-client 0.31` connection) + PR-LX.3 (offscreen `Rgba8Unorm` mask capture + readback + rectangle decomposition, 5 unit tests) + PR-LX.4 (Wayland `zwlr_layer_shell_v1` detection + `F8` "freeze character window" hotkey fallback for xdg-shell-only compositors) + PR-LX.5 (X11 `_NET_WM_STATE_SKIP_TASKBAR` / `_SKIP_PAGER` + shape extension input region via `x11rb 0.13`, 5 unit tests) + PR-LX.6 (mask capture state threaded through `AppState` + runtime resize on `WindowEvent::Resized` / `ScaleFactorChanged` + `apply_linux_click_through` derives the per-frame rectangle set from `MaskCaptureCamera::extract_rectangles` for both Wayland `set_input_region` and X11 `shape::rectangles`; mask capture gizmo `apps/ene-desktop/src/mask_gizmo.rs` overlays the downsampled rectangles as a 4-segment wireframe, gated on F3 OR the new "Linux mask overlay (debug)" Character settings checkbox) + PR-LX.7 (mask render pass: `crates/ene-vrm/src/mask_renderer.rs` builds a solid-color WGSL pipeline (`shaders/mask.wgsl`) bound to the orthographic character camera + `ModelUniform`; `render_char_frame` records the pass into a separate encoder + encodes the readback, `about_to_wait` blocks on the readback) shipped.** PR-LX.8 (input region debug overlay: F9 toggle + `InputRegionSource::{Empty,Freeze,FullWindow,Mask}` color-coded wireframe + `apply_linux_click_through` now writes back `last_applied_input_rects` + `last_input_source` on `AppState` + **window-pixel upscaling fix** (downsampled rects from `MaskCaptureCamera::extract_rectangles` are now multiplied by `mask.downsample()` before being pushed to Wayland `set_input_region` / X11 `shape::rectangles`)) shipped. PR-LX.9 (off-thread mask readback: new `apps/ene-desktop/src/platform/mask_readback.rs::MaskReadbackWorker` spawns a per-frame `std::thread` that owns a clone of the `MaskCaptureCamera` handle + `wgpu::Device` + `wgpu::Queue`; bumps an `Arc<AtomicU64>` generation so stale completions are dropped on resize / dropped frames; `render_char_frame` calls `request_readback()` after the mask encoder `queue.submit`; `about_to_wait` no longer blocks on the readback — the `#[cfg(any())]` gate from PR-LX.7 is removed) shipped. PR-LX.10+ (mask_gizmo real-time wire + settings-page persistence of the F9 toggle) tracked separately | PR5.2 (2026-06-20 redesign): winit's `Window::set_cursor_hittest` is the only OS-facing touchpoint. The global cursor position is polled each frame via the `device_query` crate (`device_query::DeviceState::new().get_mouse().coords`) and converted to window-local using `Window::outer_position()` + the scale factor. The hit test is a Rapier raycast against **per-bone colliders** (sphere / capsule / Y-capsule) sized from the actual mesh's per-vertex skinning weights: `CharacterRenderer::build_character_bone_specs` walks `model.humanoid` once and emits a `BoneShapeSpec` per humanoid bone — `BoneShape::Sphere` for head / jaw / eyes / shoulders / toes, `BoneShape::Capsule` along the parent → bone rest direction for upper / lower arms + legs + hands, `BoneShape::CapsuleY` for neck / spine / chest / upperchest / hips / foot. `VrmPrimitive` retains `Vec<MeshVertex>` (cloned at load time) and `collider::collect_weighted_world_positions_into` filters vertices whose dominant skinning weight is `< VERTEX_WEIGHT_THRESHOLD = 0.25` for the target bone, so a vertex whose dominant weight targets the *shoulder* is not allowed to inflate the *arm* capsule. `PhysicsWorld::add_character_bone_colliders` registers them as a single kinematic body parked at the world origin. Every `about_to_wait`, after `update_motion` has refreshed `model.nodes.world_positions` *and* the per-bone rest → current world rotation, the runtime calls `update_character_bone_positions` which pushes each new pose into the matching collider via `set_translation_wrt_parent` *and* `set_rotation_wrt_parent` (axis-angle via `Quat::to_axis_angle`) — so the colliders follow the animation (both position and orientation) without any per-frame GPU readback. `allows_input = !transparent || cursor_over || drag.is_dragging()`; the `is_dragging` branch keeps the window receiving input across the drag. PR5.3 (Wayland `wl_surface::set_input_region`), PR5.4 (X11 shape), PR5.5 (Linux offscreen mask + gizmo) are still pending. |
| **PR5.5 — rename `apps/ene-desktop-v2` → `apps/ene-desktop`, delete legacy Bevy sources** | §4 PR5.5 | **Shipped (2026-06-20)** | The legacy Bevy 0.18 `apps/ene-desktop` directory was deleted in a single commit; the v2 `apps/ene-desktop-v2` was renamed to `apps/ene-desktop` and its package name updated. The rename gate was loosened to "Windows portion complete, Linux portion (PR5.3 Wayland / PR5.4 X11) tracked as a separate issue" per the user-approved plan. The workspace now contains a single desktop app. |
...
> **Status (2026-06-20):** **PR5.2 (Windows) shipped — via per-bone Rapier colliders (sphere / capsule / Y-capsule, sized from the actual mesh's per-vertex skinning weights, with per-frame translation *and* rotation updates so limb capsules follow the animation).** PR5.1's BVH-trimesh approach was retired (the rest-pose trimesh never tracked the per-frame animation, so face/feet stayed unclickable while the default VRMA was playing). The per-bone collider approach reads the current bone world positions from `VrmModel::nodes::world_positions` (which `update_skin_palette` refreshes each frame) and updates each collider's local translation *and* rotation via Rapier's `set_translation_wrt_parent` / `set_rotation_wrt_parent` — the colliders follow the animation without any per-frame GPU readback. **PR5.6 (collider debug overlay, now also draws capsule wireframes) and PR5.7 (direct-render in `about_to_wait` — bypasses winit 0.30.13 multi-window `RedrawRequested` bug, restores `WaitUntil(deadline)` frame pacer) shipped.** **PR-LX.1 (Linux platform scaffold), PR-LX.2 (Wayland `set_input_region` via a stand-alone `wayland-client 0.31` connection — `apps/ene-desktop/src/platform/wayland_region.rs`), PR-LX.3 (offscreen `Rgba8Unorm` mask capture + GPU readback + greedy row-by-row rectangle decomposition in `apps/ene-desktop/src/platform/wayland_mask_capture.rs`, with 5 unit tests), PR-LX.4 (Wayland `zwlr_layer_shell_v1` detection probe in `apps/ene-desktop/src/platform/wayland_layer_shell.rs` + `F8` "freeze character window" hotkey fallback for xdg-shell-only compositors), PR-LX.5 (X11 `_NET_WM_STATE_SKIP_TASKBAR` / `_NET_WM_STATE_SKIP_PAGER` via `x11rb 0.13` ClientMessage + shape extension input region via `x11rb::protocol::shape::rectangles(SO::SET, SK::INPUT, …)` in `apps/ene-desktop/src/platform/x11_taskbar.rs`, with 5 unit tests covering the `X11Path` state machine + `x11_window_id` extraction), and PR-LX.6 (mask capture `MaskCaptureState` threaded through `AppState`, runtime resize on `WindowEvent::Resized` / `ScaleFactorChanged`, `apply_linux_click_through` derives the per-frame rectangle set from `MaskCaptureCamera::extract_rectangles` and pushes it into both the Wayland input-region context and the X11 shape extension, plus the `apps/ene-desktop/src/mask_gizmo.rs` 4-segment wireframe overlay gated on F3 OR the new "Linux mask overlay (debug)" Character settings checkbox — 2 unit tests) shipped.** PR-LX.7 (mask shader / render pass into the offscreen target), PR-LX.8 (input region debug overlay, F9 toggle + `InputRegionSource` color-coded wireframe + window-pixel upscaling fix), and PR-LX.9 (off-thread mask readback worker — `apps/ene-desktop/src/platform/mask_readback.rs`, per-frame `std::thread::spawn` with `Arc<AtomicU64>` generation gating) shipped.
...
- `apps/ene-desktop-v2/src/mask_gizmo.rs` — **new** (PR-LX.6 mask-capture gizmo). `build_mask_rect_lines(out, mask, window_w, window_h, downsample)` reads `MaskCaptureCamera::extract_rectangles()` and pushes a 4-segment wireframe per downsampled rectangle to the existing `DebugRenderer` accumulator. The gizmo is pure CPU; the runtime calls it inside `render_char_frame` whenever `show_collider_debug` (F3) or the new `UiState::debug_overlay_visible` Character settings checkbox is on. 2 unit tests pin the colour constant and the `out.clear()` semantics.
...
- `apps/ene-desktop-v2/src/platform/` — **re-introduced** in PR-LX.1 (sub-modules: `platform_runtime`, `wayland_region`, `x11_taskbar`). PR-LX.2 wired the Wayland input-region context. PR-LX.3 added `wayland_mask_capture` (offscreen `Rgba8Unorm` target + readback + rectangle decomposition, 5 unit tests). PR-LX.4 added `wayland_layer_shell` (detection probe + `F8` freeze-hotkey fallback) and `WaylandInputRegionContext::clone_connection` so the layer-shell probe shares the stand-alone connection. PR-LX.5 replaced the X11 stub with the real `x11rb 0.13` implementation (`X11Context` opens `x11rb::connect(None)`, interns `_NET_WM_STATE` / `_NET_WM_STATE_SKIP_TASKBAR` / `_NET_WM_STATE_SKIP_PAGER`, sends the EWMH `ClientMessage` on construction, and pushes the shape extension input region via `x11rb::protocol::shape::rectangles(SO::SET, SK::INPUT, …)`; 5 unit tests cover the `X11Path` state machine + `x11_window_id` extraction). PR-LX.6 threaded `MaskCaptureState` through `AppState` and the runtime (resize on `WindowEvent::Resized` / `ScaleFactorChanged`; `apply_linux_click_through` derives the per-frame rectangle set from `MaskCaptureCamera::extract_rectangles` and pushes it to both Wayland `set_input_region` and X11 `shape::rectangles`; the `apps/ene-desktop/src/mask_gizmo.rs` overlay draws the rectangle set as a 4-segment wireframe). PR-LX.7 added the `MaskRenderer` pipeline (`crates/ene-vrm/src/mask_renderer.rs` + `crates/ene-vrm/src/shaders/mask.wgsl`) and wired it into `render_char_frame` (separate encoder for the offscreen `Rgba8Unorm` target + immediate `encode_readback`; `about_to_wait` blocks on `MaskCaptureCamera::read_back` so the next `apply_linux_click_through` sees fresh data). PR-LX.8 added `apps/ene-desktop/src/input_region_debug.rs` (`InputRegionSource::{Empty,Freeze,FullWindow,Mask}` + `build_input_region_debug_lines` 4-segment wireframe) + 3 new `AppState` fields (`last_applied_input_rects` / `last_input_source` / `show_input_region_debug`) + the F9 hotkey + a window-pixel upscaling fix in `apply_linux_click_through` (downsampled rects from `MaskCaptureCamera::extract_rectangles` are now multiplied by `mask.downsample()` before being pushed to Wayland / X11). PR-LX.9 added `apps/ene-desktop/src/platform/mask_readback.rs::MaskReadbackWorker` (per-frame `std::thread::spawn` that owns an `Arc<Mutex<MaskCaptureCamera>>` + `Arc<wgpu::Device>` + `Arc<wgpu::Queue>` + `Arc<AtomicU64>` generation; `run_single_readback` calls `map_async` and blocks on `mpsc::Receiver::recv()` **on the worker thread**, then `copy_mapped_into_last_readback` + `unmap_readback`; 2 unit tests cover `Send`+`Sync` and generation monotonicity under contention) + a new `AppState::mask_readback_worker` field + `Runtime::resumed` now spawns the worker alongside `MaskCaptureState` + `Runtime::render_char_frame` calls `request_readback()` after the mask encoder `queue.submit` + the `#[cfg(any())]` gate on the synchronous readback in `Runtime::about_to_wait` (PR-LX.7) is removed.
- `apps/ene-desktop-v2/Cargo.toml` — **shipped** (PR5.1): `device_query = "2"` added; `windows-sys` no longer needs the `Win32_UI_Shell` feature. Linux target gained `wayland-client = "0.31"`, `wayland-protocols = "0.31"`, and `bitflags = "2"` (all under workspace dependency entries) in PR-LX.1 for the input-region context (PR-LX.2). PR-LX.4 added `wayland-protocols-wlr = "0.3"` (default-features = false, `client` feature) for the `zwlr_layer_shell_v1` detection probe. PR-LX.5 added `x11rb = "0.13"` (default-features = false) for the X11 fallback (EWMH `ClientMessage` + shape extension input region). `raw-window-handle` is re-exported by `winit 0.30` and requires no extra entry. `crates/ene-vrm` gained `crates/ene-vrm/src/mask_renderer.rs` (and the `shaders/mask.wgsl` shader it embeds) in PR-LX.7; no new crate dependencies were required.
