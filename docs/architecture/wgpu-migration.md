...
| **PR5 — v2: click-through (Win32 subclass + Wayland input region + X11 shape) + offscreen mask** | §4 PR5 | **PR5.2 shipped (Windows)**, PR5.3+ pending | PR5.2: winit's `Window::set_cursor_hittest` is the only OS-facing touchpoint. The global cursor position is polled each frame via the `device_query` crate (`device_query::DeviceState::new().get_mouse().coords`) and converted to window-local using `Window::outer_position()` + the scale factor. The hit test is a Rapier raycast against **per-bone sphere colliders** built from the humanoid registry: `CharacterRenderer::build_character_bone_data` walks `model.humanoid` once, computes each bone's rest-pose world position and an auto-sized radius (0.6× the distance to the furthest child bone, with a 0.05 m default for leaf bones), and filters out bones smaller than 0.025 m (typically fingers). `PhysicsWorld::add_character_bone_colliders` registers all surviving bones as a single kinematic body parked at the world origin. Every `about_to_wait`, after `update_motion` has refreshed `model.nodes.world_positions`, the runtime calls `update_character_bone_positions` which translates each sphere to its new local-frame position via Rapier's `set_translation_wrt_parent` — so the colliders follow the animation without any per-frame GPU readback. `allows_input = !transparent || cursor_over || drag.is_dragging()`; the `is_dragging` branch keeps the window receiving input across the drag. PR5.3 (Wayland `wl_surface::set_input_region`), PR5.4 (X11 shape), PR5.5 (Linux offscreen mask + gizmo) and the frame pacer are still pending — see `docs/wgpu_migration_gaps.md`. |
...
> **Status (2026-06-19):** **PR5.2 (Windows) shipped — via per-bone Rapier sphere colliders.** PR5.1's BVH-trimesh approach was retired (the rest-pose trimesh never tracked the per-frame animation, so face/feet stayed unclickable while the default VRMA was playing). The per-bone sphere approach reads the current bone world positions from `VrmModel::nodes::world_positions` (which `update_skin_palette` refreshes each frame) and updates each sphere's local translation via Rapier's `set_translation_wrt_parent` — the colliders follow the animation without any per-frame GPU readback. PR5.3+ (Wayland input region, X11 shape, Linux offscreen mask + gizmo, frame pacer) are still pending.
...
1. **Windows click-through (PR5.2 — shipped 2026-06-19, redesigned twice)** — three pieces, none of which touch Win32 directly:
   ...
   - Hit test via per-bone Rapier sphere colliders. `CharacterRenderer::build_character_bone_data` walks `model.humanoid` once at model load and emits `(local_position, radius)` for every humanoid bone. The radius is `BONE_RADIUS_RATIO × (distance to the furthest child bone in the hierarchy)`, with `DEFAULT_LEAF_BONE_RADIUS = 0.05` for bones that have no descendants and a `MIN_BONE_RADIUS = 0.025` filter that drops finger segments. `PhysicsWorld::add_character_bone_colliders` registers them as a single kinematic body parked at the world origin; the per-frame `update_character_bone_positions` walks `current_bone_local_positions` (which reads `model.nodes.world_positions` updated by `update_skin_palette`) and pushes each new position into the matching collider via `set_translation_wrt_parent`. The colliders therefore follow the animation without any per-frame GPU readback, at the cost of being slightly larger than the rendered mesh.
...
- `apps/ene-desktop-v2/src/physics.rs` — **shipped** (PR5.2): `add_character_bone_colliders` + `update_character_bone_positions` + `cast_ray` (global) + `remove_character_colliders`. The PR5.1 trimesh code (`add_character_trimesh`, `cast_ray_at_character`, `update_character_transform`) and the old AABB code (`update_character_collider`) are gone.
- `apps/ene-desktop-v2/src/character/mod.rs` — **shipped** (PR5.2): `build_character_bone_data` (rest-pose `(local_position, radius)` per humanoid bone) and `current_bone_local_positions` (live positions after `update_motion`). The PR5.1 `read_trimesh_data` and the `TrimeshBuildData` type alias are removed.
- `apps/ene-desktop-v2/src/runtime.rs` — **shipped** (PR5.2): `device_state: device_query::DeviceState` field, the `update_char_window_cursor_and_hittest` helper, `set_cursor_hittest` toggle, per-frame `update_character_bone_positions` (replaces the per-frame trimesh transform update), and a `cast_ray` + entity-id check for the press predicate. `pace_frame_rate` pending.
...
- **Deferred (PR4.5+):** normal / tangent morph displacements, multi-target blend-shape graphs (e.g. `blink_l + blink_r → blink`), look-at `expression` mode (writes `lookLeft/Right/Up/Down` directly into the weights map).
