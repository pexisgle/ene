...
|---|---|---|
| `ene-tool-proto` | `crates/ene-tool-proto` | **Wire Protocol Layer**: Serialized IPC messages, socket transport, `ToolProvider` trait, and sandbox configuration data structures. |
| `ene-tool-host` | `crates/ene-tool-host` | **Orchestration Layer**: Subprocess life cycles, authorization environment variable provisioning, and MCP (Model Context Protocol) registry mapping. |
