...
| `screen_summary` | Short-lived **text** summary from the desktop summarizer | Desktop captures a **fresh** frame on every observe tick (no cross-tick cache): active window when another app is focused, **full display** when Ene is focused (so background context behind the companion window is visible). On Wayland this uses xdg-desktop-portal `ActiveWindow` vs `Screen` targets; on X11 it uses active-window vs primary-monitor capture via xcap. Summarizes via the **local** proactive GGUF + `mmproj` (Gemma 4), and forwards **text only** into mind. The same frame is kept **ephemerally in the runtime actor** (JPEG data URI, never mind/store). When generation uses a cloud task with `supports_vision: true`, that frame is attached to the proactive generation turn. When this source is enabled, each observe cycle drives the decision LLM immediately after vision |
