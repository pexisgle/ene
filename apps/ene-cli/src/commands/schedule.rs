...
async fn add(
    ...
    while let Some(flag) = parts.next() {
        match flag {
            "--kind" => {
                kind = Some(match value("--kind", &mut parts)?.as_str() {
