//! One-shot startup-error resource consumed by `Runtime::resumed`.
//!
//! Written by `AppState::with_channel` when `AiBridge::try_new` fails
//! and read/consumed in `Runtime::resumed` to surface a fatal dialog.
//! A bevy `Resource` bridges the gap between `with_channel` (where the
//! UI entity does not yet exist) and `Runtime::resumed` (where it does).
