...
impl AppState {
    ...
    pub fn with_channel(
        ...
        let kill_on_drop = crate::core_session::kill_on_drop(settings.core_lifetime());
        let (ai, runtime_startup_error) = match CoreSession::try_new(
            tx.clone(),
            bootstrap_handle,
            kill_on_drop,
            audio_tx.clone(),
            ...
            Err(error) => {
                tracing::error!(
                    ...
                    error = %error,
                    "failed to spawn or attach to ene-core"
                );
    ...
    pub fn reconcile_chat_history_if_needed(&mut self) {
        ...
        let Ok(history) = ai.history_blocking() else {
            return;
        };
        if let Some(mut chat) = self
            ...
            .get_mut::<crate::component::chat::ChatStateComponent>(entity)
        {
            chat.0.sync_from_history(&history);
        }
    }
    ...
    pub fn reconnect_runtime(
        ...
        let kill_on_drop = crate::core_session::kill_on_drop(self.settings.core_lifetime());
        #[cfg(feature = "voice")]
        let audio_tx = self.audio_tx.clone();
        #[cfg(not(feature = "voice"))]
        let audio_tx: Option<crate::audio::AudioChunkSender> = None;
        match CoreSession::try_new(event_tx.clone(), bootstrap_handle, kill_on_drop, audio_tx) {
            Ok(bridge) => {
