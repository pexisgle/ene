...
use crate::detail_ui::DetailEguiWindow;
...
pub struct Runtime {
    ...
    detail_egui_window: Option<DetailEguiWindow>,
...
impl Runtime {
    ...
    fn create_detail_window(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let window = match event_loop.create_window(attrs) {
            ...
            Err(error) => {
                tracing::error!("Failed to create detail window: {error}");
                return;
            }
        };
        apply_window_theme(&window, self.state.settings.theme());
        let size = window.inner_size();
        match DetailEguiWindow::new(
            window,
            &self.state.gpu.instance,
            &self.state.gpu.adapter,
            &self.state.gpu.device,
            ...
            Ok(detail) => {
                let visible = self.state.ui_bevy_state().0.detail_window_visible;
                detail.window.set_visible(visible);
                if visible {
                    detail.window.request_redraw();
                }
                self.detail_egui_window = Some(detail);
            }
            Err(error) => tracing::error!("Failed to create DetailEguiWindow: {error}"),
        }
    }

    fn show_detail_window(&mut self) {
        if !self.state.ui_bevy_state().0.detail_window_visible {
            self.state.ui_bevy_state_mut().0.detail_window_visible = true;
        }
    }

    fn hide_detail_window(&mut self) {
        if self.state.ui_bevy_state().0.detail_window_visible {
            self.state.ui_bevy_state_mut().0.detail_window_visible = false;
            self.detail_egui_window = None;
        }
    }

    fn toggle_detail_window(&mut self) {
        let visible = self.state.ui_bevy_state().0.detail_window_visible;
        if visible {
...
impl ApplicationHandler for Runtime {
    ...
    fn window_event(
        ...
        if is_ui {
            ...
            self.handle_detail_window_event(event_loop, event);
    ...
    fn about_to_wait(&mut self, event_loop: &ActiveEventLoop) {
            ...
            .detail_requested
        {
            self.state
                .app
                .world_mut()
                .resource_mut::<crate::resource::event_channels::EventChannels>()
...
impl Runtime {
    fn sync_theme(&mut self) {
        ...
        for window in [
            ...
            self.detail_egui_window.as_ref().map(|w| w.window.as_ref()),
    ...
    fn sync_runtime_to_bevy(&mut self) {
        ...
        let expression_hold_secs = 1.5_f64;
    ...
    fn handle_runtime_reconnect(&mut self) {
        ...
        match self.state.reconnect_runtime(&self.event_tx, &handle) {
            Ok(()) => {
                tracing::info!(component = "CoreSession", "Runtime reconnected");
            }
            Err(message) => {
                tracing::warn!(
    ...
    fn render_per_frame(&mut self, event_loop: &ActiveEventLoop) {
        ...
        self.render_detail_frame(event_loop);
    ...
    fn render_chat_frame(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let assets_dir = self.state.settings.assets_dir.clone();
        let card_path = self
            .state
            .settings
    ...
    fn render_detail_frame(&mut self, event_loop: &ActiveEventLoop) {
        let visible = self.state.ui_bevy_state().0.detail_window_visible;
        if visible && self.detail_egui_window.is_none() {
            self.create_detail_window(event_loop);
        }
        let Some(window) = self.detail_egui_window.as_mut() else {
            return;
        };
        if window.window.is_visible() != Some(visible) {
        ...
        match window.render_frame(
            ...
            Err(error) => match error {
                AcquireError::Reconfigure => {
                    ...
                    window.reconfigure(&self.state.gpu.device, size);
                }
                AcquireError::Timeout => {}
                AcquireError::Fatal => event_loop.exit(),
            },
...
impl Runtime {
        ...
        match event {
            ...
            WindowEvent::KeyboardInput { .. } => {
                if let Some(named) = key_pressed(&event) {
                    ...
                    } else if key_code_pressed(&event) == Some(winit::keyboard::KeyCode::F4) {
                        self.toggle_detail_window();
    ...
    fn handle_detail_window_event(&mut self, _event_loop: &ActiveEventLoop, event: WindowEvent) {
        let Some(window) = self.detail_egui_window.as_mut() else {
            return;
        };
        ...
        let response = window.egui_state.on_window_event(&winit_window, &event);
        if response.repaint {
            window.window.request_redraw();
        }
        if response.consumed {
        ...
        match event {
            ...
            WindowEvent::KeyboardInput { .. } => {
                if key_pressed(&event) == Some(NamedKey::Escape)
                    || key_code_pressed(&event) == Some(winit::keyboard::KeyCode::F4)
                {
                    self.hide_detail_window();
                }
            }
