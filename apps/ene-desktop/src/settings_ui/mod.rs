...
fn render_settings_window(
    egui_ctx: Option<Single<&mut EguiContext, Without<PrimaryEguiContext>>>,
    mut settings: ResMut<CharacterSettings>,
    animation_control: ResMut<CharacterAnimationControl>,
    ai_request_writer: MessageWriter<EneRequestEvent>,
    mut input_state: ResMut<SettingsInputState>,
    emotion_queue: ResMut<EmotionQueue>,
    time: Res<Time>,
    ene: Res<EneResource>,
) {
    if !settings.ui.settings_window_visible {
...
fn apply_ai_stream_events(
    ...
    for event in stream_events.read() {
        match event {
            ...
            EneStreamEvent::UserInputRequired { request_id, prompt } => {
                settings.ui.pending_user_input = Some(crate::app_config::PendingUserInput {
                    request_id: request_id.clone(),
                    prompt: prompt.clone(),
                });
