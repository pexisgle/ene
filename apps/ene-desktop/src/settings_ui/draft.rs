...
//! Applying a draft persists atomically, pushes the config to the runtime
//! actor through [`ene_runtime::SettingsApplyRequest`], and reports the
//! actual impact back to the UI.
...
/// not). Mirrors the host-side list in `ene-plugin-host`.
...
/// results. Mirrors [`ene_runtime::SettingsImpact`] with an explicit
/// `Immediate` case for display.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
...
pub(crate) fn config_with_real_secrets() -> EneConfig {
    ...
    def.api_key.inline = "sk-stored-inline".to_string();
    drop(config.set_section(&ai));

    let mut plugins = ene_plugin_host::PluginConfig::default();
    let entry = plugins.list.entry("demo".to_string()).or_default();
    entry
        .credentials
        .insert("cred-key".to_string(), "cred-stored-value".to_string());
    entry.config = serde_json::json!({
        "token": "config-stored-token",
        "voice": "af_heart"
    });
    plugins.mcp_servers.push(ene_plugin_host::McpServerConfig {
        name: "m".to_string(),
        enabled: true,
        transport: ene_plugin_host::McpTransport::Http {
            url: "http://example.test".to_string(),
            auth_header: Some("Bearer stored-auth".to_string()),
        },
...
mod tests {
    ...
    #[test]
    fn set_section_tracks_dirty_paths_and_revision() {
        let mut d = draft();
        let mut section = ene_mind::MindConfig::default();
        section.session.session_timeout_minutes = 30;
        d.set_section(&section);
        assert!(d.is_dirty());
        assert!(d.dirty_paths().contains("mind"));
        assert_eq!(d.revision(), 1);
        assert_eq!(
            d.section::<ene_mind::MindConfig>()
                .session
    ...
    fn draft_never_holds_stored_secret_values() {
        ...
        // The `source` field stays editable next to the masked `inline`.
        let ai = draft.section::<ene_ai::AiConfig>();
        assert_eq!(ai.providers["openai"].api_key.source, "inline");
        assert_eq!(ai.providers["openai"].api_key.inline, SECRET_PLACEHOLDER);
    }
}
