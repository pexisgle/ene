...
#![expect(
    dead_code,
    reason = "settings chrome keeps extra draft fields and helpers until every inner page is wired to core"
)]
...
ene_config::define_config!(
    ...
    pub struct DesktopSection {
        ...
        /// Whether the desktop process should stop `ene-core` when it exits.
        #[serde(default)]
        pub core_lifetime: CoreLifetime,
    }
);
...
#[schemars(crate = "::ene_config::schemars")]
pub enum CoreLifetime {
    /// Stop the spawned core when the desktop process exits.
    #[serde(rename = "app")]
    #[default]
    App,
    /// Leave a spawned core running after desktop exits.
...
pub struct UiState {
    ...
    /// Separate detail window (F4 / tray): inner log, thinking, tools.
    pub detail_window_visible: bool,
    ...
    pub memory_ledger_rows: Vec<MemoryJournalRow>,
...
impl CharacterSettings {
    ...
    pub fn core_lifetime(&self) -> CoreLifetime {
        self.config_section::<DesktopSection>().core_lifetime
    }

    pub fn set_core_lifetime(&self, core_lifetime: CoreLifetime) {
        self.store.read().with_config_mut(|c| {
            if let Ok(mut d) = c.get_section::<DesktopSection>() {
                d.core_lifetime = core_lifetime;
                drop(c.set_section(&d));
            }
        });
    }
    ...
    pub fn sync_classifier_language_from_ui(&self) {
        ...
        self.with_config_mut(|c| {
            drop(c.set_path("mind.language", &format!("\"{lang}\"")));
        });
    }
}
