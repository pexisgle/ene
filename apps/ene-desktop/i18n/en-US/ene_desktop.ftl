...
proactive-source-memory-hint = Feeds the user's stored preferences and profile (e.g. "don't talk while I work", "quiet at night") into decisions — these are injected directly, never filtered by recall score — and recalls memories matching the topic before speaking. Turn off to keep decisions free of memory recall.
