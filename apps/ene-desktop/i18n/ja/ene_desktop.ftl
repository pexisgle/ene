...
proactive-source-memory-hint = ユーザーの保存された好み・プロフィール（例: 「作業中は話しかけないで」「夜は静かに」）を判定に反映します — これらの記憶は想起スコアで絞らず直接注入されるため、抑制条件が落ちません。発話前には話題に関連する記憶を想起します。判定から記憶想起を除外したい場合はオフにしてください。
