...
fn connection_table_pins_the_first_incarnation() {
    ...
    assert!(
        matches!(other, LiveDecision::Invalid),
...
fn connection_table_suppresses_redelivered_message_ids() {
    ...
    assert!(
        matches!(table.live_for(&id, &frame), LiveDecision::Duplicate),
...
fn duplicate_cache_is_bounded_and_rolls_off() {
    ...
    assert!(
        matches!(table.live_for(&id, &first), LiveDecision::Duplicate),
...
#[tokio::test]
async fn stale_regular_file_is_cleared_by_bind_singleton() {
    ...
    // A stale regular file blocks a bare bind; the production path must
    // probe it as dead, unlink it, and rebind.
    let bare = tokio::net::UnixListener::bind(&socket);
    assert!(
        bare.is_err(),
        "a stale regular file must block a fresh bind: {bare:?}"
    );
    // The production path owns stale-path clearing.
    assert!(
        bind_singleton(&socket).await.is_ok(),
        "bind_singleton must clear a stale regular file and rebind"
    );
}
...
async fn redelivery_keeps_the_connection_serving() {
    ...
    fn framed(
        ...
        ene_plugin_ipc::encode_frame(&WireFrame { envelope, payload })
...
/// IPC §7.2 / V-11: a capability advertisement with no shared major answers
/// the typed terminal `IncompatibleProtocol`, and the connection closes after
/// it (the frame is written, then the socket ends).
#[tokio::test]
async fn incompatible_protocol_is_written_then_the_connection_closes() {
    use std::time::Duration;
    ...
    let (handle, _dir) = memory_handle_with("incompatible-close", |_| {})
        .await
        .unwrap();
    let table = Arc::new(ConnectionTable::new());
    let id = table.note_accept();
    ...
    let mut envelope = envelope(incarnation(7, 8));
    envelope.message_id = WireMessageId(uuid::Uuid::new_v4());
    let capability = ene_plugin_ipc::encode_frame(&WireFrame {
        envelope,
        payload: WirePayload::CapabilityAdvertise(CapabilityAdvertise {
            supported_protocol: vec![ProtocolVersion { major: 9, minor: 0 }],
            platform: String::from("test"),
        }),
    })
    .expect("test frame must encode");
    assert!(
        client.write_all(&capability).await.is_ok(),
        "the capability must send"
    );
    let mut prefix = [0_u8; 4];
    let read = tokio::time::timeout(Duration::from_secs(5), client.read_exact(&mut prefix)).await;
    assert!(
        read.is_ok_and(|result| result.is_ok()),
        "the typed refusal must arrive"
    );
    let claimed = u32::from_be_bytes(prefix) as usize;
    let mut body = vec![0_u8; claimed];
    assert!(
        client.read_exact(&mut body).await.is_ok(),
        "the refusal body must arrive"
    );
    let mut bytes = prefix.to_vec();
    bytes.extend_from_slice(&body);
    let (frame, _) = ene_plugin_ipc::decode_frame(&bytes).expect("the refusal must decode");
    assert!(
        matches!(frame.payload, WirePayload::IncompatibleProtocol(_)),
        "a major mismatch answers the typed refusal, got {:?}",
        frame.payload
    );
    let mut byte = [0_u8; 1];
    let eof = tokio::time::timeout(Duration::from_secs(5), client.read(&mut byte)).await;
    assert!(
        matches!(eof, Ok(Ok(0))),
        "the connection must close after the typed refusal, got {eof:?}"
    );
    worker.abort();
}
...
async fn receipt_expiry_advances_after_a_push_write_failure() {
    ...
    // A new fact commits while writes fail: the unsolicited push carries its
    // item (the durable presentation-start commits and the in-memory receipt
    // is installed), then its write fails and disables further pushes.
    ...
    // The push carried its item (the durable presentation-start committed
    // and the in-memory receipt was installed) before the failed write;
    // nothing else removes the receipt while pushes are blocked.
    assert!(
