...
use ene_api::v1::handshake::{
    AuthProof, AuthResult, CapabilityAdvertise, NegotiatedConnection, PairingProvision,
...
use ene_api::v1::refs::{
    ...
    ManagementTargetWire, RequestWireId, WireMessageType,
};
...
/// The Client-side pairing proof, minted locally: the production minting
/// helper is crate-private to `ene-client`, and this module verifies the
/// Host-side acceptance of exactly this HMAC-SHA256 construction.
fn pairing_proof_hex(secret: &str, nonce: &str) -> String {
...
/// A pairing request that reuses `request_id`, as a retry of one logical
/// request does; its `message_id` stays fresh per transport attempt.
fn pairing_retry(descriptor: &str, request_id: RequestWireId) -> super::WireFrame {
    let mut frame = pairing_frame(descriptor);
    frame.envelope.correlation.request_id = Some(request_id);
    frame
}
...
async fn make_present(handle: &HostHandle, device_wire: &str) {
    let _ = attach_present(handle, device_wire).await;
}
...
fn device_mapping_is_not_issuance() {
    ...
    assert_eq!(
        mapped,
        ClientId::from_raw(ene_primitive::RawId::from_uuid(uuid::Uuid::new_v5(
            &uuid::Uuid::NAMESPACE_URL,
            b"laptop",
        ))),
        "the mapping is a stable v5 derivation, not issuance"
    );
}

#[tokio::test]
async fn pairing_denies_an_unauthorized_peer() {
    let (handle, _dir) = memory_handle("pair-deny").await.unwrap();
...
async fn pairing_denies_a_blank_descriptor() {
    let (handle, _dir) = memory_handle("pair-blank").await.unwrap();
    let transport = fake_transport();
    for descriptor in ["", "   "] {
        let responses = handle
        ...
        let first = responses.first().expect("the denial answers once");
        assert!(
            matches!(
                &first.payload,
                WirePayload::PairingResult(PairingResult::Denied { .. })
...
async fn pairing_pends_then_pairs_after_owner_approval() {
    let (handle, _dir) = memory_handle("pair-flow").await.unwrap();
    let transport = fake_transport();
    let (table, id) = fresh_conn();
...
#[tokio::test]
async fn a_pairing_retry_with_the_same_request_id_answers_the_live_pending() {
    let (handle, _dir) = memory_handle("pair-resend").await.unwrap();
    let transport = fake_transport();
    let (table, id) = fresh_conn();
    let mut provisions = register_pairing(&handle, &id);
    let request_id = RequestWireId(uuid::Uuid::new_v4());
    let first = handle
        .handle_frame(
            pairing_retry("laptop", request_id),
            live_of(&table, &id),
            &transport,
        )
        .await;
    let first_pending = pending_id_of(first.first().expect("the first request answers"));
    // A transport retry keeps the logical request id while minting a fresh
    // message id; the same connection and body must still get the live pending
    // instead of minting a second one (IPC §9.3).
    let second = handle
        .handle_frame(
            pairing_retry("laptop", request_id),
            live_of(&table, &id),
        ...
        .await;
    let second_pending = pending_id_of(second.first().expect("the resend answers"));
    assert_eq!(
        second_pending, first_pending,
        "a same-request retry returns the live pending"
    );
    assert_eq!(table.phase_of(&id), Some(ConnectionPhase::Accepted));
    let record = handle
        .approve_device(&first_pending)
        .await
        .unwrap()
        .expect("the first pending stays approveable");
    let provision = receive_provision_and_pair(&table, &id, &mut provisions).await;
    assert_eq!(
        provision.device_id.0.as_hyphenated().to_string(),
        record.wire
...
#[tokio::test]
async fn a_pairing_request_id_reused_with_a_different_body_is_refused() {
    let (handle, _dir) = memory_handle("pair-conflict").await.unwrap();
    let transport = fake_transport();
    let (table, id) = fresh_conn();
    let mut provisions = register_pairing(&handle, &id);
    let request_id = RequestWireId(uuid::Uuid::new_v4());
    ...
    let conflict = handle
        .handle_frame(
            pairing_retry("server", request_id),
            live_of(&table, &id),
            &transport,
        )
        .await;
    assert_eq!(conflict.len(), 1, "the conflict answers exactly one frame");
    let first = conflict.first().expect("the conflict answers once");
    assert!(
        matches!(
            &first.payload,
            WirePayload::PairingResult(PairingResult::Denied { .. })
        ),
        "reusing a request id with a different body is denied, got {:?}",
        first.payload
    );
    let pendings = handle.pending_devices().await.unwrap();
    assert_eq!(pendings.len(), 1, "the conflict mints no second pending");
    assert!(
        pendings
            .iter()
            .any(|entry| entry.pending_id == first_pending && entry.descriptor == "laptop"),
        "the first pending is untouched by the conflict"
    );
    let record = handle
        .approve_device(&first_pending)
        .await
...
async fn identical_descriptors_on_distinct_connections_get_distinct_pendings() {
    let (handle, _dir) = memory_handle("pair-distinct").await.unwrap();
    let transport = fake_transport();
    let (first_table, first_id) = fresh_conn();
    let (second_table, second_id) = fresh_conn();
    let mut first_provisions = register_pairing(&handle, &first_id);
    let _second_provisions = register_pairing(&handle, &second_id);
    let first = handle
        .handle_frame(
    ...
    let second = handle
        ...
        .await;
    let first_pending = pending_id_of(first.first().expect("the first request answers"));
    let second_pending = pending_id_of(second.first().expect("the second request answers"));
    assert_ne!(second_pending, first_pending);
    let approved = handle.approve_device(&first_pending).await.unwrap();
    assert!(approved.is_some());
    let provision =
        receive_provision_and_pair(&first_table, &first_id, &mut first_provisions).await;
    assert_eq!(
        approved.unwrap().wire,
        provision.device_id.0.as_hyphenated().to_string()
    );
    assert!(
        handle
...
async fn re_pairing_on_a_paired_connection_is_invalid_phase() {
    let (handle, _dir) = memory_handle("pair-immutable").await.unwrap();
    let transport = fake_transport();
    let (table, id) = fresh_conn();
...
async fn unpaired_domain_frames_close_with_an_unpaired_notice() {
    let (handle, _dir) = memory_handle("gate-drop").await.unwrap();
    let transport = fake_transport();
    let responses = handle
...
async fn unknown_connection_closes_even_with_a_device() {
    let (handle, _dir) = memory_handle("gate-unknown").await.unwrap();
    let transport = fake_transport();
    let input = LiveInput {
...
async fn unsolicited_challenge_and_result_answer_nothing() {
    let (handle, _dir) = memory_handle("auth-deferred").await.unwrap();
    let transport = fake_transport();
...
async fn challenge_proof_accepts_and_binds_the_connection() {
    let (handle, _dir) = memory_handle("auth-flow").await.unwrap();
    let transport = fake_transport();
    let (table, id) = fresh_conn();
    let mut provisions = register_pairing(&handle, &id);
    ...
    .await;
    let Some(WirePayload::AuthChallenge(_)) = second_challenged.get(1).map(|f| &f.payload) else {
...
async fn pre_accept_denials_and_closes_hide_the_connection_id() {
    let (handle, _dir) = memory_handle("auth-hidden").await.unwrap();
    let transport = fake_transport();
    let (pairing_table, pairing_id) = fresh_conn();
    let _provisions = register_pairing(&handle, &pairing_id);
    let denied = handle
        .handle_frame(
            pairing_frame("laptop"),
            live_of(&pairing_table, &pairing_id),
            &transport,
        )
        .await;
    ...
    assert!(
        mismatch
            .first()
            .is_some_and(|first| matches!(&first.payload, WirePayload::IncompatibleProtocol(_))),
        "a major mismatch answers the typed terminal refusal, got {mismatch:?}"
    );
    let dropped = handle
        .handle_frame(submit_frame(), unpaired_input(), &transport)
        .await;
...
async fn unauthed_domain_frame_closes_even_without_a_connection_id() {
    let (handle, _dir) = memory_handle("gate-bypass").await.unwrap();
    let transport = fake_transport();
...
async fn superseded_connection_replay_is_stale_not_a_close() {
    let (handle, _dir) = memory_handle("gate-superseded").await.unwrap();
    let transport = fake_transport();
    ...
    authenticate(&table, &first, "laptop");
    let second = table.note_accept();
    authenticate(&table, &second, "laptop");
...
async fn approved_secret_verifies_from_a_fresh_handle_on_the_same_dir() {
    ...
    let (first_table, first_id) = fresh_conn();
    let mut provisions = register_pairing(&first, &first_id);
    ...
    let record = first
        ...
        .unwrap()
        .expect("owner approval must pair");
    let provision = receive_provision_and_pair(&first_table, &first_id, &mut provisions).await;
    let secret = provision.pairing_secret;
    assert!(
    ...
    let proof = pairing_proof_hex(secret.expose_secret(), &challenge.nonce);
...
async fn proof_without_challenge_is_invalid_phase() {
    let (handle, _dir) = memory_handle("auth-nochallenge").await.unwrap();
    let transport = fake_transport();
...
async fn re_advertising_keeps_the_nonce_and_is_invalid_phase() {
    let (handle, _dir) = memory_handle("caps-repeat").await.unwrap();
    let transport = fake_transport();
...
async fn negotiated_version_is_fixed_per_connection() {
    let (handle, _dir) = memory_handle("version-fixed").await.unwrap();
    let transport = fake_transport();
    ...
    assert!(
        pre.first()
            .is_some_and(|first| !matches!(&first.payload, WirePayload::Reject(_))),
        "a same-major newer-minor pre-negotiation frame is read under v1.0, got {pre:?}"
    );
    assert_eq!(
        pre.first().map(|first| first.envelope.sender.connection_id),
        Some(None),
        "a pre-auth answer still hides the connection"
    );
}
...
#[tokio::test]
async fn capability_major_mismatch_answers_typed_incompatible_protocol() {
    let (handle, _dir) = memory_handle("caps-mismatch").await.unwrap();
    let (table, id) = fresh_conn();
    let frame = advertise_frame(None, ProtocolVersion { major: 9, minor: 3 });
    let transport = fake_transport();
    let responses = dispatch(&handle, &table, &id, frame, &transport).await;
    assert_eq!(responses.len(), 1, "mismatch answers exactly one frame");
    let first = responses.first().unwrap();
    let WirePayload::IncompatibleProtocol(notice) = &first.payload else {
        panic!(
            "a major mismatch must answer IncompatibleProtocol, got {:?}",
            first.payload
        );
    };
    assert_eq!(
        notice.host_max,
        ProtocolVersion::V1,
        "the refusal names the host maximum"
    );
    assert_eq!(
        notice.client_max,
        ProtocolVersion { major: 9, minor: 3 },
        "the refusal names the client's highest advertised version"
    );
    assert!(
        !notice.hint.is_empty(),
        "the refusal carries the design's upgrade hint"
    );
    assert_eq!(
        table.phase_of(&id),
        Some(ConnectionPhase::Accepted),
...
/// IPC §7.2: the same-major newer-minor case is processed within the older
/// (host) range instead of being refused.
#[tokio::test]
async fn capability_same_major_newer_minor_negotiates_the_older_range() {
    let (handle, _dir) = memory_handle("caps-newer-minor").await.unwrap();
    let device = DeviceWireId(uuid::Uuid::from_u128(31));
    let (table, id) = fresh_conn();
    assert!(table.note_paired(&id, &device.0.as_hyphenated().to_string()));
    let frame = advertise_frame(Some(device.0), ProtocolVersion { major: 1, minor: 1 });
    let transport = fake_transport();
    let responses = dispatch(&handle, &table, &id, frame, &transport).await;
    assert_eq!(
        responses.len(),
        2,
        "a same-major newer minor still negotiates terms plus the challenge"
    );
    let first = responses.first().unwrap();
    let WirePayload::NegotiatedConnection(terms) = &first.payload else {
        panic!(
            "a same-major newer minor must negotiate, got {:?}",
            first.payload
        );
    };
    assert_eq!(
        terms.version,
        ProtocolVersion::V1,
        "the host's older understood range governs the connection"
    );
}

/// IPC §7.2: an envelope the Host's major cannot interpret is refused with the
/// typed terminal DTO before any negotiation, naming the frame's own version
/// as the client maximum.
#[tokio::test]
async fn pre_negotiation_major_mismatch_answers_typed_incompatible_protocol() {
    let (handle, _dir) = memory_handle("pre-negotiation-major").await.unwrap();
    let transport = fake_transport();
    let mut early = pairing_frame("laptop");
    early.envelope.protocol = ProtocolVersion { major: 9, minor: 0 };
    let answers = handle
        .handle_frame(early, unpaired_input(), &transport)
        .await;
    ...
    let WirePayload::IncompatibleProtocol(notice) = &first.payload else {
        panic!(
            "an uninterpretable major must answer IncompatibleProtocol, got {:?}",
            first.payload
        );
    };
    ...
    assert_eq!(
        ...
        "the refusal names the envelope's own version"
    );
    assert_eq!(
        first.envelope.sender.connection_id, None,
        "a pre-accept refusal still hides the connection"
    );
}
...
async fn capability_match_negotiates_and_challenges_without_attaching() {
    ...
    let (handle, _dir) = memory_handle("caps-ok").await.unwrap();
...
async fn disconnect_without_presence_is_a_no_op() {
    ...
    let (handle, _dir) = memory_handle("disc-noop").await.unwrap();
...
async fn superseded_connection_replays_are_stale_and_have_no_effect() {
    let (handle, _dir) = memory_handle("stale-replay").await.unwrap();
    let transport = fake_transport();
...
async fn superseded_capability_replay_is_stale_and_never_moves_device() {
    let (handle, _dir) = memory_handle("stale-unknown").await.unwrap();
    let transport = fake_transport();
    let device = uuid::Uuid::from_u128(7);
    let device_wire = device.as_hyphenated().to_string();
    let table = Arc::new(ConnectionTable::new());
    let first = table.note_accept();
    authenticate(&table, &first, &device_wire);
    let second = table.note_accept();
    authenticate(&table, &second, &device_wire);
    let replay = dispatch(
        &handle,
        &table,
        &first,
...
async fn current_close_falls_back_even_with_a_lingering_superseded_socket() {
    let (handle, _dir) = memory_handle("close-linger").await.unwrap();
    let table = Arc::new(ConnectionTable::new());
    let first = table.note_accept();
    authenticate(&table, &first, "laptop");
    let second = table.note_accept();
...
async fn close_before_auth_falls_back_and_the_install_does_not_revive() {
    let (handle, _dir) = memory_handle("close-then-auth").await.unwrap();
    let table = Arc::new(ConnectionTable::new());
    let first = table.note_accept();
    authenticate(&table, &first, "laptop");
    make_present(&handle, "laptop").await;
...
async fn close_racing_a_new_auth_never_clears_the_new_current() {
    let (handle, _dir) = memory_handle("auth-then-close").await.unwrap();
...
async fn disconnect_falls_back_to_a_permitted_same_machine_client() {
    let (handle, _dir) = memory_handle("disc-fallback").await.unwrap();
...
async fn disconnect_fallback_ignores_non_permitted_or_remote_candidates() {
    let (handle, _dir) = memory_handle("disc-fallback-skip").await.unwrap();
    let (companion, _generation) = attach_present(&handle, "client-a").await;
...
async fn disconnect_fallback_never_crowns_a_target_that_lost_currentness() {
    let (handle, _dir) = memory_handle("disc-fallback-lost").await.unwrap();
    let (companion, _generation) = attach_present(&handle, "client-a").await;
...
async fn plain_state_open_does_not_normalize_presence() {
    ...
    let (handle, dir) = memory_handle("disc-no-normalize").await.unwrap();
...
async fn startup_normalization_refusal_fails_the_serving_boundary() {
    ...
    let (handle, dir) = memory_handle("disc-normalize-fail").await.unwrap();
    let (companion, _generation) = attach_present(&handle, "client-a").await;
...
async fn cancel_task_admission_wires_the_cooperative_stop() {
    ...
    let running_delegation = DelegationId::generate();
    assert!(
        handle
            .task_executions
            .reserve(running_delegation, task.task)
    );
    let crate::task_run::TakeReservation::Admitted(registration) = handle
        .task_executions
        .take_reservation(running_delegation, task.task)
    else {
        panic!("the running execution registers");
    };
    assert_eq!(
        handle
    ...
    assert!(
        handle
            .task_executions
            .reserve(completed_delegation, completed.task)
    );
    let crate::task_run::TakeReservation::Admitted(completed_registration) = handle
        .task_executions
        .take_reservation(completed_delegation, completed.task)
    else {
        panic!("the running execution registers");
    };
    assert_eq!(
...
async fn management_cancel_reaches_the_cancel_admission_and_replays() {
    ...
    let live = live_input("management-cancel");
    let transport = fake_transport();
...
async fn stage5_frames_distinguish_superseded_from_unauthenticated() {
    let (handle, _dir) = memory_handle("stage5-gate").await.unwrap();
    let transport = fake_transport();
    ...
    authenticate(&table, &c1, &device_wire);
    let c2 = table.note_accept();
    authenticate(&table, &c2, &device_wire);
    assert_eq!(table.phase_of(&c1), Some(ConnectionPhase::Superseded));
...
async fn supersession_drops_the_replaced_connections_presentation_state() {
    ...
    let (handle, _dir) = memory_handle("stale-purge").await.unwrap();
    let transport = fake_transport();
...
async fn targeted_deletion_awaits_host_confirmation_then_starts_once() {
    ...
    let live = live_input("management-deletion");
    let transport = fake_transport();
...
async fn a_confirmed_request_without_an_operation_is_admitted_and_driven_by_the_intent_inlet() {
    ...
    let live = live_input("management-deletion-confirm-window");
    let transport = fake_transport();
...
async fn targeted_deletion_is_unreachable_from_untrusted_client_wire_alone() {
    ...
    let live = live_input("management-deletion-untrusted");
    let transport = fake_transport();
...
async fn targeted_deletion_stale_and_foreign_targets_leave_no_trace() {
    ...
    let live = live_input("management-deletion-stale");
    let transport = fake_transport();
...
async fn deletion_status_pages_are_bounded_and_reject_malformed_queries() {
    use ene_preservation::{
        ...
        StageTargetedDeletionRequestOutcome, TargetedDeletionRequest, TargetedDeletionTarget,
    };
    use ene_primitive::{RawId, WallClockWithTz};
    ...
    let live = live_input("management-deletion-status");
    let transport = fake_transport();
    ...
    // completed operation is terminal, so this fixture stages and confirms one
    // more operation through the production request path and holds it.
        ...
        .unwrap()
    {
        ...
        other => panic!("the held fixture must stage: {other:?}"),
    };
...
async fn startup_reconciliation_settles_orphaned_usage_reservations() {
    ...
    use ene_permission::{
        ...
        UsageCapConsumption, UsageCapRepository as _, UsageCapScope, UsageCapStatusQuery,
    ...
    let system = before
        .iter()
        .find(|s| {
            *s.cap.scope() == UsageCapScope::System && s.cap.window() == UsageCapWindow::DailyUtc
        })
    ...
    let UsageCapConsumption::Known {
        ...
        panic!("the fixture cap is comparable");
    };
    let reserved_bound = reserved_before.micros();
    handle
    ...
    let UsageCapConsumption::Known {
        ...
        panic!("the settled cap is comparable");
    };
    assert_eq!(
        reserved.micros(),
        0,
        "the terminal reservation is no longer reserved"
    );
    assert_eq!(
        committed_unknown.micros(),
        reserved_bound,
        "the unknown settlement keeps the reserved upper bound"
    );
