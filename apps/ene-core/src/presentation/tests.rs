...
//! test hook. S5-02/07/08/09/10/12.
...
async fn seed_task(handle: &HostHandle) -> TaskRef {
    seed_task_with_assoc(handle).await.0
}
...
impl FrameSink for FullSink {
    fn emit(&mut self, _frame: WireFrame) -> Result<(), crate::serve::FrameDeliveryError> {
        Err(crate::serve::FrameDeliveryError)
    }
}
...
async fn s5_07_old_round_input_and_ack_never_replay_and_auth_restores_nothing() {
    ...
    assert!(
        matches!(same_epoch, UndeliveredAckOutcome::Presented { .. }),
        "same-epoch ACK still applies, got {same_epoch:?}"
    );
...
async fn s5_08_host_crash_represents_under_new_receipts() {
    ...
    assert_eq!(
        ...
        "durable attribution stays Present across a plain reopen (no startup normalization)"
    );
...
/// A cursor-less request is the next logical page (IPC §13.3). Each advance
/// consumes the predecessor wire it continued from, so a Client that walks
/// pages without echoing `next_cursor` cannot grow the connection's cursor
/// map (and the abandoned wires stop being replayable).
#[tokio::test]
async fn cursor_less_paging_keeps_one_connection_cursor() {
    let (handle, _dir) = open_handle("present-cursorless").await;
    let live = live_input(DEVICE_A);
    let fresh = attach(&handle, DEVICE_A).await;
    for index in 0..3 {
        append_reply(
            &handle,
            &format!("cursorless row {index}"),
            fresh.generation,
        )
        .await;
    }
    ...
    loop {
        let page = summary_of(fetch(&handle, &live, None, Some(1), false).await);
        assert_eq!(page.items.len(), 1, "one row per bounded page");
        assert_eq!(
            handle
                .presentation_counts_for_test(&live.connection_id)
                .cursors,
            usize::from(page.next_cursor.is_some()),
            "a cursor-less advance replaces its predecessor wire"
        );
        let outcome = ack(
            &handle,
            &live,
    ...
    assert_eq!(pages, 3, "every row presents exactly once");
    assert_eq!(
        handle
            .presentation_counts_for_test(&live.connection_id)
            .cursors,
        0,
        "a drained pass leaves no continuation cursor"
    );
}
...
async fn s5_10_mid_pass_arrivals_wait_for_the_next_pass() {
    ...
    // Page two is materialized before the arrival commits, fixing its bound.
    let second = summary_of(fetch(&handle2, &live2, Some(cursor2), Some(10), false).await);
    assert_eq!(second.items.len(), 1);
    assert_eq!(second.items[0].excerpt, "race row two");
    let mid2 = attribution_of(&handle2).await;
    append_reply(&handle2, "race arrival", mid2.generation).await;
    // ACK page two so the later head pass is not a duplicate of an
    // outstanding receipt.
    let second_ack = ack(
        &handle2,
        &live2,
    ...
    .await;
    assert!(
        matches!(second_ack, UndeliveredAckOutcome::Presented { .. }),
        "got {second_ack:?}"
    );
    let catchup2 = summary_of(fetch(&handle2, &live2, None, Some(50), false).await);
    assert_eq!(catchup2.items.len(), 1);
    assert_eq!(catchup2.items[0].excerpt, "race arrival");
...
#[tokio::test]
async fn s5_12_reads_are_pure_and_change_nothing() {
    ...
    // Every read surface, repeatedly: reads never consult a provider, so
...
async fn task_queries_validate_limits_cursors_and_refs() {
    ...
    // Seed one action-attempt report row so the report query mints a cursor
    // that can be replayed against the list query below.
    let delegation = DelegationId::generate();
    let outcome = handle
        .store
        .create_delegation(DelegationCreationPremise {
    ...
    let live_b = live_input(DEVICE_A);
    let next = page.next_cursor.expect("a full page mints a cursor");
    ...
    let cursor = report
        .next_cursor
        .expect("a full report page mints a cursor");
...
async fn resume_wire_maps_to_the_owner_gate_with_epoch_replay() {
    ...
    assert!(
        matches!(
            frames.into_iter().next().unwrap().payload,
            WirePayload::CommandReplayReject(_)
        ),
...
async fn concurrent_resume_commands_commit_at_most_once() {
    ...
    assert_eq!(
        resumed, 1,
        "exactly one commit, got {first:?} and {second:?}"
    );
    ...
    let after = handle
        ...
        .expect("exists");
    assert!(
        after.task.reference.revision.as_u64() > revision,
        "a resume must advance the revision"
    );
    let live_c = live_input(DEVICE_A);
    let payload = WirePayload::ResumeTask(ResumeTask {
...
async fn frame_too_large_withholds_without_mutation() {
    ...
    assert_eq!(
        handle
            .presentation_counts_for_test(&live_a.connection_id)
            .receipts,
        0,
        "a withheld frame mints no receipt"
    );
...
async fn repeated_reconnects_do_not_grow_presentation_state() {
    ...
    }
    {
        ...
        assert_eq!(
            state.retired.len(),
            crate::presentation::RETIRED_RECEIPT_CAP,
            "the retired receipt window stays capped"
        );
    }
...
async fn repeated_replacement_keeps_connection_bound_memory_bounded() {
    ...
    assert_eq!(
        fact.state,
        PresenceState::Present,
        "the first attach leaves the device present"
    );
}
...
/// Stages and confirms one deletion operation through the production
/// request/confirmation path.
async fn admit_staged(handle: &HostHandle, text: &str, participants: Vec<ParticipantOwnerRef>) {
    let staged = handle
        .store
    ...
    {
        ConfirmTargetedDeletionOutcome::Started(_) => {}
...
async fn admit_and_drive(handle: &HostHandle, text: &str, participants: Vec<ParticipantOwnerRef>) {
    admit_staged(handle, text, participants).await;
    let outcome = handle
...
async fn admit_condition_only(handle: &HostHandle, text: &str) {
    admit_staged(handle, text, vec![ParticipantOwnerRef::Companion]).await;
}
...
async fn a4_a_covered_item_is_neither_started_nor_acked() {
    ...
    // Presented. The condition is still current, so the compare is held again
    ...
    assert_eq!(
        ...
        "the still-current condition keeps the ACK held"
    );
    let statuses = unpresented_statuses(&handle).await;
    assert_eq!(statuses[0].1, ReportStatus::Pending);
}

/// A presentation start held by an erasure condition writes no status. Once
/// the condition settles, the row is still `Pending`, so the ACK compare
/// answers `StaleSource` and the batch stays re-presentable: `KeptUnknown`,
/// never `AlreadyPresented`.
#[tokio::test]
async fn a4_a_settled_hold_ack_keeps_the_row_re_presentable() {
    let (handle, _dir) = open_handle("present-a4-ack-settled").await;
    let _ = attach(&handle, DEVICE_A).await;
    let live = live_input(DEVICE_A);
    let generation = attribution_of(&handle).await.generation;
    append_reply(&handle, "the target body", generation).await;
    ...
    // Start under the current condition: the covered row is selected with no
    // status write and its excerpt withheld.
    let summary = summary_of(fetch(&handle, &live, None, None, false).await);
    assert_eq!(summary.items.len(), 1);
    assert_eq!(summary.items[0].excerpt, "");

    // Settle the condition by driving the admitted operation to completion.
    for _ in 0..64 {
        let outcome = handle
            .drive_targeted_deletion(TargetedDeletionPass::new(100, 16))
            .await
            .expect("the fan-out must not fail");
        if outcome.demands == 0 && outcome.reconciliation_pages == 0 {
            break;
        }
    }
    assert!(
        !handle.current_coverage().await.covers("the target body"),
        "the settled condition no longer covers the row"
    );
    ...
    assert_eq!(
        ...
        "a settled hold wrote nothing and stays re-presentable"
    );
}
