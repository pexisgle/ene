...
/// The one-delta streaming contract for the single-shot test fakes: push the
/// full text once and map an aborted consumer to `StreamAborted`.
async fn push_completion(
    sink: &mut (dyn ene_inference::DeltaSink + Send),
    text: String,
) -> Result<ene_inference::ProviderResponse, ene_inference::InferenceTechnicalError> {
    match sink.push_delta(&text).await {
        ene_inference::DeltaFlow::Continue => {
            Ok(ene_inference::ProviderResponse { text, usage: None })
        }
...
/// Stages and confirms one Targeted Deletion operation through the production
/// request/confirmation path.
async fn admit_staged(
    handle: &HostHandle,
    text: &str,
    participants: Vec<ene_preservation::ParticipantOwnerRef>,
...
/// Extracts the typed command-id conflict answering `responses`, asserting
/// the post-auth frame carries `connection` (IPC §5, §24).
fn command_conflict_on(
    responses: &[ene_plugin_ipc::WireFrame],
    connection: ConnectionWireId,
) -> Result<CommandReplayRejectWire, String> {
    let (_, answers) = split_presence_fact(responses);
    let Some(first) = answers.first() else {
        return Err(String::from("the submit must answer"));
    };
    ...
    match &first.payload {
        WirePayload::CommandReplayReject(reject) => Ok(reject.clone()),
        other => Err(format!("expected a command replay reject, got {other:?}")),
    }
}
...
impl crate::serve::FrameSink for RefuseFirstSink {
    fn emit(
        ...
        if !self.refused {
            self.refused = true;
            return Err(crate::serve::FrameDeliveryError);
        }
...
#[tokio::test]
async fn full_dialogue_round_streams_and_restores() -> Result<(), String> {
    use ene_companion::CompanionRepository as _;
    use ene_presence::PresenceRepository as _;
    ...
    let WirePayload::RoundIntakeOutcome(RoundIntakeOutcomeWire::AcceptedForRound { round }) =
        ...
        return Err(format!(
            "the winning attach must accept a round, got {:?}",
            accepted.payload
        ));
    };
    ...
    let WirePayload::HistoryResponse(HistoryResponse::Items(items)) = &restored_frame.payload
        ...
        return Err(format!(
            "history must answer items, got {:?}",
            restored_frame.payload
        ));
    };
    ...
    let WirePayload::HistoryResponse(HistoryResponse::Items(second)) = &second_frame.payload else {
        return Err(format!(
            "the replay must still restore items, got {:?}",
    ...
    let scoped = handle
        .handle_frame(
            history_frame_filtered(
                ...
                live.connection_id,
                None,
                ene_api::v1::round::HISTORY_LIMIT_MAX,
...
/// The history limit bound is applied before any store work: with an
/// unreadable store, an over-limit request is still named as an invalid
/// request instead of surfacing the storage failure.
#[tokio::test]
async fn history_limit_over_the_bound_is_refused_before_the_store() -> Result<(), String> {
    let transport = ok_transport();
    let live = live_input("client-a");
    let (handle, dir) = round_test_handle("dlg-history-limit", &live, &transport).await?;
    let corrupted = std::fs::write(dir.path().join("app.db"), vec![0x5A_u8; 8192]);
    assert!(corrupted.is_ok(), "the test must corrupt the store file");
    ...
    let over = handle
        .handle_frame(
            history_frame_filtered(
                ...
                live.connection_id,
                None,
                ene_api::v1::round::HISTORY_LIMIT_MAX + 1,
                None,
            ),
            live,
            &transport,
        ...
        .await;
    assert_eq!(
        over.first().unwrap().payload,
        WirePayload::HistoryResponse(HistoryResponse::InvalidRequest),
        "an over-limit request is refused before the store is read"
    );
    Ok(())
}
...
async fn retry_with_changed_round_intent_conflicts() -> Result<(), String> {
    ...
    let command_id = first
        ...
        .correlation
        .command_id
        .expect("the submit carries a command id");
    ...
    assert_eq!(
        command_conflict_on(&declined, live.connection_id)?,
        CommandReplayRejectWire::CommandIdConflict { command_id },
...
async fn consent_replay_is_idempotent_and_moves_report_staleness() {
    ...
    assert!(
        ...
        "a re-issued assign on its now-stale base reports staleness (not a silent success), got {:?}",
        same.payload
    );
...
async fn assign_intent_replay_returns_the_stored_success() {
    ...
    let assigned = handle
        .handle_frame(assign_x("consent-dialogue-none"), live.clone(), &transport)
        .await;
    assert!(
        matches!(
...
impl ene_inference::ProviderTransport for RevokingTransport {
    ...
    > {
        ...
        Box::pin(async move {
            ...
            inner.complete_streaming(req, sink).await
        })
    }
}
...
#[tokio::test]
async fn setup_edge_cases_clarify() {
...
impl ProviderTransport for LearningAwareTransport {
    ...
    > {
        ...
        Box::pin(async move { push_completion(sink, text).await })
    }
}
    ...
    > {
        ...
        Box::pin(async move {
            if learning {
                ...
                return push_completion(
                    sink,
                    String::from(r#"{"summary": "Nothing new.", "memories": []}"#),
                )
                .await;
            }
            push_completion(sink, reply).await
        })
    }
}
...
/// A TARGET-bearing ExperienceCandidate popped from the Learning queue
...
async fn a_popped_learning_candidate_is_refused_after_deletion_completes() {
    ...
    let current = admit_staged(&handle, target, current_product_surface_owners()).await;
    for _ in 0..64 {
        let outcome = handle
...
/// A TARGET-bearing ExperienceCandidate that exists after pin_experience
...
/// Aborting handle_frame after the Learning pin is acquired must
...
async fn approving_a_credential_redacts_its_prior_occurrences() {
    ...
    let row_before = before
        ...
        .expect("the appended row is present before approval");
    assert_eq!(
        row_before.text, "my old key was test-bearer",
        "a pending registration leaves the prior row as plaintext"
    );
...
async fn memory_view_withholds_covered_bodies_and_tracks_only_delivered_ones() {
    ...
    use ene_preservation::ParticipantOwnerRef;
    use ene_primitive::WallClockWithTz;
...
async fn a_condition_committed_before_the_evidence_write_withholds_the_body() {
    ...
    use ene_preservation::{ParticipantOwnerRef, PreservationRepository as _};
    use ene_primitive::WallClockWithTz;
...
async fn same_round_newer_owner_input_supersedes_the_running_reply() -> Result<(), String> {
    ...
    assert_eq!(
        shown,
        vec!["Hel"],
        "the running prefix is presented before supersession, got {shown:?}"
    );
    assert!(
        !first_frames.iter().any(|frame| matches!(
            &frame.payload,
...
/// A Targeted Deletion condition that becomes durable while a
...
/// A dialogue claim that a deletion admission associated with its interval
...
async fn m2_a_dialogue_claim_hold_stops_streaming_and_reply_adoption() {
    ...
    // The admission's enumeration covers the seeded row because its body
    // carries the target: the already claimed dialogue attempt's read-set
    // intersects it, so the durable hold is written. No participant is driven,
    // so the transient fence stays put and only the claim hold can stop the
    // rest.
    admit_staged(
        &handle,
        "a seeded ordinary question",
        vec![ParticipantOwnerRef::Companion],
    )
    .await;
