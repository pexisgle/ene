...
impl ProviderTransport for ScriptedTransport {
    ...
    > {
        ...
        Box::pin(async move { super::push_completion(sink, text).await })
    }
}
...
impl ProviderTransport for GatedTransport {
    ...
    > {
        ...
        Box::pin(async move {
            ...
            super::push_completion(sink, String::from(r#"{"final":"report done"}"#)).await
        })
    }
}
...
async fn stage4_a_multi_read_transcript_stays_within_the_input_bound() {
    ...
    assert!(
        inputs[2].contains(&"b".repeat(100)) && !inputs[2].contains(&"a".repeat(100)),
        "the retained exchange must be the newest (second) read"
    );
    assert_eq!(
