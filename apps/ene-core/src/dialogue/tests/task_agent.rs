...
/// Seeds a durable Targeted Deletion condition covering one stored History
/// body through the production request/confirmation path: the admission's
/// enumeration publishes the row because its body carries the target.
async fn seed_covering_condition(store: &ene_store::Store, text: &str) {
    ...
    let request = match staged {
        ...
        other => panic!("the covering scope must stage, got {other:?}"),
    };
    match store
        ...
        .expect("the confirmation must answer")
    {
        ...
        other => panic!("the covering scope must start, got {other:?}"),
    }
}
...
async fn task_agent_turn_is_data_use_held_when_the_instruction_source_is_covered() {
    ...
    seed_covering_condition(&handle.store, "read the notes first").await;
...
async fn reopened_handle_does_not_replay_a_turn_and_resolves_the_instruction_again() {
    ...
    let instructions = OwnerInstructionSource::new(&reopened.store);
