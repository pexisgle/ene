...
impl ProviderTransport for GateTransport {
    ...
    > {
        ...
        Box::pin(async move {
            ...
            super::push_completion(sink, text).await
        })
    }
}
...
async fn production_launcher_steers_a_running_task_and_the_new_revision_executes() {
    ...
    agent.release();
    // Wait for the released detached runner to finish instead of racing a
    // fixed sleep; otherwise the negative asserts below pass vacuously.
    let deadline = tokio::time::Instant::now() + Duration::from_secs(10);
    while handle.task_executions.task_has_reservation_or_running(task) {
        assert!(
            tokio::time::Instant::now() < deadline,
            "the superseded execution did not stop"
        );
        tokio::time::sleep(Duration::from_millis(10)).await;
    }
    assert!(!workspace.path().join("stale.md").exists());
...
async fn provider_workspace_injection_is_invalid_and_changes_nothing() {
    ...
    let responses = handle
        ...
        .await;
    // An invalid reserved-protocol directive ends the turn interrupted: the
    // command never runs and no clarification reply is stored.
    assert!(
        accepted_round(&responses).is_ok(),
        "the owner message must be accepted: {responses:?}"
    );
    assert!(
        ...
        "the invalid directive must close the stream interrupted: {responses:?}"
    );
    assert!(!closes_completed(&responses), "{responses:?}");
    assert!(!contains_directive(&responses), "{responses:?}");

    // No clarification reply is stored: only the Owner row remains.
    let companion = handle
        .store
        .ensure_running_companion()
        .await
...
async fn a_transient_provider_failure_never_becomes_a_task_failure() {
    ...
    // Wait for the released detached runner to settle instead of asserting
    // while its failure may still be in flight.
    let deadline = tokio::time::Instant::now() + Duration::from_secs(10);
    while handle.task_executions.task_has_reservation_or_running(task) {
        assert!(
            tokio::time::Instant::now() < deadline,
            "the transient runner did not settle"
        );
        tokio::time::sleep(Duration::from_millis(10)).await;
    }
    assert_eq!(
...
#[tokio::test]
async fn conversation_invalid_directive_interrupts_without_changing_anything() {
    ...
    let responses = handle
        .handle_frame(
            request_frame(&handle, &live, Some(generation), "invalid", "do the thing"),
            live.clone(),
            &dialogue,
        )
        .await;
    // An invalid reserved-protocol directive ends the turn interrupted: no
    // command runs and no clarification reply is stored.
    assert!(
        accepted_round(&responses).is_ok(),
        "the owner message must be accepted: {responses:?}"
    );
