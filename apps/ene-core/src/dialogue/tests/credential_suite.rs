...
async fn assert_legacy_present(handle: &HostHandle, seed: &LegacySeed, secret: &str) {
    ...
    assert!(
        ...
        "the raw value is recorded in the resume activity"
    );
    ...
    assert!(
        summaries
            .iter()
            .any(|summary| summary.content.contains(secret)),
        "the raw value is recorded in the Summary"
    );
}
...
async fn assert_legacy_swept(handle: &HostHandle, seed: &LegacySeed, secret: &str) {
    ...
    assert!(
        !summaries.is_empty(),
        "the summary must still exist after the sweep"
    );
    assert!(
        summaries
            .iter()
...
async fn registered_secret_is_absent_from_provider_captures_and_every_surface() {
    ...
    let instructions = OwnerInstructionSource::new(&handle.store);
    ...
    assert!(
        memories.len() >= 2,
        "the Learning formation must add its own Memory to the seeded one, got {memories:?}"
    );
    ...
    // The seed records no Action attempt, so `action_attempt.real_target` path
    // fields are not directly swept here: paths are references to real-world
    // objects, not content (see the module doc). The durable all-column
    // backstop below still fails the suite if a path ever carries the value.
    ...
    // The composed user-facing report renders the result body and any Action
    // attempt targets; its own rendering is a surface too. This seed records no
    // Action attempt, so only the durable all-column backstop covers paths.
    let composed = handle
