...
async fn run<S: SecretScrubber>(
    ...
    assert!(registry.reserve(fixture.delegation, fixture.task.task));
    let super::TakeReservation::Admitted(registration) =
        registry.take_reservation(fixture.delegation, fixture.task.task)
    else {
        panic!("the fixture reservation must be admitted");
    };
...
#[test]
fn unconfirmed_effect_is_a_stop_and_is_not_a_workspace_body() {
    use ene_action::{ActionOutput, EffectGrounds, ObservedEffect};

    // The predicate and rendering the loop relies on: an unresolved effect is
    // classified as a stop and is never a replayable workspace body. This pins
    // the helpers, not the loop's early return (it has no deterministic seam).
