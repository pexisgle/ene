...
/// Reads one confirmation frame under a hang guard. `GuiChannel::recv` is a
/// blocking read, so the timeout cannot cancel the blocking task; it only fails
/// the test promptly instead of hanging the whole run.
async fn recv_within(gui: &ene_local_control::GuiChannel) -> Option<FromConfirmation> {
    tokio::time::timeout(
        Duration::from_secs(10),
        tokio::task::spawn_blocking({
            let mut channel = gui.try_clone().ok()?;
            move || channel.recv()
        }),
    )
    .await
...
async fn a_new_spawned_gui_invalidates_the_previous_seats_sessions() {
    ...
    let reply = recv_within(&first).await.expect("the old channel answers");
    assert!(
...
async fn concurrent_credential_publications_leave_the_latest_committed_snapshot_active() {
    ...
    assert_eq!(active, mutation.candidate_version);
    assert!(
...
/// A restart runs the bounded retirement cleanup from the durable row: a
/// retirement whose removal never ran is swept by startup, and its mutation
/// stays `CleanupPending` until that sweep records the removal.
#[tokio::test]
async fn startup_resumes_a_pending_retirement_cleanup() {
    let dir = tempfile::tempdir().expect("scratch");
    let credential = CredentialRef::new("openai", "main").expect("credential ref");
    let first_store = MemoryVersionedStore::new();
    first_store
        .put_version(&credential, 11, "sk-first")
        .expect("first item");
    first_store
        .put_version(&credential, 12, "sk-second")
        .expect("second item");
    {
        let handle =
            HostHandle::open_with_cred_store(dir.path(), CredStore::MemoryVersioned(first_store))
                .await
                .expect("first host");
        let store = handle.store_for_tests();
        store
            .begin_credential_mutation(
                String::from("m-1"),
                MutationKind::Register,
                ...
                String::from("main"),
                None,
                Some(SecretVersionId::from_u64(11)),
            )
            .await
            .expect("begin first");
        store
            .mark_credential_staged("m-1", SecretVersionId::from_u64(11))
            .await
            .expect("stage first");
        store
        ...
        store
            .begin_credential_mutation(
                ...
                String::from("main"),
                None,
                Some(SecretVersionId::from_u64(12)),
            )
            .await
            .expect("begin rotation");
        store
            .mark_credential_staged("m-2", SecretVersionId::from_u64(12))
            .await
        ...
        let pending = store
            ...
            .await
            .expect("pending read");
        assert_eq!(pending.len(), 1, "the rotation left one pending version");
        assert_eq!(pending[0].version, SecretVersionId::from_u64(11));
        assert_eq!(pending[0].mutation_id, "m-2");
        assert_eq!(
            store
                .credential_mutation("m-2")
                .await
                .expect("journal read")
                .expect("rotation mutation")
                .phase,
            MutationPhase::CleanupPending
        );
    }
    // The OS store survives the restart with the same items; the DB keeps the
    // pending row.
    let second_store = MemoryVersionedStore::new();
    second_store
        .put_version(&credential, 11, "sk-first")
        .expect("first item");
    second_store
        ...
        .expect("second item");
    let restarted =
        HostHandle::open_with_cred_store(dir.path(), CredStore::MemoryVersioned(second_store))
            .await
            .expect("restarted host");
    let store = restarted.store_for_tests();
    assert_eq!(
        store
            .pending_credential_retirements(10)
            .await
    ...
    restarted
        ...
        .await
        .expect("startup must resume the cleanup");
    assert!(
        store
            .pending_credential_retirements(10)
            .await
            .expect("pending read")
            .is_empty(),
        "startup swept the pending retirement"
    );
    assert_eq!(
        store
            .credential_mutation("m-2")
        ...
        "the retiring mutation completes only after the item is gone"
    );
}
...
async fn serving_time_control_approve_and_credential_put() {
    ...
    gui.send(&ToConfirmation::SessionComplete { session_id, nonce })
        .expect("send");
    let answer = recv_within(&gui).await.expect("the device approve answers");
    ...
    gui.send(&ToConfirmation::SessionComplete { session_id, nonce })
        .expect("send");
    let answer = recv_within(&gui).await.expect("the credential put answers");
    assert!(
        matches!(
            answer,
    ...
    match state {
        FromHost::RequestStatus { state, .. } => match state {
            RequestState::Applied {
                outcome: RequesterOutcome::CredentialRefused { provider, label },
            } => {
                assert_eq!(provider, "openai");
                assert_eq!(label, "rotated");
            }
            other => panic!("expected the refused credential outcome, got {other:?}"),
        },
        other => panic!("expected RequestStatus, got {other:?}"),
    }
...
#[tokio::test]
async fn sequentially_prepared_requests_keep_their_own_answers_and_drain_the_stream() {
    let dir = tempfile::tempdir().expect("scratch");
    let transport = GateTransport::with_replies(&["first-turn"]);
    let (_handle, server, mut client) = serve_and_setup(dir.path().to_path_buf(), transport).await;

    let first = client.prepare(WirePayload::ManagementViewRequest(
        cmds::setup_view_request(),
    ));
    let second = client.prepare(WirePayload::DeletionStatusRequest(DeletionStatusRequest {
        cursor: None,
        limit: Some(10),
    }));
    let first_answer = tokio::time::timeout(Duration::from_secs(10), client.execute(&first))
        .await
        .expect("first request must not time out")
        .expect("first request must answer");
    let second_answer = tokio::time::timeout(Duration::from_secs(10), client.execute(&second))
        .await
        .expect("second request must not time out")
        .expect("second request must answer");
    assert!(
        matches!(first_answer, WirePayload::ManagementView(_)),
        "the first request keeps its own view answer, got {first_answer:?}"
    );
    assert!(
        matches!(second_answer, WirePayload::DeletionStatusResponse(_)),
        "the second request keeps its own status answer, got {second_answer:?}"
    );
...
async fn deletion_demand_while_waiting_does_not_steal_the_answer() {
    ...
    gui.send(&ToConfirmation::SessionComplete { session_id, nonce })
        .expect("send");
    let answer = recv_within(&gui)
        .await
        .expect("the deletion confirmation answers");
    assert!(
        matches!(
            answer,
