...
use ene_task::{
    AssigneeRef, SteeringPremiseRef, Task, TaskCommitOutcome, TaskCommitPremise, TaskContextEntry,
...
#[tokio::test]
async fn propose_steering_reports_a_missing_task() {
    let task = TaskId::generate();
    let expected = TaskRef {
        task,
        revision: revision(1),
    ...
    let outcome = propose_steering(
        steer_command(expected, purpose, None, RawId::new()),
        &repository,
    )
    .await
...
#[tokio::test]
async fn propose_steering_reports_a_stale_purpose_token() {
    let task = TaskId::generate();
    let expected = TaskRef {
        task,
        revision: revision(2),
    ...
    let outcome = propose_steering(
        steer_command(expected, stale_purpose, None, RawId::new()),
        &repository,
    )
    .await
    ...
    assert!(
        repository.forwarded().is_empty(),
        "a stale purpose token must not forward"
    );
}
