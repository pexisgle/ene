...
#[cfg(test)]
mod tests {
    use super::{MutationKind, MutationOutcome, MutationPhase, SecretVersionId};
    ...
    #[test]
    fn only_a_committed_outcome_names_a_revision() {
        assert!(MutationOutcome::Activated { revision: 3 }.is_committed());
        assert!(MutationOutcome::Revoked { revision: 3 }.is_committed());
        assert!(!MutationOutcome::Stale.is_committed());
        assert!(!MutationOutcome::Refused.is_committed());
        assert!(!MutationOutcome::Rejected.is_committed());
        assert!(!MutationOutcome::Unknown.is_committed());
    }

    #[test]
    fn an_exhausted_version_counter_refuses_to_wrap() {
        assert_eq!(
            SecretVersionId::from_u64(u64::MAX).checked_next(),
            None,
            "a wrapped version would collide with a retired item"
        );
    }
}
