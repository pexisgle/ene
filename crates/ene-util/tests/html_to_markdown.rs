...
fn extract_html_full_no_trim_preserves_script() {
    ...
    assert!(
        out.contains("<script>"),
        "full/no-trim should preserve script"
    );
...
fn extract_html_full_trim_strips_script() {
    ...
    assert!(
        !out.contains("<script>"),
        "full/trim should remove script tags"
    );
    assert!(
        !out.contains("var x"),
        "full/trim should remove script content"
    );
...
fn extract_html_full_trim_strips_style() {
    ...
    assert!(
        !out.contains("<style>"),
        "full/trim should remove style tags"
    );
    assert!(out.contains("hello"), "full/trim should preserve text");
}
...
fn extract_html_full_trim_strips_noscript() {
    ...
    assert!(
        ...
        "full/trim should remove noscript"
    );
    assert!(
        !out.contains("fallback"),
        "full/trim should remove noscript content"
    );
}
...
fn extract_html_body_works() {
    ...
    assert!(
        out.contains("<body>"),
        "body extraction should include <body>"
    );
    assert!(
        out.contains("content"),
        "body extraction should include body content"
    );
}
...
fn extract_html_main_selects_main() {
    ...
    assert!(
        out.contains("<main>"),
        "main extraction should include <main>"
    );
    assert!(
        out.contains("content"),
        "main extraction should include content"
    );
}
...
fn extract_html_main_falls_back_to_body() {
    ...
    assert!(
        out.contains("<body>"),
        "main fallback should include <body>"
    );
    assert!(
        out.contains("content"),
        "main fallback should include content"
    );
}
...
fn extract_html_main_falls_back_to_root() {
    ...
    assert!(
        out.contains("content"),
        "ultimate fallback should include content"
    );
}
...
fn extract_html_strips_multiple_skip_tags() {
    ...
    assert!(
        out.contains("also keep"),
        "non-skip content should be preserved"
    );
...
fn extract_html_nested_skip_tags() {
    ...
    assert!(
        !out.contains("inner"),
        "inner content of script should be removed"
    );
}
...
fn extract_markdown_full_trim_strips_script() {
    ...
    assert!(
        !md.contains("alert"),
        "markdown should not contain script content"
    );
}
...
fn html_to_markdown_converts_headings_and_paragraphs() {
    ...
    assert!(
        md.contains("Hello"),
        "html_to_markdown should convert heading"
    );
    assert!(
        md.contains("World"),
        "html_to_markdown should convert paragraph"
    );
}
