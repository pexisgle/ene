...
) -> Result<ConversationSummaryResult, AiCoreError> {
    if history.is_empty() {
...
## summary の書き方
- 出来事を時系列に羅列しないでください
- 会話から「重要な結論」「決定事項」「新しい発見」「感情の重要な変化」のみを抽出してください
...
fn parse_summary_json(raw: &str) -> Result<ConversationSummaryResult, AiCoreError> {
    let cleaned = raw
        .trim()
