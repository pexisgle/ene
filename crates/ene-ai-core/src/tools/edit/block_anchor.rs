...
pub fn block_anchor_replace(content: &str, old: &str, new: &str, _replace_all: bool) -> Option<String> {
    ...
    for i in 0..original_lines.len() {
        if original_lines[i].trim() != first_line { continue; }
    ...
    let check_candidate = |start_line: usize, end_line: usize| -> Option<(usize, usize)> {
        ...
        if lines_to_check > 0 {
            for j in 1..search_block_size - 1 {
                if j >= actual_block_size - 1 { break; }
    ...
    if candidates.len() == 1 {
        ...
        for k in 0..s { start_idx += original_lines[k].len() + 1; }
    ...
    if max_similarity < multi_threshold { return None; }
    let (s, e) = best?;
    let mut start_idx = 0usize;
    for k in 0..s { start_idx += original_lines[k].len() + 1; }
    let mut end_idx = start_idx;
