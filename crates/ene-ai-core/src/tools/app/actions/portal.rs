...
#[cfg(target_os = "linux")]
pub async fn capture_screen_portal(scale_percent: u32) -> Result<DynamicImage, AiCoreError> {
    ...
    let response = Screenshot::request()
        ...
        .send()
        .await
        .map_err(|e| AiCoreError::AppError(format!("Portal screenshot request failed: {e}")))?
        .response()
        .map_err(|e| AiCoreError::AppError(format!("Portal screenshot response failed: {e}")))?;

    let uri = response.uri();
    let path = uri.as_str().strip_prefix("file://").unwrap_or(uri.as_str());

    let image = image::open(path)
        .map_err(|e| AiCoreError::AppError(format!("Failed to open screenshot file: {e}")))?;
...
#[cfg(target_os = "linux")]
pub async fn capture_window_portal(scale_percent: u32) -> Result<DynamicImage, AiCoreError> {
    ...
    let proxy = Screencast::new()
        .await
        .map_err(|e| AiCoreError::AppError(format!("Failed to create Screencast proxy: {e}")))?;

    let session = proxy
        .create_session(Default::default())
        .await
        .map_err(|e| AiCoreError::AppError(format!("Failed to create screencast session: {e}")))?;
    ...
    proxy
        ...
        .await
        .map_err(|e| AiCoreError::AppError(format!("Failed to select sources: {e}")))?;

    let response = proxy
        .start(&session, None, StartCastOptions::default())
        .await
        .map_err(|e| AiCoreError::AppError(format!("Failed to start screencast: {e}")))?
        .response()
        .map_err(|e| AiCoreError::AppError(format!("Screencast start response failed: {e}")))?;

    let streams = response.streams();
    if streams.is_empty() {
        return Err(AiCoreError::AppError(
    ...
    tokio::task::spawn_blocking(move || capture_pipewire_frame(node_id))
        .await
        .map_err(|e| AiCoreError::AppError(format!("PipeWire task failed: {e}")))?
...
#[cfg(target_os = "linux")]
fn capture_pipewire_frame(node_id: u32) -> Result<DynamicImage, AiCoreError> {
    use pipewire as pw;
    ...
    let mainloop = pw::main_loop::MainLoopRc::new(None)
        .map_err(|e| AiCoreError::AppError(format!("Failed to create PipeWire mainloop: {e}")))?;

    let context = pw::context::ContextRc::new(&mainloop, None)
        .map_err(|e| AiCoreError::AppError(format!("Failed to create PipeWire context: {e}")))?;

    let core = context
        .connect_rc(None)
        .map_err(|e| AiCoreError::AppError(format!("Failed to connect PipeWire core: {e}")))?;
    ...
    .map_err(|e| AiCoreError::AppError(format!("Failed to create PipeWire stream: {e}")))?;
    ...
    let _listener = stream
        ...
        .register()
        .map_err(|e| AiCoreError::AppError(format!("Failed to register stream listener: {e}")))?;
    ...
    .map_err(|e| AiCoreError::AppError(format!("Failed to serialize pod: {e}")))?
    .0
    .into_inner();

    let pod = Pod::from_bytes(&values)
        .ok_or_else(|| AiCoreError::AppError("Failed to parse pod from bytes".to_string()))?;
    let mut params = [pod];
    ...
    stream
        ...
        .map_err(|e| AiCoreError::AppError(format!("Failed to connect PipeWire stream: {e}")))?;
    ...
    let pixels = s
        .pixels
        .as_ref()
        .ok_or_else(|| AiCoreError::AppError("No frame captured from PipeWire".to_string()))?;
    let width = s.width;
    let height = s.height;
    let is_bgr = s.is_bgr;
    let bpp = s.bpp;
...
#[cfg(target_os = "linux")]
fn list_windows_hyprland() -> Result<String, AiCoreError> {
    let output = std::process::Command::new("hyprctl")
        .args(["clients", "-j"])
        .output()
        .map_err(|e| AiCoreError::AppError(format!("Failed to run hyprctl: {e}")))?;
    ...
    let clients: Vec<serde_json::Value> = serde_json::from_slice(&output.stdout)
        .map_err(|e| AiCoreError::AppError(format!("Failed to parse hyprctl JSON: {e}")))?;
...
#[cfg(target_os = "linux")]
fn list_windows_sway() -> Result<String, AiCoreError> {
    let output = std::process::Command::new("swaymsg")
        .args(["-t", "get_tree"])
        .output()
        .map_err(|e| AiCoreError::AppError(format!("Failed to run swaymsg: {e}")))?;
    ...
    let tree: serde_json::Value = serde_json::from_slice(&output.stdout)
        .map_err(|e| AiCoreError::AppError(format!("Failed to parse sway tree JSON: {e}")))?;
...
#[cfg(target_os = "linux")]
fn list_windows_gnome() -> Result<String, AiCoreError> {
    ...
    let output = std::process::Command::new("gdbus")
        ...
        .output()
        .map_err(|e| AiCoreError::AppError(format!("Failed to run gdbus: {e}")))?;
    ...
    let result = parse_gdbus_tuple_string(&stdout)
        .ok_or_else(|| AiCoreError::AppError("Failed to parse gdbus output".to_string()))?;
...
#[cfg(target_os = "linux")]
fn try_qdbus_eval(js: &str) -> Result<String, AiCoreError> {
    let output = std::process::Command::new("qdbus")
        .args(["org.kde.plasmashell", "/PlasmaShell", "evaluateScript", js])
        .output()
        .map_err(|e| AiCoreError::AppError(format!("Failed to run qdbus: {e}")))?;
...
#[cfg(target_os = "linux")]
fn try_gdbus_kwin_eval(js: &str) -> Result<String, AiCoreError> {
    let output = std::process::Command::new("gdbus")
        .args([
            "call",
            "--session",
...
#[cfg(target_os = "linux")]
fn list_windows_kde() -> Result<String, AiCoreError> {
...
#[cfg(target_os = "linux")]
fn focus_window_hyprland(title: &str) -> Result<String, AiCoreError> {
    let output = std::process::Command::new("hyprctl")
...
#[cfg(target_os = "linux")]
fn focus_window_sway(title: &str) -> Result<String, AiCoreError> {
...
#[cfg(target_os = "linux")]
fn focus_window_gnome(title: &str) -> Result<String, AiCoreError> {
...
#[cfg(target_os = "linux")]
fn focus_window_kde(title: &str) -> Result<String, AiCoreError> {
    let escaped = title.replace('\\', "\\\\").replace('\'', "\\'");
    let js = format!(
...
#[cfg(target_os = "linux")]
pub fn focus_window_wayland(title: &str) -> Result<String, AiCoreError> {
    let compositor = detect_wl_compositor().unwrap_or_default();
    match compositor.as_str() {
