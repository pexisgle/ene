...
mod schema;
mod serialization;
use serialization::{f32_slice_to_bytes, bytes_to_f32_vec};
...
impl MemoryStore {
    pub fn open(path: &Path, embedding_dim: usize) -> Result<Self, AiCoreError> {
        ...
        let store = Self { conn: Mutex::new(conn), embedding_dim };
    ...
    pub fn search_summaries(
        ...
        for row_result in rows {
            let (id, sid, cn, summary, blob, ca, ea, sim) =
                row_result.map_err(|e| AiCoreError::MemoryStoreError(e.to_string()))?;
            ...
            results.push(RecalledSummary {
                entry: ConversationSummary {
                    id, session_id: sid, card_name: cn, summary, embedding,
                    created_at: parse_dt(&ca), ended_at: parse_dt(&ea),
                },
                similarity: sim,
            });
        }
    ...
    pub fn list_recent_summaries(
        ...
        for r in rows {
            let (id, sid, cn, summary, blob, ca, ea) =
                r.map_err(|e| AiCoreError::MemoryStoreError(e.to_string()))?;
            let parse_dt = |s: &str| DateTime::parse_from_rfc3339(s)
                .map(|dt| dt.with_timezone(&Utc))
                .unwrap_or_else(|_| Utc::now());
            entries.push(ConversationSummary {
                id, session_id: sid, card_name: cn, summary,
    ...
    pub fn delete_summary(&self, id: i64) -> Result<usize, AiCoreError> {
        let conn = self.conn.lock().unwrap();
        let tx = conn.unchecked_transaction()?;
        tx.execute("DELETE FROM conversation_keyfacts WHERE summary_id = ?1", params![id])?;
        let count = tx.execute("DELETE FROM conversation_summaries WHERE id = ?1", params![id])?;
        tx.commit()?;
        Ok(count)
    }
    ...
    pub fn get_all_keyfacts(&self, card_name: &str) -> Result<Vec<KeyFact>, AiCoreError> {
        ...
        for r in rows {
            facts.push(r.map_err(|e| AiCoreError::MemoryStoreError(e.to_string()))?);
        }
        Ok(facts)
    }
    ...
    pub fn get_logs_by_session(
        ...
        for r in rows {
            result.push(r.map_err(|e| AiCoreError::MemoryStoreError(e.to_string()))?);
        }
        Ok(result)
    }
}
...
mod tests {
    use super::*;
    use super::serialization::{f32_slice_to_bytes, bytes_to_f32_vec};
    ...
    fn test_insert_and_search_summaries() {
        ...
        store.insert_summary("s1", "char", "Summary A", &[], &emb_a, Utc::now()).unwrap();
        store.insert_summary("s2", "char", "Summary B", &[], &emb_b, Utc::now()).unwrap();
    ...
    fn test_keyfacts_crud() {
        ...
        let summary_id = store.insert_summary("s1", "char", "Summary", &[], &emb, Utc::now()).unwrap();

        let facts = vec![
            KeyFact { key: "job".to_string(), value: "engineer".to_string() },
            KeyFact { key: "food".to_string(), value: "ramen".to_string() },
        ];
        let now = Utc::now().to_rfc3339();
    ...
    fn test_insert_summary_with_empty_value_deletes_keyfact() {
        ...
        store.insert_summary(
            "s2", "char", "Summary 2",
            &[
                KeyFact { key: "job".to_string(), value: "designer".to_string() },
                KeyFact { key: "hobby".to_string(), value: "".to_string() },
            ],
            &emb2,
            Utc::now(),
        ).unwrap();
