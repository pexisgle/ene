...
impl ConversationSession {
    ...
    pub fn load_card(&mut self, path: &str) -> Result<Vec<ResolvedExpression>, crate::error::AiCoreError> {
        ...
        let file_content = std::fs::read_to_string(path)
            .map_err(crate::error::AiCoreError::CardReadError)?;

        let mut card = serde_json::from_str::<CharacterCardV3>(&file_content)
            .map_err(crate::error::AiCoreError::JsonError)?;
