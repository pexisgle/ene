//! Typed memory domain model — re-exported from `ene-core` (#270).
//!
//! These types are pure domain vocabulary (kinds, statuses, scores, queries)
//! with no `SeaORM`/SQL concerns, so their definitions now live in
//! `ene-core`. This module stays so existing `ene_store::typed_memory::*` /
