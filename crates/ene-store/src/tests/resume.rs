...
use ene_preservation::{DeletionOperationRef, ParticipantOwnerRef};
use ene_task::{
...
/// Admits one body-only deletion operation against the Task owner through the
/// production stage/confirm path.
async fn admit(store: &Store, text: &str) -> DeletionOperationRef {
    super::preservation::admit_with(store, text, Vec::new(), vec![ParticipantOwnerRef::Task]).await
}

/// The collected form of a covered body: the mechanical target is gone and the
/// fixed marker records where it was removed.
...
#[tokio::test]
async fn a_carried_resume_purpose_and_scope_under_a_condition_are_materialized_body_free() {
    let store = open_store().await;
    let (companion, generation) = running_companion(&store).await.unwrap();
    let target = "the private key";
    // Both carried copies the resume freezes — the purpose text and the
    // workspace boundary — start under the condition's exact target.
    let workspace = task_workspace(
        "/srv/workspace/the private key",
        Some("/srv/workspace/the private key/out"),
    );
    let created = store
        .create_task(TaskCreationPremise {
            task: TaskId::generate(),
            purpose: TaskPurpose {
                text: format!("write about {target}"),
            },
            entry: TaskContextEntryId::generate(),
            origin: TaskContextOrigin {
                kind: TaskContextOriginKind::OwnerConversation,
    ...
    let record = store.load_task(created.task).await.unwrap().unwrap();
    assert_collected(&record.revision.purpose_text.text, target);
    let stored = store
        .load_delegation(new_delegation)
        .await
        .unwrap()
    ...
    assert_collected(&scope.folder.path, target);
    assert_collected(
        &scope.save_target.expect("the copy had a save target").path,
        target,
    );
}

#[tokio::test]
async fn inconsistent_resume_snapshot_rows_fail_closed_without_writes() {
    for corrupt in ["purpose", "assignee"] {
        let store = open_store().await;
        let (companion, generation) = running_companion(&store).await.unwrap();
        ...
        let record = store.load_task(created.task).await.unwrap().unwrap();
        {
            ...
            let sql = match corrupt {
                "purpose" => {
                    "UPDATE task_revision SET purpose_adopted_revision = 99 WHERE task_id = ?1 AND revision = ?2"
                }
                _ => {
                    "UPDATE task_revision SET assignee = 'bogus' WHERE task_id = ?1 AND revision = ?2"
                }
            };
            guard
                .execute(
        ...
        assert!(
            ...
            "a corrupt {corrupt} snapshot must fail closed, got {outcome:?}"
        );
        assert_eq!(
            table_counts(&store),
            before,
            "a corrupt {corrupt} snapshot writes nothing"
        );
    }
}
...
async fn activity_for_another_task_is_a_forged_reference() {
    ...
    // A fresh key records independently of the earlier activity's task and
    // content; the conflicting-key refusal is pinned by
    // `conflicting_command_key_reuse_fails_closed` below.
...
#[tokio::test]
async fn past_fact_lines_neutralize_control_characters_in_the_target() {
    let store = open_store().await;
    let (companion, _) = running_companion(&store).await.unwrap();
    let (created, assoc) = seed_task(&store, companion).await;
    let delegation = seed_delegation(&store, created, assoc).await;
    start_attempt(&store, delegation, created, assoc, "line\nbreak.txt").await;
    ...
    let line = &page.facts[0].line;
    assert!(
        !line.contains('\n') && !line.contains('\r'),
        "one fact is one line, got {line}"
    );
    assert!(
        line.contains("line\\nbreak.txt"),
        "the newline is rendered as its escape, got {line}"
    );
}
