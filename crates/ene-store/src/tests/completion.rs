...
/// The completion audit is body-free: the target body, an unrelated
/// secret-looking body, and a prompt body are all absent from every audit
/// column, while the protected search material itself is destroyed.
#[tokio::test]
...
/// including the published source correlation and the staged request's
/// protected text. The first-party admission re-derives the mechanical target
/// and keeps no staged semantic hints, so the hint table is not this path's
/// subject.
#[tokio::test]
async fn completion_destroys_the_search_material_unrecoverably() {
    ...
    // A covered History body gives admission a real source correlation to
    // publish; the Companion sweep below must collect it before completion.
    insert_delayed_history(&store, companion, &format!("body with {target}"));
    let staged = store
    ...
    // Admission publishes the source correlation for the covered History body;
    // it must be real for the completion wipe to be observed.
    assert_eq!(
    ...
    assert_eq!(
        operation_rows(&store, "erasure_condition_source", current),
        1,
        "Finalizing keeps the source correlation until the completion commit"
    );
    assert_eq!(
        store.complete_deletion_finalizing(current).await.unwrap(),
        DeletionFinalizationOutcome::Completed
    ...
    assert_eq!(
        ...
        "the Companion sweep collected the covered History body"
    );
