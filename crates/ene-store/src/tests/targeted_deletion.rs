...
use ene_preservation::{
    DeletionLifecycleChange, DeletionLifecycleOutcome, DeletionOperationId, DeletionSearchMaterial,
    DeletionSweepGeneration, DemandLocalErasureCommand, ErasureConditionRef,
    ErasureParticipant as _, MechanicalDeletionTarget, ParticipantCompletionFact,
    ParticipantCompletionStatus, ParticipantErasureScope, ParticipantHoldClass,
    ParticipantOwnerRef, PreservationRepository as _, TargetedDeletionTarget,
};
...
async fn admit_owner_with_sources(
    ...
    super::preservation::admit_with(store, text, sources, vec![owner])
        .await
        .condition()
}
...
async fn drive(
    ...
    drive_with_sources(participant, condition, owner, text).await
}
...
pub(super) async fn drive_with_sources(
    ...
    for _ in 0..ERASURE_SCAN_ROWS * 8 {
        let fact = participant
            .demand_local_erasure(local_command(condition, owner, text))
            .await;
        if matches!(
            fact.status(),
...
async fn a_multi_page_sweep_restarts_from_the_head_after_a_lost_cursor() {
    ...
    let first = participant
        .demand_local_erasure(local_command(sweep, ParticipantOwnerRef::Companion, target))
        .await;
...
async fn a_repeat_demand_of_a_verified_sweep_erases_nothing_new() {
    ...
    // The continuation tracks only unfinished sweeps, so a repeat demand on a
    // verified sweep starts a fresh sweep from the head: it finds nothing left
    // to erase and re-verifies with a zero count.
    let repeat = participant
        .demand_local_erasure(local_command(sweep, ParticipantOwnerRef::Companion, target))
        .await;
    assert_eq!(repeat.status(), ParticipantCompletionStatus::Verified);
    assert_eq!(repeat.erased_count(), 0);
    assert!(erased >= 1, "the first sweep erased the matching row");
...
async fn a_matching_current_memory_takes_its_history_and_index_with_it() {
    ...
    // own (the current row was rewritten while the revision snapshot kept the
    // original text). The sweep must erase the recognition whole, not only the row the
...
/// The durable walk cursor is a scheduling position for one bounded walk: it
/// round-trips per walk, is independent between walks, survives a reopen, and
/// never substitutes for the operation's durable phase.
#[tokio::test]
async fn deletion_walk_cursor_round_trips_per_walk_and_survives_reopen() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("walk-cursor.db");
    let store = Store::open(&path).await.unwrap();
    let condition = admit_owner(&store, "walk-cursor-target", ParticipantOwnerRef::Companion).await;
    assert_eq!(
        store
            .deletion_walk_cursor(ene_preservation::DeletionWalk::FanOut)
            .await
            .unwrap(),
        None,
        "a fresh walk starts at the head"
    );
    store
        .set_deletion_walk_cursor(
            ene_preservation::DeletionWalk::FanOut,
    ...
    assert_eq!(
        store
            .deletion_walk_cursor(ene_preservation::DeletionWalk::RetryableHold)
            .await
            .unwrap(),
        None,
        "each walk keeps its own position"
    );
    drop(store);
    ...
    assert_eq!(
        reopened
            ...
            .unwrap(),
        Some(condition.operation),
        "the scheduling position survives restart"
    );
    // Writing a position is not a lifecycle action: the operation is still
    // unfinished Active state, derived from its own rows.
    let record = reopened
        .unfinished_deletions(None, 10)
        .await
        .unwrap()
        .into_iter()
        .find(|record| record.current.operation == condition.operation)
        .expect("the operation stays unfinished");
    assert_eq!(
        record.phase,
        ene_preservation::DeletionOperationPhase::Active
    );
    // Resetting the walk removes the row instead of storing a sentinel.
    reopened
        .set_deletion_walk_cursor(ene_preservation::DeletionWalk::FanOut, None)
        .await
        .unwrap();
    assert_eq!(
        reopened
