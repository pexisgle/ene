...
async fn credential_erasure_removes_metadata_without_touching_protected_values() {
    use ene_credential::available_credential;
...
async fn presence_erasure_clears_target_bearing_references_and_proves_absence() {
    ...
    let target_client = ClientId::from_raw(RawId::new());
    let other_client = ClientId::from_raw(RawId::new());
    let other_companion = RawId::new();
    present_on(&store, companion, target_client).await;
    {
        let conn = store.conn.lock().unwrap();
        conn.execute(
            "UPDATE relocation_hint SET recovery_destination = ?1 WHERE companion_id = ?2",
            params![
                crate::codec::encode_id(target_client.as_raw()),
    ...
    assert_eq!(erased, 3, "the attribution stop and the hint clear");
