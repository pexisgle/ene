...
async fn enumerated_sources_survive_restart_and_sweep_and_clear_on_completion() {
    ...
    let participant = crate::CompanionErasureParticipant::new(reopened.clone());
    super::targeted_deletion::drive_with_sources(
        &participant,
        next.condition(),
        ParticipantOwnerRef::Companion,
        target,
    ...
    .await;
    super::preservation::complete_via_a5(&reopened, next).await;
    assert_eq!(
