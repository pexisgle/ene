...
use super::task_result::target_path;
...
fn attempt_premise(
    ...
    // The shared builder fixes the canonical path and the `Create` operation;
    // the Action fixtures vary the operation, so override that one field.
    let mut premise =
        super::task_result::attempt_premise(attempt, delegation, task, workspace, name);
    premise.operation = operation;
    premise
}
...
async fn seed_workspace_delegation(store: &Store) -> (TaskRef, DelegationId, WorkspaceAssocId) {
    ...
    let delegation = super::task_result::create_workspace_delegation(
        store,
        created,
        assoc,
