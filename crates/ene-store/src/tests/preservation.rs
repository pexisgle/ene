...
/// Stages one fixture scope through the production request inlet.
pub(super) async fn stage(store: &Store, text: &str) -> DeletionRequestId {
    match store
        ...
        .unwrap()
    {
        ...
        other => panic!("the fixture scope must stage, got {other:?}"),
    }
}

/// Admits one fixture operation through the production stage/confirm path,
/// then restores the durable shape the deleted direct admission seam produced:
/// the caller-named source correlations and already-complete reconciliation
/// cursors. Tests that pin the production enumeration drive the staged path
/// itself (see `source_reconciliation.rs`).
pub(super) async fn admit_with(
    ...
    let request = stage(store, text).await;
        ...
        .unwrap()
    {
        ...
        other => panic!("the fixture confirmation must start, got {other:?}"),
    };
    let id = crate::codec::encode_id(current.operation.as_raw());
    let sweep = i64::try_from(current.sweep.as_u64()).unwrap();
    let guard = store.conn.lock().unwrap();
    for source in sources {
        guard
            .execute(
                "INSERT OR IGNORE INTO erasure_condition_source (operation_id,sweep,source) VALUES (?1,?2,?3)",
                params![id, sweep, crate::codec::encode_id(source)],
            )
            .unwrap();
    }
    guard
        .execute(
            "UPDATE deletion_reconciliation SET complete=1 WHERE operation_id=?1 AND sweep=?2",
            params![id, sweep],
        )
        .unwrap();
    drop(guard);
...
pub(super) async fn admit(store: &Store, text: &str, sources: Vec<RawId>) -> DeletionOperationRef {
    admit_with(store, text, sources, fixture_participants()).await
}
...
async fn closed_completed_condition_is_not_a_permanent_ban_and_corruption_fails_closed() {
    ...
    admit(&store, "temporary", vec![RawId::new()]).await;
}
...
async fn finalizing_next_sweep_returns_to_active_on_remainder_and_stays_finalizing_without_material()
 {
    ...
    assert_eq!(rows[0].hold, None);
    {
        // The new generation resets the walk; the fabricated finalizing shape
        // must re-establish the same durable premise the transition would. The
        // fixture drops the staged-request link with the material: an
...
async fn admission_snapshots_the_required_participants_and_restart_keeps_them() {
    ...
    let current = admit_with(&store, "snapshot-target", vec![], participants).await;
    let page = store
...
async fn an_empty_or_duplicate_required_participant_set_is_refused() {
    let store = open_memory().await.unwrap();
    let request = stage(&store, "no-participants").await;
    assert_eq!(
        store.confirm_targeted_deletion(request, vec![]).await,
        Err(PreservationTechnicalError::InvalidParticipantSet),
        "an operation with no required participant could never be verified"
    );
    let request = stage(&store, "duplicate-participants").await;
    assert_eq!(
        store
...
async fn holds_are_distinct_durable_incomplete_outcomes() {
    ...
    let current = admit_with(&store, "held-target", vec![], participants).await;
    let condition = current.condition();
    let at = WallClockWithTz::now();
...
#[tokio::test]
async fn material_read_returns_the_protected_target_and_indexed_source_membership() {
    let store = open_memory().await.unwrap();
    let source = RawId::new();
    let other = RawId::new();
    ...
    let DeletionMaterialOutcome::Material(_) = store
    ...
    // Destroyed, never as corrupt and never as protected material.
    {
        ...
        // shape must re-establish the whole durable premise. The fixture drops
        // the staged-request link with the material: an operation whose A5
        // wipe already removed the target has no exact text left to match its
        // request row.
        reconcile_to_complete(&store, next).await;
        mark_all_verified(&store, next);
        let conn = store.conn.lock().unwrap();
