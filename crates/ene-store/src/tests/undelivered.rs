...
use ene_companion::{
    ...
    ResumeActivityOutcome, TaskFact, TerminalKindWire, UNDELIVERED_PAGE_MAX, UndeliveredCursor,
    UndeliveredId, UndeliveredSource,
};
use ene_task::{
...
async fn reads_register_nothing_and_excerpts_stay_bounded() {
    ...
    let (_, registered) = store
        .append_reply_with_undelivered(history_command(companion, generation, body), None)
        .await
        .unwrap();
    ...
    // An activity record is body-bearing: the notification must not be
    // delivered empty just because its origin is the activity record.
    let loaded = store.load_task(task.task).await.unwrap().unwrap();
    let recorded = store
        .record_resume_activity(RecordResumeActivityCommand {
            companion: CompanionId::from_raw(loaded.revision.assignee.companion),
            task,
            purpose: loaded.task.purpose,
            body: String::from("continue with the report"),
            command: RawId::new(),
        })
        .await
        .expect("the activity must record");
    let ResumeActivityOutcome::Recorded(activity) = recorded else {
        panic!("the clean activity must record, got {recorded:?}");
    };
    let excerpt = store
        .load_undelivered_excerpt(UndeliveredSource::ActivityRecord(activity.as_raw()), 1024)
        .await
        .unwrap()
        .expect("the activity source carries its body");
    assert_eq!(excerpt.text, "continue with the report");
    assert_eq!(excerpt.total_bytes, "continue with the report".len() as u64);
}
