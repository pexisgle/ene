...
fn consumed(
    ...
    let breakdown =
        crate::usage_cap::consumption_breakdown(&guard, scope, window, CurrencyCode::Usd, at_value)
            .expect("the accounting read must answer")
            .expect("the fixture accounting must be representable");
    breakdown.reserved + breakdown.committed_reported + breakdown.committed_unknown
}
...
async fn a_cap_refusal_creates_no_attempt_reservation_or_usage() {
    ...
    // The limit is below the 201-micro upper bound: the very first claim is
    ...
    assert!(
        matches!(&outcome, AttemptBeginOutcome::HeldByCap),
        "a cap below the upper bound must hold the send, got {outcome:?}"
    );
...
async fn concurrent_claims_for_one_remaining_slot_start_at_most_one() {
    ...
    // Exactly one 201-micro reservation fits.
    let (scope, window) = system_daily();
    ...
    let held = [&a, &b]
        .iter()
        .filter(|outcome| matches!(outcome, AttemptBeginOutcome::HeldByCap))
        .count();
    assert_eq!(
        (started, held),
        (1, 1),
    ...
    let held_ticket = if matches!(a, AttemptBeginOutcome::HeldByCap) {
        first_ticket
    } else {
        second_ticket
    };
...
async fn the_smaller_of_provider_and_system_caps_is_enforced() {
    ...
    assert!(
        matches!(&second, AttemptBeginOutcome::HeldByCap),
        "the system cap must hold the second call, got {second:?}"
    );
    ...
    assert!(
        matches!(&second, AttemptBeginOutcome::HeldByCap),
        "the provider cap must hold the second call, got {second:?}"
    );
...
async fn daily_and_monthly_windows_are_checked_separately() {
    ...
    assert!(
        matches!(&second, AttemptBeginOutcome::HeldByCap),
        "the monthly cap must hold the second call, got {second:?}"
    );
}
...
async fn a_cap_raise_applies_to_the_next_admission_after_the_update_commits() {
    ...
    let premise = route_consent(&store).await;
    let (scope, window) = system_daily();
    // One reserved upper bound fits (201 <= 301); two do not (402 > 301).
    ...
    assert!(
        matches!(third, AttemptBeginOutcome::HeldByCap),
    ...
    // raised cap and fits the second reserved bound (201 + 201 <= 402).
...
async fn a_stale_cap_expectation_writes_nothing() {
    ...
    let stale = update_cap(
        ...
        Some(UsageCapRef::new(
            ene_permission::UsageCapId::new(scope.clone(), window),
...
async fn reported_settlement_counts_actual_and_releases_the_unused_reservation() {
    ...
    // (201 + 128 = 329 <= 350), but two reserved bounds would not
    // (402 > 350): only a settlement that released the unused reservation
...
async fn a_committed_reported_reservation_still_counts_after_restart() {
    ...
    assert_eq!(
        second,
        AttemptBeginOutcome::Started,
        "the committed slot stays counted: 128 + 201 <= 350"
    );
}
