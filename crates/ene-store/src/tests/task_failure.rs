...
use super::task_result::{progress, seed_workspace_execution};
...
async fn in_progress_task_with_its_delegation_moves_to_failed() {
    let store = open_memory().await.unwrap();
    let (created, delegation, _assoc) = seed_workspace_execution(&store).await;
    assert_eq!(
...
async fn completion_cannot_overwrite_failed_and_failed_cannot_overwrite_completion() {
    ...
    let (failed_task, failed_delegation, _) = seed_workspace_execution(&store).await;
    assert_eq!(
        store
    ...
    let (completed_task, completed_delegation, _) = seed_workspace_execution(&store).await;
...
async fn cancel_cannot_overwrite_failed_and_failed_cannot_overwrite_cancel() {
    ...
    let (failed_task, _, _) = seed_workspace_execution(&store).await;
    assert_eq!(
    ...
    let (cancelled_task, cancelled_delegation, _) = seed_workspace_execution(&store).await;
    assert_eq!(
...
async fn a_stale_premise_cannot_fail_a_steered_task() {
    let store = open_memory().await.unwrap();
    let (created, delegation, _) = seed_workspace_execution(&store).await;
...
async fn a_foreign_delegation_is_a_fail_closed_technical_error() {
    let store = open_memory().await.unwrap();
    let (first, _, _) = seed_workspace_execution(&store).await;
    let (_, foreign_delegation, _) = seed_workspace_execution(&store).await;
    assert!(matches!(
        store
...
async fn missing_identities_answer_their_own_variants() {
    ...
    let (created, _, _) = seed_workspace_execution(&store).await;
...
async fn existing_admission_gates_refuse_a_failed_task() {
    let store = open_memory().await.unwrap();
    let (created, delegation, assoc) = seed_workspace_execution(&store).await;
    assert_eq!(
        store
            .fail_task(failure(created, Some(delegation)))
            .await
...
#[tokio::test]
async fn a_terminal_task_at_a_moved_revision_answers_terminal_not_stale() {
    let store = open_memory().await.unwrap();
    let created = store
        .create_task(task_premise(None))
        .await
        .expect("the AU2 task must commit");
    // Move the Task forward, then fail it: the terminal row now sits at a
    // revision later than the one a delayed delegation request carries.
    let second = store
        .forward_steering(TaskCommitPremise {
            expected: created,
            new_purpose: Some(task_purpose_adoption("moved before the terminal")),
            adopted_purpose_entry: TaskContextEntryId::generate(),
            adopted_instruction: None,
        })
        .await
        .expect("the steering forward must answer");
    let TaskCommitOutcome::CommittedAs(second) = second else {
        panic!("expected CommittedAs, got {second:?}");
    };
    assert_eq!(
        store
            .fail_task(failure(second, None))
            .await
            .expect("the failure commit must answer"),
        TaskFailureOutcome::FailedAs(second)
    );

    // The delayed AU3 request still carries the pre-move revision. The
    // terminal answer must win over the stale compare, with zero delegation
    // rows written.
    let refused = store
        .create_delegation(delegation_premise(
            DelegationId::generate(),
        ...
        .await
        .expect("the delegation admission must answer");
    assert_eq!(
        refused,
        DelegationOutcome::TaskTerminal {
            task: created.task,
    ...
    assert_eq!(task_table_count(&store, "delegation"), 0);
}
