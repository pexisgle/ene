...
use ene_preservation::{
    DeletionOperationPhase, DeletionOperationRef, ErasureConditionRef, ParticipantCompletionFact,
    ParticipantCompletionOutcome, ParticipantOwnerRef, PreservationRepository as _,
};
use ene_task::{DelegationId, TaskAgentResultArrival, TaskId, TaskResultId, TaskRevision};

use super::preservation::{complete_via_a5, enter_finalizing_via_a5, mark_all_verified};
...
/// of a body-only fixture) plus any additional required owners, through the
/// production stage/confirm path.
async fn admit(
    store: &Store,
    text: &str,
    sources: Vec<RawId>,
    ...
    super::preservation::admit_with(store, text, sources, participants).await
}
...
async fn append_reply_with_body(
    ...
    store
        .append_reply_with_undelivered(command, None)
        .await
...
async fn seed_delegation(store: &Store, task: TaskRef) -> DelegationId {
    ...
    super::task_result::create_workspace_delegation(store, task, assoc, None).await
}
...
async fn condition_first_refuses_a_dialogue_append_without_leaving_a_row() {
    ...
    let (outcome, registered) =
        append_reply_with_body(&store, companion, generation, "about the private key").await;
...
async fn append_first_then_condition_refuses_the_delayed_reply() {
    ...
    let (outcome, registered) = store
        .append_reply_with_undelivered(reply, None)
        .await
        .expect("the reply append must answer");
    assert_eq!(outcome, HistoryAppendOutcome::HeldForErasure);
    assert!(registered.is_none());
...
async fn the_admission_hold_probe_uses_the_source_correlation_index() {
    ...
    assert!(
        !plan.iter().any(|line| line.contains("SCAN u")),
...
fn assert_collected(text: &str, target: &str) {
    ...
    assert!(
        text.contains(crate::erasure::ERASED_MARKER),
...
async fn condition_first_holds_a_presentation_start_and_ack() {
    ...
    let (companion, generation) = companion_with_generation(&store).await;
    let (outcome, registered) =
        append_reply_with_body(&store, companion, generation, "the private key").await;
...
async fn torn_or_unreadable_current_state_fails_closed_at_the_boundary() {
    ...
    let current = admit(&store, "the private key", Vec::new(), Vec::new()).await;
    {
        ...
        let conn = store.conn.lock().unwrap();
        // The fixture drops the staged-request link with the material: an
        // operation whose A5 wipe already removed the target has no exact text
        // left to match its request row.
        conn.execute(
            "UPDATE deletion_operation SET phase='finalizing', request_id=NULL WHERE operation_id=?1",
            [&id],
        )
        .unwrap();
        conn.execute(
...
#[tokio::test]
async fn a_held_result_body_already_at_the_marker_still_verifies_against_an_overlapping_target() {
    let store = open_memory().await.unwrap();
    let files = tempfile::tempdir().unwrap();
    let (_task, delegation) = seed_workspace_execution(&store, "write the report").await;
    let task = store
        ...
        .unwrap()
        .task;
    // The exact target is a substring of the fixed `[erased]` marker, so
    // certifying an already-collected body clean by identity would leave the
    // target in the durable row.
    let source = workspace_source(&files, "input.txt", "please erase this");
    let attempt = claim_read_attempt(&store, task, delegation, &source).await;
    record_observation(&store, delegation, Some(attempt), Some("please erase this")).await;

    let current = admit(&store, "erase", Vec::new(), Vec::new()).await;
    assert_eq!(delegation_hold_rows(&store, delegation), 1);

    // The delayed arrival is materialized body-free as the fixed marker.
    let arrival = result_arrival(
        &store,
        delegation,
        "the report paraphrases material without quoting it",
    )
    .await;
    let record = expect_recorded(
        store
    ...
    assert!(
        ...
        "the overlapping target must still verify, got {fact:?}"
    );

    let stored = store
        .load_task_result(record.result)
        .await
        .unwrap()
        .expect("the result row is retained");
    assert!(
        !stored.body.text().contains("erase"),
        "the marker must not certify an overlapping target clean, got {:?}",
        stored.body
    );
    let guard = store.conn.lock().unwrap();
    assert_eq!(
        crate::erasure::exact_remainder_probe(&guard, "erase").unwrap(),
        0
    );
}
...
/// The occurrence ledger copies the producing attempt's workspace, so that
/// copy must agree with the delegation scope: a rewritten attempt workspace is
/// torn state and refuses the write instead of copying a foreign workspace.
#[tokio::test]
async fn a_foreign_attempt_workspace_refuses_the_observation_write() {
    let store = open_memory().await.unwrap();
    let files = tempfile::tempdir().unwrap();
    let (_task, delegation) = seed_workspace_execution(&store, "write the report").await;
    let task = store
    ...
    let attempt = claim_read_attempt(&store, task, delegation, &source).await;
    {
        let foreign = crate::codec::encode_id(RawId::new());
        let conn = store.conn.lock().unwrap();
        conn.execute(
            "UPDATE action_attempt SET workspace_assoc_id=?1 WHERE attempt_id=?2",
            rusqlite::params![foreign, crate::codec::encode_id(attempt)],
        )
    ...
    assert!(
        ...
        "a foreign attempt workspace must fail closed, got {outcome:?}"
    );
    assert_eq!(
        observation_rows(&store),
        0,
        "the refused write must leave no observation row"
    );
}

/// The occurrence ledger decodes the producing attempt's operation as a
/// closed-world name, so an unknown stored value is torn state even when the
/// premise carries no observed body: the write must fail closed instead of
/// publishing a correlation from a row the Action reads reject.
#[tokio::test]
async fn an_unknown_attempt_operation_refuses_the_observation_write() {
    let store = open_memory().await.unwrap();
    let files = tempfile::tempdir().unwrap();
    let (_task, delegation) = seed_workspace_execution(&store, "write the report").await;
    let task = store
    ...
    let attempt = claim_read_attempt(&store, task, delegation, &source).await;
    {
        let conn = store.conn.lock().unwrap();
        conn.execute(
            "UPDATE action_attempt SET operation='teleport' WHERE attempt_id=?1",
            [crate::codec::encode_id(attempt)],
        )
        .expect("the corruption must apply");
    }
    ...
    assert!(
        matches!(outcome, Err(TaskTechnicalError::StorageUnavailable { .. })),
        "an unknown attempt operation must fail closed, got {outcome:?}"
    );
    assert_eq!(
        observation_rows(&store),
        0,
...
/// The body-in-memory window after a workspace read and before the
...
/// Action start during Finalizing must still hold the
...
/// One sealed paraphrase of a body that carried two exact targets must
...
async fn one_delegation_corresponds_to_two_unfinished_operations() {
    ...
    // The erasure_use_hold PRIMARY KEY (use_kind,use_id,operation_id) makes a
    // repeated association a no-op; this pins the storage guarantee the
    // production INSERT OR IGNORE association relies on.
    {
        let conn = store.conn.lock().unwrap();
        conn.execute(
...
/// One inference claim whose data_use covers two targets is
...
/// A formation identity published before the Learning claim
