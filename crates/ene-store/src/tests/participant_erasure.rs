...
const MARKER: &str = crate::erasure::ERASED_MARKER;
...
async fn demand(
    ...
    let scope = ParticipantErasureScope::local(material.target().clone());
    participant
...
/// revision purpose, one workspace association with the target in both paths,
/// and the delegation's copied scope.
