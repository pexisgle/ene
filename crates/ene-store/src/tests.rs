...
/// A v11 database migrates through v12 into v13 in one chain: history rows
/// gain the canonical UTC projection and pre-index memories gain token
/// rows, so neither the History window nor lexical recall goes dark.
#[tokio::test]
async fn migration_v11_applies_v12_then_v13_in_order() {
    ...
    assert_eq!(
        ...
        "v12 backfills the canonical UTC rendering, keeping the offset display"
    );
    let candidates = store
        .recall_candidates(companion, &[String::from("jasmine")], 3)
        .await
    ...
    assert!(
        ...
        "v13 backfills the token index for the pre-index memory"
    );
}

/// A v12 database with the History projection applied migrates into v13
/// with that projection intact and the token schema added.
#[tokio::test]
async fn migration_v12_preserves_at_utc_and_adds_the_token_index() {
    let dir = tempfile::tempdir().expect("a temp dir must open");
    let path = dir.path().join("app.db");
    let companion = RawId::new();
    let memory = MemoryId::generate();
    let message = RawId::new();
    {
        ...
        let outcome = store
            ...
            .await;
        assert!(matches!(outcome, Ok(MemoryChangeOutcome::Committed { .. })));
        // Rewind to the v12 shape the History change left behind: the UTC
        // projection stays applied, only the token index is missing.
        {
            ...
            guard
                ...
                .expect("the version-12 rewind must apply");
        }
    }
    let store = Store::open(&path).await.expect("migration must succeed");
    assert_eq!(
        read_schema_version(&path),
        Some(13),
        "a v12 database must converge on v13"
    );
    let (projection, columns) = {
        ...
        let projection: String = guard
            ...
            .expect("the v12 projection must survive");
    ...
    assert_eq!(
        projection, "2026-09-08T03:00:00.000000000Z",
        "v13 must not disturb the v12 projection"
    );
    for index in [
        ...
        assert!(
            ...
            "v13 must create {index}, got {columns:?}"
        );
    }
    let candidates = store
        .recall_candidates(companion, &[String::from("jasmine")], 3)
    ...
    assert!(
        candidates
            .iter()
            .any(|candidate| candidate.content.contains("jasmine tea")),
        "the v12 memory must be lexically reachable after v13"
    );
}
