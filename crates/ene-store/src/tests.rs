...
PRAGMA user_version = 2;",
    ...
    assert!(
        matches!(version, Ok(21)),
        "migration must record version 21"
    );
...
async fn migration_v3_reopen_keeps_pairing_state() {
    ...
    assert!(
        matches!(version, Ok(21)),
        "reopened database must record schema version 21"
    );
}
...
async fn migration_v11_applies_v12_through_v18() {
    ...
    assert_eq!(
        read_schema_version(&path),
        Some(21),
        "a v11 database must converge on v21"
    );
    let projection = {
        let guard = match store.conn.lock() {
            Ok(locked) => locked,
...
async fn migration_v13_preserves_at_utc_and_adds_the_token_index() {
    ...
    assert_eq!(
        read_schema_version(&path),
        Some(21),
        "a v13 database must converge on v21"
    );
...
async fn task_migration_adds_tables_to_a_v14_database() {
    ...
    assert_eq!(
        read_schema_version(&path),
        Some(21),
        "a v14 database must converge on v21"
    );
...
async fn task_migration_v15_context_rows_backfill_as_adopted_purpose() {
    ...
    assert_eq!(
        read_schema_version(&path),
        Some(21),
        "a v15 database must converge on v21"
    );
...
async fn delegation_migration_adds_the_table_to_a_v16_database() {
    ...
    let (creation, created, dropped) = {
        ...
        assert_eq!(
            read_schema_version(&path),
            Some(21),
            "a fresh database converges on v21"
        );
        assert!(
    ...
    assert_eq!(
        read_schema_version(&path),
        Some(21),
        "a v16 database converges on v21"
    );
