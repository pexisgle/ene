...
impl PricingCatalog {
    ...
    pub fn resolve(&self, provider: &str, model: &str, at: WallClockWithTz) -> PricingResolution {
        ...
        match entry {
            Some(entry) => PricingResolution::Priced(entry.clone()),
    ...
    pub fn first_party() -> Result<Self, PricingCatalogError> {
        ...
        let entries = FIRST_PARTY_OPENAI_RATES
            .iter()
            .map(|(model, input, cached_input, output)| PricingSnapshot {
                ...
                effective_at,
                source_revision: FIRST_PARTY_REVISION,
            })
            .collect();
        Self::new(entries)
    }
...
mod tests {
    ...
    fn entry(model: &str, at_value: &str) -> PricingSnapshot {
        PricingSnapshot {
            provider: String::from("openai"),
            model: String::from(model),
            currency: CurrencyCode::Usd,
    ...
    fn duplicate_effective_instants_for_one_route_are_refused() {
        let result = PricingCatalog::new(vec![
            entry("gpt-test", "2025-06-01T00:00:00Z"),
            entry("gpt-test", "2025-06-01T00:00:00Z"),
        ]);
        assert_eq!(
