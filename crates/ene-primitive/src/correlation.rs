...
///
/// `from_raw` is the source side (for example the derivative) and `to_raw` the
/// target side (for example the source range). Arrival order, identifier
/// magnitude, and wall-clock time are never the basis of the correspondence;
/// only this explicit pairing is. `purpose` is explanatory, never a match key
/// (correspondence-identity §4.6): no logic may branch on its contents,
/// prefix-match it, or treat equal purposes as the same correspondence.
...
impl DirectedPair {
    /// Rejects a pair whose `purpose` is empty: a correspondence with nothing
    /// to say about itself explains nothing.
