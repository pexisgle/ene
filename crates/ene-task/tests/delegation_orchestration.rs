...
#[tokio::test]
async fn terminal_task_with_a_moved_revision_is_reported_as_terminal() {
    let task = TaskId::generate();
    let purpose = TaskPurposeRef {
        task,
        adopted_revision: revision(2),
    };
    let mut loaded = record(task, revision(2), purpose, TaskContextEntryId::generate());
    loaded.task.progress = TaskProgress::Failed;
    let repository = FakeTaskRepository::new(Ok(Some(loaded)), assignee());
    let requested = TaskRef {
        task,
        revision: revision(1),
    };
    ...
    let outcome = orchestrate_delegation(
        &repository,
        command(requested, DelegationScope { workspace: None }),
    )
    .await
    .expect("terminal progress is a domain outcome, not a technical error");
    ...
    assert_eq!(
        ...
        "a terminal Task must be reported as TaskTerminal, never folded into a stale revision"
    );
    assert!(
        repository.delegated().is_empty(),
        "a terminal Task must not create a delegation"
