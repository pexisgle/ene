...
impl MemoryRerankOptions {
    /// Build options from cognition memory config.
    #[must_use]
...
mod tests {
    ...
    use ene_provider::LlmProviderError;
    use std::pin::Pin;
    use tokio_stream::Stream;

    type StreamResult = Pin<
        Box<dyn Stream<Item = Result<ene_provider::LlmResponseChunk, LlmProviderError>> + Send>,
    >;
    ...
    fn sample_item(id: i64, title: &str, content: &str, total: f32) -> ScoredMemory {
        ScoredMemory {
            item: MemoryItem {
                id: Some(id),
                scope: ene_memory::MemoryScope::Character,
    ...
    fn options_from_config_applies_defaults() {
        let config = CognitionMemoryConfig {
            rerank_enabled: true,
