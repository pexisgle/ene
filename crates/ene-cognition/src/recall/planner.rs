...
impl RecallPlannerOptions {
    /// Build recall-planner options from cognition context and memory config.
    #[must_use]
    pub fn from_config(context: &ContextConfig, memory: &CognitionMemoryConfig) -> Self {
        Self {
...
impl Default for RecallPlannerOptions {
    fn default() -> Self {
        Self::from_config(&ContextConfig::default(), &CognitionMemoryConfig::default())
    }
}
...
mod tests {
    ...
    use ene_memory::{ActiveCommitmentPrompt, DiscreteEmotion};
    ...
    fn simple_input_generates_semantic_and_episodic_queries() {
        ...
        assert!(
            plan.required_kinds
                .contains(&ene_memory::MemoryKind::Semantic)
        );
        assert!(
            plan.required_kinds
                .contains(&ene_memory::MemoryKind::Episodic)
        );
    }
    ...
    fn preference_input_biases_semantic_queries() {
        ...
        assert!(
            plan.required_kinds
                .contains(&ene_memory::MemoryKind::Preference)
        );
        assert!(
    ...
    fn active_commitments_require_commitment_kind() {
        ...
        assert!(
            plan.required_kinds
                .contains(&ene_memory::MemoryKind::Commitment)
        );
    }
    ...
    fn budget_hints_follow_context_config() {
        ...
        let memory = CognitionMemoryConfig {
