...
pub async fn classify_for_config(
    ...
    classify_with_resilient_fallback(
        move |attempt_cap| {
            ...
            ene_provider::create_openai_compatible_chat_provider(&config, model.as_deref(), cap)
...
pub fn classify_failure_reason(error: &CognitionError) -> &'static str {
    match error {
        CognitionError::Classifier(classifier_error) => {
...
async fn call_provider(
    ...
    match transport {
        ClassifierTransport::NonStreaming => provider
            ...
            .await
            .map_err(ClassifierError::Provider),
        ClassifierTransport::Streaming => ene_provider::collect_chat_completion(provider, messages)
            .await
            .map_err(ClassifierError::Provider),
    }
}
...
mod tests {
    ...
    fn classify_failure_reason_labels() {
        ...
        let provider = CognitionError::Classifier(ClassifierError::Provider(
            ene_provider::LlmProviderError::Provider("boom".into()),
        ));
