...
/// | Procedure | ✓ | — | — | — | ✓ | standard |
/// | `WorldState` | — (reserved, #209) | — | — | — | ✓ | standard |
/// | Reflection | ✓ | — | — | — | ✗ (#347) | standard |
///
...
/// talk about the same subject (#351): *vector (title embedding)* compares
...
/// (`mind.memory.default_forgetting_half_life_days`, #76); there is no
...
pub enum MemoryKind {
    ...
    /// world state (#209) and not yet generated by any producer. The LLM
    ...
    /// only appear via direct store writes or future #209 work.
    WorldState,
...
/// Natural-forgetting treatment for a memory kind (#76).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
...
impl MemoryKind {
    ...
    /// structured time-series world state (#209) and no producer exists yet, so
    ...
    /// supersede the prior record rather than accumulate (#348). In the normal
    ...
    /// subject (#351). Only meaningful when [`Self::is_contradiction_kind`] is
    /// true.
    ...
    /// Whether this kind is eligible for normal (cognitive) recall (#347).
    ///
    ...
    /// Natural-forgetting treatment for this kind (#76).
    ///
...
mod tests {
    ...
    /// The per-kind policy table (#348) must cover every variant and encode
