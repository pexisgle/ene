...
// ── Cognition types ──
// Re-exporting the cognition config types here forces the `ene-cognition`
// crate to be linked into every `ene-core` consumer (CLI, Desktop). That
// in turn fires the `ctor::ctor` block generated by `define_config!` in
// `ene-cognition::CognitionConfig`, registering the `cognition` section
// in the global schema registry. Without these re-exports the schema
// generator never sees the section and the JSON schema shipped to users
// is missing the cognitive runtime block (see issue #95).
...
/// it via [`EneConfig::get_section`] returns the macro-defined defaults
