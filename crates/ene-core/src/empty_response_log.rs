...
use ene_cognition::PromptPacketMeta;
...
pub(crate) fn log_empty_response_if_needed(ctx: EmptyResponseContext<'_>) {
    ...
    if let Some(meta) = ctx.prompt_meta {
        ...
        warn.record(
            ...
            meta.post_history_included,
        );
        warn.record(
            "identity_kernel_included",
            meta.identity_kernel_included,
        );
        if !meta.dropped_sections.is_empty() {
            warn.record(
                "dropped_sections",
                format!("{:?}", meta.dropped_sections),
            );
        }
    }
