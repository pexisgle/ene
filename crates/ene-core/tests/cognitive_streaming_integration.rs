...
use ene_core::streaming::{StreamContext, run_stream};
use ene_core::{EneEvent, TerminalReason};
...
async fn run_stream_cognitive_path_completes_with_logs() {
    ...
    while let Ok(event) = event_rx.try_recv() {
        if let EneEvent::Terminal(TerminalReason::Done) = event {
            saw_done = true;
        }
    }
