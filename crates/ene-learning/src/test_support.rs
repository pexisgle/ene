...
impl FakeLearningRepository {
    ...
    /// Makes every later commit answer [`MemoryChangeOutcome::HeldForErasure`]
    /// without writing, modelling a claim or content under a deletion
    /// interval.
    pub(crate) fn hold_for_erasure(&self) {
        *self.held_for_erasure.lock().expect("fake hold lock") = true;
    }
...
impl LearningRepository for FakeLearningRepository {
    async fn commit_memory_change(
        ...
        if *self.held_for_erasure.lock().expect("fake hold lock") {
