...
impl EmbeddingProvider for CloudEmbeddingProvider {
    async fn embed(&self, text: &str, kind: EmbeddingKind) -> Result<Vec<f32>, EmbeddingError> {
        ...
        let input = apply_kind_prefix(text, kind, self.query_prefix.as_deref());
    ...
    async fn embed_query(&self, text: &str) -> Result<Vec<f32>, EmbeddingError> {
        // Single source of truth for query prefixing lives in `embed`;
        // calling `embed` here with the already-prefixed text would
        // prepend the prefix twice.
