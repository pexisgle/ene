...
- Field order (always use this order): `reason`, `should_speak`, `confidence`, `topic_hint`, `urgency`
- No extra keys.
...
- `reason` (string): Internal diagnostic only — never spoken aloud. 1–3 short lines. Explain why you chose speak or silence. No dialogue, no greetings.
...
- `topic_hint` (string): Hint for a later generator — empty string when `should_speak` is false. 0–2 lines about what to talk about. Do not copy `reason` verbatim.
- `urgency` (string): One of `"low"`, `"normal"`, `"high"`.

## Decision rules
- Prefer `should_speak=false` when unsure or when context is thin.
- Set `should_speak=true` only when conversation history, screen summary, commitments, or activity gives a concrete hook.
...
- Do: write `reason` first to justify the boolean fields that follow.
