...
モデル ⇄ Tool Calling(層ごとにフィルタされたスキーマ面)
      ...
      統一ツールレジストリ(この文書)
              ↑
  供給元: ビルトイン / プラグイン(IPC) / MCP / 委譲(秘匿サブエージェント)
```

## 1. 責務と責務外

- **責務**: ツール定義の登録・照合、層ごとのモデル向けスキーマの公開、
  実行パイプラインの駆動、出力の正規化と上限、UI 表示用の投影。
- **責務外**: ツール本体の実装(供給元の責務)、承認判断
...
| `execute` | 実行ハンドラ | 供給元ごとの実体(ビルトイン関数 / IPC 呼び出し / MCP 呼び出し) |
