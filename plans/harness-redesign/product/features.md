...
|---|---|---|---|---|
| P-101 | テキスト対話 | テキスト入力でターンを起こし、ストリーミング応答を得る。複数クライアントから同一セッションに同時接続可能。 | v1.0 | core/agent-loop, platform/server-api |
| P-102 | 全二重音声対話 | 話しかけによる割り込み(バグイン)、自声回避、中断からの再開を含む双方向音声。パイプラインはコアデーモン所有。 | v1.0 | body/voice |
...
| P-107 | 複数コンパニオンの並行会話 | 各コンパニオンが独立セッションで同時に会話できる。対話レーンは soul ごとに独立。 | v1.0 | core/agent-loop, platform/clients |
...
| P-504 | 対話1 + job 並行 | soul ごとに単一フライトの対話レーンと、独立した job レーンを持つ。 | v1.0 | core/agent-loop |
...
| P-508 | 秘匿サブエージェント | 内部で委譲実行するサブエージェント。ユーザーのイベント・UI から完全に秘匿。実体は非同期委譲(P-519)の internal モードであり、job は同機構の public モード。 | v1.0 | core/delegation |
...
| P-514 | エージェント実装の差し替え | Agent 抽象の実装差し替え(別アーキテクチャの soul 実装を積む)。 | 後継 | core/agent-loop |
...
| P-519 | 非同期委譲と主従対話 | 実作業の委譲は非同期。親エージェントは委譲を待たずに対話を続ける。親は委譲に進捗照会・追加指示・キャンセルでき、子は親に進捗報告・質問・完了報告を送れる。 | v1.0 | core/delegation, core/agent-loop |
...
| P-521 | 完了報告の対話化 | 委譲の完了報告は親の対話レーンにターンとして届き、親がユーザーへの伝え方(発話・内面・静黙)を判断する。報告が対話を遮ることはない。 | v1.0 | core/delegation |

## P-6xx ツールと仕事面

| ID | 機能 | 説明 | 優先度 | 対応サブシステム |
|---|---|---|---|---|
| P-601 | 統一ツールレジストリ | 全供給元のツールを1つのレジストリに集約。モデルには単一のスキーマ面を公開。 | v1.0 | tools/registry |
