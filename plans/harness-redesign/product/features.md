...
| P-104 | 内面の表示 | 思考・感情表出・行動意図を発話と分離して記録(内面チャネル)。**表示は既定 OFF**。ON にできるのは詳細画面だけ。表層UIに出す設定は持たない(D-20)。 | v1.0 | companion/inner-channel |
...
| ID | 機能 | 説明 | 優先度 | 対応サブシステム |
|---|---|---|---|---|
| P-501 | イベントソーシング・セッションログ | セッションの全事実を追記専用ログに記録。SQLite 単一ソース。適用範囲は**会話ログのみ**で、記憶・スケジュール・成果物・設定は通常のテーブル(D-9)。読み取りが投影を通るため、スナップショットまたはキャッシュを持つ。 | v1.0 | core/session-log |
...
| P-515 | 中断の検出と報告 | 再起動時に、中断されたターン/ジョブを検出し、後始末をして、ユーザーに中断を報告する。**重複効果なしの再開は保証しない**(D-5)。単なるツール呼び出しの失敗はこれとは別で、失敗として表示すれば足りる。 | v1.0 | core/agent-loop |
...
| P-518 | スキーマ進化 | ストレージの永続形式にバージョンを持ち、アプリ更新時に開く際の連鎖マイグレーションで追従する。開いているターン/job の中断状態(D-5)も変換対象。レジスタと操作状態機械の変換規則は後継(P-525 / D-4)。 | v1.0 | core/storage-model |
...
| P-520 | 表示の深さ | 秘匿は「見せない」の一律規則ではなく、表示面ごとの深さで決まる。深さは `surface` / `detail` の2段階だけ(D-11)。表層UIは発話と成果物のみ(作業中の1行表示までは可)、詳細画面では内面・ツール呼び出し・層間メッセージ・thinking まで表示できる。desktop も Web も詳細画面を持つ(D-31)。 | v1.0 | core/visibility |
...
| P-615 | ハーネス機能ツール | ホスト内で実行されるがモデルからは呼べるツール(記憶想起・委譲・内面・ユーザーへの質問)。ハーネス状態と密結合するためプロセス境界の向こうに置けない(D-10)。 | v1.0 | tools/registry |
...
| P-712 | 詳細画面 | 内面・ツール呼び出し・層間メッセージ・thinking・感情の内部状態を検証するための表示面(D-11/D-19/D-20)。desktop は別ウィンドウで設定・記憶編集も含む。Web も同じ深さの表示面を持つが、閲覧とデバッグに限り、変更は desktop のみ(D-31)。CLI は `--verbose` / `ene debug`。 | v1.0 | platform/clients, core/visibility |
