...
内面の**表示は既定 OFF**。詳細画面でのみ ON にできる(D-20)。

これは [../core/visibility.md](../core/visibility.md) の「表示の深さ」に従う。
内面は表層UIの深さには含まれず、詳細画面の深さに含まれる。

| 表示面 | 内面 |
|---|---|
| 表層UI(stage・チャット) | 出さない(既定) |
| 詳細画面 | 出す |

表層UIに出す設定も持つが、既定は OFF である。内面が常時流れていると、
stage が「機構の実況中継」になり、対話相手と話している感覚が薄れる。

クライアントが表示する場合は「発話とは別の視覚様式」を使う
(吹き出しの外・薄い色・斜体等はクライアントの自由)。
ライブストリームではチャンク単位で流し、確定時に永続イベントと
対応付ける([../core/session-log.md §6](../core/session-log.md#6-ライブストリームとの分担))。
配信するかどうかの判定はサーバ側で行い、クライアントのフィルタに依存しない。
...
| `mind.inner.surface_visible` | **`false`** | 表層UIに出すか(D-20)。詳細画面は常に見える |
