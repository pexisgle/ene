...
> soul はコンパニオンの**表層**である([../product/vision.md](../product/vision.md#51-コアデーモン内の2層))。
...
Ene は感情モデルとして両者を(対外層と内部層の)2層で統合する。
コンパニオン実行時の表層/裏層とは別の話である
...
| **内部層(PAD 状態)** | 連続値の状態ベクトル。減衰・関係メトリクス・疲労を時間で動かす | システム。数値は表層UIに出さず、詳細画面にのみ出す(D-19) |
...
4. **ヒステリシス**: 同一ラベルの再発火とラベル変更にそれぞれ最小間隔を置き、
   小さすぎる強度差の更新は吸収する。表情のちらつきを防ぐため。
5. **レート制限**: 表情変更に毎分の上限を置き、超過はマージする。

## 7. 感情の可視化と編集(P-307, P-308 / D-19)

感情は**常に body の emotion と連携して表示する**。これは opt-in ではない
——表情が動かないアバターは、感情モデルを持っていないのと同じだからである。

**body の扱い方はクライアントが決め、コアは指定しない。**
コアが配るのは離散感情ラベルと強度だけで、それを VRM の表情に写すか、
テキストで示すか、何もしないかはクライアントの判断である。

body を持たない desktop / web 構成では、**body の emotion をそのまま
簡単なテキストUIに出せば足りる**。「body なしのときの感情表現」を
別途設計しない——同じラベルを別の様式で出すだけであり、
専用の語彙を作ると2つの表現がずれていく。

一方、**内部状態(PAD の連続値・関係メトリクス・fatigue)は
詳細画面にのみ出す**。表層UIに数値は出さない。

| 表示面 | 感情 |
|---|---|
| 表層UI | body の emotion(離散ラベル+強度)。数値は出さない |
| 詳細画面 | PAD の連続値、関係メトリクス、fatigue、更新の履歴 |

ベースラインの編集(P-308, stretch)は設定 UI から行い、変更は監査ログに残る。

## 8. コンパニオン間の関係と家庭モデル

- soul 同士は**既定では関係メトリクスを持たない**。
- 記憶の相互影響は共有記憶プール
  ([memory.md §4](memory.md#4-スコープと共有記憶プールp-207--d-7))を介する。
  ユーザーに関する事実は全 soul が共有し、体験と関係性は soul ごとに閉じる。
- 「家庭」(複数 soul の緩いグループ)は**メタデータのみ**:
  名前と参加 soul の列挙。関係メトリクス(trust 等)はユーザー↔soul のみで、
  soul 間は持たない(複雑化の抑止)。同室会話(P-114)は後継。
...
| キー | 説明 |
|---|---|
| `mind.affect.classifier` | 分類器に使う補助LLM |
| `mind.affect.decay.*` | 減衰時定数(速い成分・遅い成分) |
| `mind.affect.arbiter.min_interval` | ラベル変更の最小間隔(ちらつき防止) |
| `mind.affect.arbiter.max_per_minute` | 表情変更のレート上限 |
