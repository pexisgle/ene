...
| P-5xx | ハーネス | core/(語彙層5 + 耐久性層4 + 駆動/診断面2 の計11文書) |
...
11. 内面チャネルは永続+自己参照窓。
12. 音声は全二重・サーバー所有。
13. workspace は内部概念のみ。成果物は広範+段階化。skill は業界標準準拠。
...
18. 実作業は**非同期委譲**に移す。親は待たず会話を続け、完了報告は
    対話レーンにターンとして届く(P-519/P-521)。job と秘匿サブエージェントは
    同機構の public/internal モード。
