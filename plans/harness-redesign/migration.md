...
- ツールレジストリ+パイプライン。バンドル済みプラグインの最小(fs/exec/web)。
  ホスト内関数は置かない。
- Broker の最小(fs/net)。サンドボックス(Landlock)の適用。
- **承認 plane 完成前の既定**: `read_only` 以外は **deny-by-default**。
  開発中の暗黙 allow は禁止する。W2 の plane が来るまで、副作用のある
  ツールは明示 allow リストか手動オーバーライド以外では動かない。
- **完了条件**: アウトプロセスのツールが登録・実行され、
  サンドボックス内で動く。IPC オーバーヘッドが **`minimal` プロファイル**で
  予算内(5ms 中央値、[platform/process-model.md §6](platform/process-model.md#6-性能予算))。
  非 read_only の暗黙実行がテストで拒否される。
...
- W1 の deny-by-default を plane が引き継ぐ。`approval.mode` が実行時の正。
  `auto` 以外で read_only 以外の暗黙 allow は残さない。
...
- 見るもの: 対話+委譲+job+MCP の通し、W1 の deny-by-default が
  W2 承認と衝突していないこと、`minimal` の性能予算。
...
- **完了条件**: 3クライアントが同一コアに接続し、同時接続・
  排他調停が動く(P-7xx)。性能予算は `minimal` を正、`desktop` は別予算
...
| リスク | 影響 | 緩和 |
|---|---|---|
| 全アウトプロセスによる軽量化の困難 | 性能予算の未達 | 測定は `minimal` プロファイルで行う(W1 で早期にベンチ)。`desktop` は別予算。予算超過時は IPC の最適化に集中 |
