...
impl ApprovalGate {
    ...
    ///
    /// Direct calls without a turn arrive with a unique synthetic turn id
    /// generated by the host, so every invocation refreshes the scope.
    ...
    ///
    /// `target` is a stable identifier without private content, terminated
    /// with `#` (e.g. `homeassistant:entity:light.living_room#`); the
    /// human-readable `description` is shown to the user only and is not
    /// logged by the host's audit trail.
    ///
    /// The `#` terminator is what makes prefix matching safe: entity ids
    /// cannot contain `#`, so a pattern ending in `#` can never be a raw
    /// prefix of a sibling entity (`light.living_room#` does not match
    /// `light.living_room2#`). Patterns without a terminator still grant a
    /// deliberate prefix, e.g. `homeassistant:entity:light.` for the whole
    /// light domain.
    ///
...
mod tests {
    ...
    #[test]
    fn session_pattern_does_not_cover_sibling_entities() {
        let gate = ApprovalGate::new();
        gate.on_call_context("conv-1", Some("turn-1"));
        gate.allow_pattern(
            actions::HOMEASSISTANT_TURN_ON,
            "homeassistant:entity:light.living_room#",
        );
        gate.check(actions::HOMEASSISTANT_TURN_ON, TARGET, "preview")
            .expect("the exact entity must match");
        for sibling in [
            "homeassistant:entity:light.living_room2#",
            "homeassistant:entity:light.living_room_guest#",
            "homeassistant:entity:light.living_room.extra#",
        ] {
            let err = gate.check(actions::HOMEASSISTANT_TURN_ON, sibling, "preview");
            assert!(
                matches!(err, Err(ToolError::PermissionRequired { .. })),
                "sibling entity {sibling} must not match"
            );
        }
    }
    ...
    #[test]
    fn direct_call_with_synthetic_turn_gets_a_fresh_scope() {
        // The host gives every context-less direct call a unique synthetic
        // turn id, so each direct call must start deny-by-default.
        let gate = ApprovalGate::new();
        gate.on_call_context("conv-1", Some("turn-1"));
        let Err(ToolError::PermissionRequired { request_id, .. }) =
            gate.check(actions::HOMEASSISTANT_TURN_ON, TARGET, "preview")
