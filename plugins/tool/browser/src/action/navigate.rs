...
impl NavigateAction {
    ...
    async fn run(&self) -> Result<String, ToolError> {
        ...
        page.goto(&self.url)
            .await
            .map_err(|e| ToolError::execution_failed(format!("Navigation failed: {e}")))?;

        let current_url = page
            .url()
            .await
            .map_err(|e| ToolError::execution_failed(format!("Failed to get URL: {e}")))?
            .unwrap_or_else(|| self.url.clone());

        let title = page
            .evaluate("document.title")
            .await
            .map_err(|e| ToolError::execution_failed(format!("Failed to get title: {e}")))?
            .into_value::<String>()
            .unwrap_or_default();
        ...
        let ready_state = page
            ...
            .await
            .map_err(|e| ToolError::execution_failed(format!("Failed to get readyState: {e}")))?
            .into_value::<String>()
            .unwrap_or_default();
