//! Minimal in-process HTTP mock of the `ElevenLabs` TTS API, used as a
//! test fixture. Compiled into the plugin's `#[cfg(test)]` module tree; it
//! is written without the test lint opt-outs so it stays production-clean.
...
/// Handle to an in-process fake `ElevenLabs` API.
...
impl Drop for MockElevenLabsServer {
    fn drop(&mut self) {
        self.task.abort();
    }
}
