//! Shared CPU-contract-test harness: pinned GGUF fixtures and a spawned
//! plugin process with a handshaken IPC stream.
//!
...
impl PluginSession {
    /// Spawns the real plugin binary and performs the v6 handshake with
    /// `acceleration: cpu` and the given profiles.
    pub async fn start(profiles: &Value) -> Self {
    ...
    /// Sends one request and reads its response (`MessagePack`, v6).
